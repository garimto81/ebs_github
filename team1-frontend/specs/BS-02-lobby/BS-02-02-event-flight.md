...
> **참조**: Flight/Table 엔티티 필드는 `contracts/data/DATA-02-entities.md §1.4 §1.5`.
...
Flight는 `is_pause: bool` 필드도 보유한다 (DATA-02 참조). Late Reg 남은 시간 계산 시 `is_pause == true`면 `elapsed_in_current_level` 증가가 **중단**된다. Flight 단위의 일시정지(브레이크/카메라 리셋/중재)에 사용된다.
...
- `contracts/data/DATA-02-entities.md §1.4 Flight` — 필드 정의
- `contracts/specs/BS-03-settings/BS-03-04-rules.md §5` — `BlindDetailType` + Late Reg 계산
- `contracts/api/API-01-backend-endpoints.md` — `/events`, `/flights` REST
