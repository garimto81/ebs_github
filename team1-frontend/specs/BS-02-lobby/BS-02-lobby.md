...
## UI 화면 설계 — 3계층 + Player 독립 레이어 + Settings 별도

### 설계 근거

**EBS 로비의 핵심 목적**: Feature Table을 찾아서 Command Center로 진입하는 것. 대회/이벤트는 테이블을 찾기 위한 탐색 경로이다.

**왜 3계층 + Player 독립 레이어인가**: EBS의 drill-down 구조는 `시리즈 > 이벤트(Day) > 테이블` 3계층이다. Event와 Flight는 **하나의 화면에서 accordion 펼침**으로 제공한다. Player는 Table의 하위가 아닌 **독립 레이어**로, 사이드바 등에서 상시 접근 가능하다. Settings는 테이블 종속이 아닌 **글로벌 단위**로 모든 CC에 동일 적용되므로 별도 설계한다.
...
| Player | Player List (독립 레이어) | Table 종속 아님, 어디서든 접근 |
