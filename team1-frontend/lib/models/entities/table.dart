...
class EbsTable with _$EbsTable {
  const factory EbsTable({
    ...
    @JsonKey(name: 'ante_type') @Default(0) int anteType,
    @JsonKey(name: 'ante_amount') @Default(0) int anteAmount,
    @JsonKey(name: 'rfid_reader_id') int? rfidReaderId,
    @JsonKey(name: 'deck_registered') @Default(false) bool deckRegistered,
    @JsonKey(name: 'output_type') String? outputType,
    @JsonKey(name: 'current_game') int? currentGame,
    @JsonKey(name: 'delay_seconds') @Default(0) int delaySeconds,
    int? ring,
    @JsonKey(name: 'is_breaking_table') @Default(false) bool isBreakingTable,
