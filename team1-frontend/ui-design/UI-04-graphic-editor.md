...
BS-08-00 §4 의 3-Zone 구조를 Quasar `q-page` + `q-splitter` 로 구현한다. 한 줄 65자 이내, mermaid 미사용.
...
- Quasar 컴포넌트: `q-toolbar` + 자식 `q-btn`, `q-input`, `q-select`.
- "Upload .gfskin" 버튼: Admin 에게만 노출 (GER-01). 클릭 시 `q-file` 트리거 또는 dropzone 오버레이 (§6).
- 검색 바: `q-input` `debounce="300"`, `clearable`. skin_name + version 에 대해 client-side filter.
- 필터: `q-select` `options=[All, Active, Inactive]`, 기본값 `All`.

**Zone 2 — Skin List (좌측 30%)**

- Quasar 컴포넌트: `q-list separator` → `q-item clickable v-ripple` 반복.
- 활성 스킨은 `● ` (녹색 `q-icon name="circle" color="positive"`) 아이콘. 비활성은 `○` (회색 outline).
- 행 레이아웃: `q-item-section avatar` (상태 아이콘) + `q-item-section` (skin_name + version) + `q-item-section caption` (author · 파일 크기).
- 선택 행은 `active` prop → Quasar primary background.
- 선택 변경 → `useGeStore.selectedSkinId` 업데이트 → Zone 3 리렌더.
- 리스트 하단: `q-infinite-scroll` (페이지네이션, BS-08-00 §4 Left Zone 언급). 초기 fetch: `GET /api/v1/skins?limit=50`.

**Zone 3 — Detail Pane (우측 70%)**

- 상단 50%: rive-js 프리뷰 canvas (§7)
- 하단 40%: 메타데이터 폼 (§5, `q-form` + `q-card` 섹션 분할)
...
| 960 ~ 1279 px | `q-splitter` 비율 40:60, 프리뷰 canvas 540×405 축소 |
| < 960 px | Zone 2 / Zone 3 를 `q-tabs` 로 분리. "List" 탭 ↔ "Detail" 탭 |
...
| ID | skin.json path | Quasar 컴포넌트 | 제약 | 예시 |
|----|----------------|------------------|------|------|
...
| ID | skin.json path | Quasar 컴포넌트 | 제약 | 예시 |
|----|----------------|------------------|------|------|
| GEM-04 | `resolution.width` + `.height` | `q-select` (single dropdown, `emit-value map-options`) | enum 쌍: {1920×1080, 2560×1440, 3840×2160}, 필수 | "1920 × 1080" |
| GEM-05 | `background.type` / `.color` / `.chromakey_color` / `.file` | `q-select` (type) + 조건부 `q-color` (popup) + `q-file` (image 업로드) | `type ∈ {image,color,chromakey}`, color `#hex6` | type=color, color="#101820" |
...
| GEM-14 | `colors.*` (1 optional key) | `q-input` (key 이름) + `q-color` (값) | additionalProperties | "badge_raise": "#FF00AA" |

**UI 규칙**:

- 각 컬러는 `q-field outlined` 내부에 좌측 색상 프리뷰 `q-avatar square color="{hex}"` + 우측 `q-input` (hex 문자열) + 우측 끝 `q-color` popup 트리거.
...
| ID | skin.json path | Quasar 컴포넌트 | 제약 | 예시 |
|----|----------------|------------------|------|------|
| GEM-15 | `fonts.pot.family` | `q-select` | enum: {Inter, Roboto, Noto Sans, Oswald, JetBrains Mono, Custom...} | "Inter" |
| GEM-16 | `fonts.pot.size` | `q-input type="number"` + suffix `px` | integer, 8~96, 필수 | 48 |
| GEM-17 | `fonts.pot.weight` | `q-select` | enum: {regular, bold, italic, bold-italic} | "bold" |
...
UI-00 §9 버튼 패턴 준수 (`q-btn unelevated color="primary"` Save, `q-btn flat` Revert).
...
모든 메타데이터 `q-input` / `q-select` / `q-color` 에 `:readonly="isOperator"` 또는 `:disable="isOperator"` 부여.
