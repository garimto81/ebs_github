...
| 요소 | Quasar 컴포넌트 | 바인딩 |
|------|-----------------|--------|
| 6자리 TOTP 입력 | `q-input` × 6 (auto-advance) 또는 `mask="# # # # # #"` 단일 `q-input` | `twofa.code` |
| Verify | `q-btn color="primary"` | `POST /auth/verify-2fa` |
| 취소 | `q-btn flat` | `temp_token` 폐기 → Login 화면 |
| 관리자 문의 링크 | `q-btn type="a" flat` | `mailto:$t('login.adminEmail')` |
| 실패 카운터 | `q-banner` | 3회 실패 시 30초 쿨다운 표시 |

i18n key: `$t('login.twofa.title')`, `$t('login.twofa.prompt')`, `$t('login.twofa.verify')`.

#### API 호출

- **요청**: `POST /auth/verify-2fa`
  ```json
  { "temp_token": "tmp_...", "totp": "123456" }
  ```
...
> **계약 확장 필요**: `POST /auth/verify-2fa` endpoint 명과 `temp_token` 필드는 BS-01-auth A-16 본문 서술에는 존재하나 API-06 Auth 엔드포인트 표에 별도 row 가 보이지 않는다. 구현 전 API-06 에 row 추가 확인 필요.
...
Login 화면의 "Forgot password?" 링크 클릭 시 `q-dialog` 모달 또는 별도 route `/forgot-password` 로 이동한다. 3단계 `q-stepper` 로 구성한다.
...
- **Quasar**: `q-dialog` + `q-stepper` + `q-input type="email"` + `q-btn`
- **API**: `POST /auth/forgot-password { "email": "user@example.com" }`
...
- **Quasar**: `q-input` mask + `q-countdown`(수동 tick) + `q-btn flat label="재전송"`
- **API**: `POST /auth/verify-reset-code { "email": "...", "code": "123456" }`
- **응답 200**: `{ "reset_token": "rst_..." }` → 단계 3
...
- **Quasar**: `q-input type="password"` + `q-icon name="visibility"` toggle + `q-chip` 강도 indicator
- **API**: `POST /auth/reset-password { "reset_token": "rst_...", "new_password": "..." }`
...
| 요소 | Quasar | 바인딩 |
|------|--------|--------|
| 다이얼로그 | `q-dialog persistent` | `showSessionRestore` |
| 항목 리스트 | `q-list dense` + `q-item` | `lastContext.*` |
| 상대 시간 표시 | `q-item-label caption` | `formatRelative(lastContext.timestamp)` |
| Continue | `q-btn color="primary"` | 해당 route 로 `router.push()` |
| 새로 시작 | `q-btn flat` | `/series` 로 이동 |

i18n key: `$t('login.restoreTitle')`, `$t('login.restoreContinue')`, `$t('login.restoreFresh')`.

#### API 계약

`POST /auth/login` 및 `POST /auth/verify-2fa` 응답 body 에 다음 필드를 포함한다 (BS-01 A-22 근거):
...
**Quasar**: `q-header` + `q-toolbar class="bg-red-9 text-white"` (높이 56px)
...
**Quasar**: `q-drawer side="left" bordered` + `q-list` + `q-expansion-item` (섹션별)
...
| 요소 | 타입 | 바인딩 |
|------|------|--------|
| Search | `q-input outlined dense` | `filter.search` — 시리즈명 실시간 필터 |
| Only updated | `q-checkbox dense` | `filter.onlyUpdated` — 최근 변경 시리즈만 (기준: 24h 이내 `updated_at`, GAP-L-015) |
| Show bookmarks | `q-checkbox dense` | `filter.bookmarked` — 북마크 시리즈만 |
| Reset | `q-btn flat dense icon="refresh"` | 모든 필터 초기화 |
...
| 요소 | Quasar | 바인딩 | 동작 |
|------|--------|--------|------|
| Event No | `q-input dense outlined` | `filter.eventNo` | 이벤트 번호 정확 검색 |
| Tournament Name | `q-input dense outlined` | `filter.name` | 이벤트명 부분 일치 |
| Mix | `q-select dense outlined` | `filter.mix` | Mix/Single 필터 |
| Game Type | `q-select dense outlined` | `filter.gameType` | 게임 유형 (HOLDEM, PLO4 등) |
| Tournament Type | `q-select dense outlined` | `filter.tournType` | 토너먼트 유형 (Freezeout, Re-entry 등) |
| Search | `q-btn color="primary" icon="search"` | — | 필터 적용 |
| Reset | `q-btn flat icon="refresh"` | — | 모든 필터 초기화 |
...
**Quasar**: `q-tabs v-model="statusFilter" dense` + `q-badge` (카운트)
...
**Quasar**: `q-table` + `body` slot 에서 `q-tr` 확장 행 (expand)
...
| Event Table | `q-table virtual-scroll` | `GET /series/{id}/events` |
| Pagination | `q-pagination` | `q-table` 내장 |
...
| 요소 | Quasar | 바인딩 |
|------|--------|--------|
| Day 탭 | `q-tabs v-model="selectedDay"` | `GET /events/{id}/flights` |
| 탭 라벨 | `q-tab` | `{flight.name}` + `{out_count} Out | {table_count} Tables` |
| [+ New Day] | `q-tab icon="add"` | 새 Flight 생성 다이얼로그 |
...
**Quasar**: `q-table` + `body` slot. 각 행은 Table, 각 열은 Seat 1~N. 셀은 `div class` 로 색상 표시.
...
> 근거: `contracts/api/API-01-backend-endpoints.md` §`POST /tables/rebalance` (CCR-010/CCR-020). G-05 gap 해결.
...
- **Quasar**: `q-btn color="orange-8" icon="shuffle"` + 비활성 조건 tooltip
- **비활성 조건**: flight.status != `Running`, 또는 `isRebalanceInProgress == true` (중복 실행 차단)

클릭 → 확인 다이얼로그(`q-dialog`) → `POST /tables/rebalance` 호출. `Idempotency-Key` 헤더에 `uuidv4()` 를 클라이언트가 생성해 첨부 (CCR-010 §멱등성 필수).
...
[상세 보기] 클릭 시 `q-dialog` 안에 `q-list` 로 각 step 을 표시한다.
...
각 step row 는 `q-item` + `q-item-section avatar/main/side` (duration). 실패 행은 `q-expansion-item` 으로 펼쳐 `error.code` / `error.message` 표시.
...
| + Add Player | `q-btn` | 플레이어 등록 다이얼로그 |
| Player Table | `q-table virtual-scroll` | `GET /players` 또는 `GET /events/{id}/players` |
...
> **✅ CCR-017 APPLIED (2026-04-10)** — `contracts/specs/BS-01-auth/BS-01-02-rbac.md`, `BS-02-lobby/BS-02-02-event-flight.md`, `BS-02-lobby/BS-02-03-table.md`, `BS-03-settings/BS-03-04-rules.md`, `contracts/data/DATA-02-entities.md` 5개 파일에 EventFlightStatus / isRegisterable / dayIndex / BlindDetailType / isPause / Bit Flag Permission 확장 완료. 승격본: `docs/05-plans/ccr-inbox/promoting/CCR-017-wsop-parity.md`.
...
`dayIndex` 는 해당 Event 내부의 Day 순서를 나타내는 0-based 정수이며, CCR-017 APPLIED 로 `contracts/data/DATA-02-entities.md` 의 Flight 엔티티에 `Flight.dayIndex: int` 필드가 추가되었다. 구현 시 곧바로 이 필드명을 사용한다.
...
`BlindDetailType` 5 타입(`Blind`, `Break`, `DinnerBreak`, `HalfBlind`, `HalfBreak`) 은 CCR-017 APPLIED 로 `contracts/specs/BS-03-settings/BS-03-04-rules.md` 에 enum 정의되었다. 구현 시 계약 파일의 enum 값을 참조한다.
...
- CCR-017 APPLIED 로 `contracts/specs/BS-01-auth/BS-01-02-rbac.md` 에 Bit Flag Permission enum(`None=0, Read=1, Write=2, Delete=4`) 이 추가되었다. 서버 응답의 `permission: int` 필드를 비트 연산으로 체크하여 UI 활성화를 판단한다.
...
`POST /auth/login` / `POST /auth/verify-2fa` / `GET /auth/session` 응답 body 에 `lastContext` 필드를 포함한다.
...
| Account Status | `q-toggle` | ✅ | Active/Disabled | Disabled = 로그인 차단 |
...
| CCR | 상태 | 변경 대상 | 관련 섹션 |
|-----|------|----------|----------|
| **CCR-017** wsop-parity (EventFlightStatus / isRegisterable / dayIndex / Restricted / BlindDetailType / isPause / Bit Flag RBAC) | ✅ APPLIED | `contracts/specs/BS-01-auth/BS-01-02-rbac.md`, `BS-02-lobby/BS-02-02-event-flight.md`, `BS-02-lobby/BS-02-03-table.md`, `BS-03-settings/BS-03-04-rules.md`, `contracts/data/DATA-02-entities.md` | §9.1 ~ §9.5 전체 |
...
| **CCR-011** ge-ownership-move (Graphic Editor Team 4 → Team 1 Lobby 허브 이관) | ✅ APPLIED | `contracts/specs/BS-08-graphic-editor/BS-08-00~04`, `BS-00-definitions.md` | Lobby 화면 범위(Graphic Editor 허브 추가), §권한(Admin/Operator/Viewer RBAC gate) |
| **CCR-025** bs03-graphic-settings-tab (BS-03-02-gfx 시각 asset 메타 확장) | ✅ APPLIED | `contracts/specs/BS-03-settings/BS-03-02-gfx.md` | (간접) Settings GFX 탭의 Team 4 기여 필드 |
