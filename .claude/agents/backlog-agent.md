...
**제외 대상 (블랙리스트):**
- 디렉토리: `guides/`, `.claude/`, `backlogs/`, `published/`, `.git/`
- 루트 파일: `README.md`, `CONTRIBUTING.md` 등 저장소 루트의 `.md` 파일

나머지 하위 디렉토리의 `.md` 파일이 기획 문서 대상입니다.
...
Confluence 조회:
  ...
  - 매칭 기준: 문서 제목과 Confluence 페이지 제목의 유사도
```

**매칭 후 반드시 사용자에게 결과를 보여주고 확인을 받습니다:**
```
매칭 결과:
  08_Rules/PRD-GAME-01-flop-games.md ↔ [EBS] Flop Games 계열 (ID: 3714154757) ✓
  05_Documents/poc-scenario.prd.md ↔ EBS POC Scenario (ID: 3719364781) ✓
  08_Rules/PRD-GAME-02-draw.md ↔ 매칭 없음 ✗
```

매칭 결과를 근거 문서 링크에 사용:
```markdown
- [08_Rules/PRD-GAME-01-flop-games.md](08_Rules/PRD-GAME-01-flop-games.md)
  — [Confluence](https://ggnetwork.atlassian.net/wiki/spaces/WSOPLive/pages/3714154757)
```
...
- **05_Documents/ 시나리오를 기준**으로 Feature를 도출한다 (사용자 플로우 기반)
- **08_Rules/ 게임 규칙은 참조 문서**로 사용한다 (Feature가 아닌 도메인 지식)
- 시나리오의 큰 단계가 Feature가 된다 (예: 방송 셋업, 핸드 진행, 쇼다운 등)
