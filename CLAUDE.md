...
1. **좀비 스캔** — `docker ps -a --format "table {{.Names}}\t{{.Image}}\t{{.Status}}" | grep "^ebs-"` → compose 에 없는데 돌고 있는 컨테이너 = 좀비. 즉시 stop + rm + rmi
2. **재빌드** — 내 소유 서비스 코드 변경 시:
   ...
   - team4: `docker compose --profile web build --no-cache cc-web && docker compose --profile web up -d cc-web`
...
- **compose 에서 제거했는데 런타임은 살려두기** — 좀비 원인 1순위 (2026-04-22 `ebs-lobby-web` 사건)
