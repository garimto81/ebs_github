...
> **SSOT**: `docs/2. Development/2.5 Shared/team-policy.json` (~~v7~~ → **v7.1**, `governance_model: free_write_with_decision_owner_or_conductor_full_authority`). Hook · `tools/*` 모두 이 파일 하나만 참조.
>
> **거버넌스 모델 (~~v7~~ → v7.1, 2026-04-27 SG-024)**: **2 모드 시스템**:
> - **Mode A — 단일 세션 (default in v7.1)**: Conductor 단독 활동 시 모든 영역 자율 진행 (team1~4 코드 + decision_owner override). 사용자 B-Q5 ㉠ 채택.
> - **Mode B — 멀티 세션 (옵션)**: 팀 세션 활성화 시 자동 회복. v7 모델 (decision_owner 권한 보호) 그대로.
>
> 위 표의 "owner" 컬럼은 **Mode B 의 decision_owner**. Mode A 에서는 Conductor 가 override 가능. 쓰기 제한은 hook 으로 차단하지 않는다 (계속 유효).
>
> **Mode A 한계 (Conductor 자율 금지)**: vendor 외부 메일 / destructive 시스템 변경 (DB drop, prod 배포) / git config / 사용자 인텐트 변경 / memory 사용자 본인 결정 메모 임의 폐기. 자세히: `team-policy.json` `mode_a_limits`, `Conductor_Backlog/SG-024-governance-expansion.md`.
