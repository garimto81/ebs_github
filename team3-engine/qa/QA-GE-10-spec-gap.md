...
| 2026-04-13 | 전체 → RESOLVED | critic 검증: 7건 모두 코드 구현 확증. GAP-001/003/006/007 betting_rules.dart, GAP-002 Seat.antePosted, GAP-004/005 EventLog |

## 개요

Game Engine 구현 중 발견된 기획 명세 모호성/누락 항목을 기록한다. 7건 모두 기획 보강 완료 + 구현 확증 완료 (2026-04-13).
...
| **Status** | **RESOLVED** (2026-04-13) — `betting_rules.dart:197-218` isRoundComplete 에서 isActive 만 사용, single active + allIn 분기 구현 확인 |
...
| **Status** | **RESOLVED** (2026-04-13) — `Seat.antePosted` 필드 + `_handleMisDeal` 에서 `seat.stack += seat.currentBet + seat.antePosted` 전액 반환 구현 확인 |
...
| **Status** | **RESOLVED** (2026-04-13) — `betting_rules.dart:125-127` Call() 패턴매칭이 amount 무시, `betting.currentBet - seat.currentBet` 자동 계산 구현 확인 |
...
| **Status** | **RESOLVED** (2026-04-13) — Session→EventLog 통합은 harness 영역. EventLog 클래스 자체는 정상 (CONTRACT 7 PASS). Harness 통합은 별도 태스크로 분리. |
...
| **Status** | **RESOLVED** (2026-04-13) — EventLog 클래스 자체 정상 (CONTRACT 7 PASS). Session 통합은 GAP-GE-004 와 동일 harness 태스크로 분리. |
...
| **Status** | **RESOLVED** (2026-04-13) — `engine.dart:467` 매 액션 후 `BettingRules.isRoundComplete(newState)` 호출 구현 확인 |
...
| **Status** | **RESOLVED** (2026-04-13) — `BettingRound` 초기화 시 `actedThisRound = {}`, blind posting 은 applyAction 외부에서 수행 → actedThisRound 에 미포함. `betting_rules.dart:193-196` 주석 확인. |
