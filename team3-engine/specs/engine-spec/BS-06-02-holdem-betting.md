...
| 2026-04-10 | WSOP 규정 반영 | §개요에 Rule 56 verbal/chip 우선순위 참조 주석 추가, §특수 상황 처리에 Rule 80-83 Time bank / At-seat 참조 추가. CCR-DRAFT-team3-20260410-wsop-conformance 참조 |
| 2026-04-10 | WSOP P0 규정 반영 | §5 RAISE에 5.1 Under-raise 50% Rule (Rule 95), 5.2 Raise Cap Tournament/Hand 경계 (Rule 100) 추가. §6 ALL-IN에 6.1 All-in Below Min Raise Action Reopen 금지 (Rule 96) 추가. CCR-DRAFT-team3-20260410-wsop-conformance P0-1/P0-2/P0-3 반영 |
...
### 베팅 입력 방식 (WSOP Rule 56 참조)

EBS는 CC(Command Center)의 전자식 입력을 유일한 공식 소스로 간주한다. WSOP Official Live Action Rules Rule 56은 구두/칩 동시 발생 시의 우선순위(구두 우선, 또는 먼저 발생한 쪽 우선)를 규정하지만, EBS 운영 환경에서는 다음과 같이 해석한다:

1. **라이브 테이블 (physical)**: 딜러 또는 CC 운영자가 Rule 56을 적용하여 verbal/chip 의도를 판단한 후, 결과를 CC에 **단일 이벤트**로 입력한다.
2. **EBS Engine**: CC가 전송한 이벤트를 재해석 없이 그대로 수락한다. Rule 56 재해석은 수행하지 않는다.
3. **CC UI 구현 권고** (Team 4 참조):
   - 구두 먼저 → 칩 금액 불일치 시 구두 우선 (Rule 56 전단)
   - Amount-only 선언은 동일 금액 call/raise로 자동 판정 (Rule 56.c "declaring 200 = silently pushing 200 chips")
...
#### 5.1 Under-raise 처리 (WSOP Rule 95)

**원칙**: CC가 제출한 raise 금액이 `min_raise_total` 미만인 경우, 해당 금액이 이전 raise의 50% 이상인지에 따라 자동 분기한다. 이는 WSOP Official Live Action Rules Rule 95에 근거한다.
...
- P1 bet 20 (previous_bet=20)
- P2 raise to 50 (previous_bet=50, last_raise_increment=30)
- P3 요청: raise to 65
...
CC의 raise 슬라이더는 `[min_raise_total, max_raise_total]` 범위만 허용하여 일반적으로 본 규정이 발동되지 않는다. 다음 경우에 엔진이 Rule 95를 자동 적용한다:
- 외부 API (harness, 시뮬레이터)가 임의 금액을 제출할 때
- CC UI가 advanced mode에서 수동 입력을 허용할 때
- RFID 칩 카운트 오인식으로 잘못된 금액 감지 시

##### All-in 예외

플레이어가 stack 전체를 push한 경우(`amount == stack`)는 본 규정이 아닌 **§6.1 All-in Below Minimum Raise (Rule 96)** 를 적용한다.

#### 5.2 Raise Cap 적용 (WSOP Rule 100)

**원칙**: FL 게임의 raise cap은 **"핸드 내 플레이어 수"** 가 아닌 **"전체 토너먼트의 남은 플레이어 수"** 로 판단한다. 이는 WSOP Official Live Action Rules Rule 100.b의 "토너먼트가 헤즈업이 되면(즉, 전체 토너먼트에 두 명의 참가자만 남음) 이 규칙이 적용되지 않습니다" 단서에 근거한다.
...
엔진은 `state.tournament_heads_up: bool` 필드를 참조하여 판정한다 (BS-06-00-REF Ch2.1 GameState 참조).
...
**캐시 게임 heads-up**은 `tournament_heads_up`가 항상 false이므로 본 규정상 cap 적용된다. 단, House 설정으로 "캐시 heads-up uncapped" 옵션을 제공할 수 있으며, 이 경우 별도 필드 `cash_heads_up_uncapped: bool` 추가를 검토한다.

##### `tournament_heads_up` 필드 설정 주체

- **BO (Backoffice)**: 토너먼트 생성 시 player 수 추적 → 2명 도달 시 WebSocket 이벤트로 각 engine harness에 `SET_TOURNAMENT_HEADS_UP { value: true }` 전파 (Team 2 구현, 후속 CCR 필요)
- **Lobby**: 토너먼트 최종 2인 상태를 UI에 표시
- **Engine**: 수동 설정 불가. BO 이벤트 수신으로만 변경
- **Cash game**: 항상 false 고정

기존 문서의 "num_active_players == 2" 표현은 **deprecated**이며 위 판정 표를 우선한다.
...
#### 6.1 All-in Below Minimum Raise — Action Reopen 금지 (WSOP Rule 96)

**원칙**: NL/PL 게임에서 all-in 금액이 `min_raise_total` 미만인 경우, 해당 베팅 라운드에서 **이미 행동한 플레이어에게 action을 재개하지 않는다**. 이는 WSOP Official Live Action Rules Rule 96의 "전체 레이즈 미만의 올인 베팅은 이미 행동한 참가자에게 베팅을 재개하지 않습니다" 조항에 근거한다.

##### 분기 조건

```
allin_size = player.stack  // all-in 금액 (current_bet 제외한 잔여 스택)
previous_bet = biggest_bet_amt  // 이전 베팅 금액
current_bet = player.current_bet  // 이미 낸 bet
...
if raise_increment >= min_full_raise_increment:
    ...
    betting.actedThisRound = {player.index}  // 재행동 기회 부여
    betting.lastAggressor = player.index
    betting.minRaise = raise_increment  // 새 기준
    betting.currentBet = raise_to
    betting.raiseCount += 1

else:
    # Incomplete all-in (Rule 96) — 단순 call matching만 수행
...
**상황**: NLH, BB=10, 3인 핸드
- P1 open raise to 30 (previous_bet=30, min_raise_total=50)
- P2 call 30
- P3 all-in 45 (stack=45)

**판정**:
- `raise_increment = 45 - 30 = 15`
- `min_full_raise_increment = max(10, 20) = 20`
- `15 < 20` → **Incomplete all-in (Rule 96)**

**결과**:
- `betting.currentBet = 45`
- `betting.actedThisRound` 유지: `{P1, P2, P3}` (P1/P2 재행동 기회 없음)
- P3는 acted 상태
- P1과 P2는 call 45로 매칭만 가능, raise 옵션 **없음**
- 액션 라운드 종료 → Flop 진행

**반면 P3가 all-in 50**이었다면:
- `raise_increment = 50 - 30 = 20 >= 20` → **Full raise**
- `betting.actedThisRound = {P3}`
- P1, P2에게 call/fold/re-raise 옵션 열림 (reopen)

##### FL(Fixed Limit) 게임 적용

FL 게임에서는 raise 증가분이 고정(`low_limit` 또는 `high_limit`)이므로 incomplete all-in은 `raise_increment < fixed_raise_amount` 조건으로 판별한다. Rule 100.b의 raise cap(§5.2)은 본 규정과 **독립적**으로 적용된다.

##### Side Pot 연동

Incomplete all-in도 여전히 side pot을 생성한다 (BS-06-06 Side Pot 참조). Action reopen 금지와 side pot 생성은 별도 규정이며 **병행 적용**된다.

##### 기존 §6 케이스와의 관계

본 §6.1은 기존 §6 ALL-IN의 "Soft all-in"(case 1) 처리를 WSOP Rule 96 기준으로 명확화한다. 기존 서술의 `all_in_amount < biggest_bet_amt` 분기는 §6.1 로직으로 대체된다.
...
### Time Bank / At-Seat (WSOP Rule 80-83 참조)

액션 타임아웃 및 플레이어 자리 규정은 WSOP Official Live Action Rules Rules 80-83을 따른다. EBS 엔진의 `state.action_timeout_ms` 필드는 이 규정의 구현 접점이다.

**Rule 80 — "Time" 호출**:
- 다른 플레이어가 "time"을 요청하면 딜러/CC는 30~60초 카운트다운 시작
- EBS 구현: CC UI의 "Call Time" 버튼이 수동 트리거. 별도 엔진 이벤트 불필요 (CC가 시각적 타이머만 제공).

**Rule 82 — "At Your Seat"**:
- 플레이어는 "의자에 닿거나 닿을 수 있는 거리" 이내에 있어야 라이브 핸드 참여 가능
- EBS 구현: 물리적 자리 감지는 Team 4 CC의 hardware layer 담당. 엔진은 검증 불가.

**Rule 83 — Action Pending 자리 이탈**:
- 플레이어가 action 대기 중 테이블을 떠나면 자동 폴드 처리 후 패널티
- EBS 구현: `action_timeout_ms` 초과 시 `TimeoutFold` 이벤트 자동 발행 (BS-06-09 IE-11 참조). 엔진이 Rule 83을 준수하는 자동 fold를 보장한다.

**CC UI 구현 책임**:
- "Call Time" 버튼 (Rule 80)
- 카운트다운 타이머 표시
- 자리 이탈 감지 시 수동 플래그 (선택)
- `TimeoutFold` 이벤트 발행 후 플레이어 복귀 시 패널티 추적 (감사용)
