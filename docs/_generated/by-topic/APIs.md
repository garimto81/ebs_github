---
title: APIs 집계
owner: ci
tier: generated
---

# APIs

| 경로 | Publisher | Legacy |
|------|-----------|--------|
| `docs/2. Development/2.2 Backend/APIs/Auth_and_Session.md` | team2 | API-06 |
| `docs/2. Development/2.2 Backend/APIs/Backend_HTTP.md` | team2 | API-01 |
| `docs/2. Development/2.2 Backend/APIs/Backend_HTTP_Status.md` | team2 | - |
| `docs/2. Development/2.2 Backend/APIs/Graphic_Editor_API.md` | team2 | API-07 |
| `docs/2. Development/2.2 Backend/APIs/WebSocket_Events.md` | team2 | API-05 |
| `docs/2. Development/2.3 Game Engine/APIs/Harness_REST_API.md` | team3 | API-04.2 |
| `docs/2. Development/2.3 Game Engine/APIs/OutputEventBuffer_Boundary.md` | team3 | API-04.3 |
| `docs/2. Development/2.3 Game Engine/APIs/OutputEvent_Serialization.md` | team3 | API-04.1 |
| `docs/2. Development/2.3 Game Engine/APIs/Overlay_Output_Events.md` | team3 | API-04 |
| `docs/2. Development/2.4 Command Center/APIs/RFID_HAL.md` | team4 | BS-04-04 |
| `docs/2. Development/2.4 Command Center/APIs/RFID_HAL_Interface.md` | team4 | API-03 |
