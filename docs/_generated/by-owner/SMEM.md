---
title: SMEM 소유 문서
owner: ci
tier: generated
---

# SMEM

- `docs/_journey/2026-05-11_smem_audit.md` — SMEM Weekly Memory Audit — 2026-05-11
- `docs/_journey/2026-05-11_smem_cycle2.md` — SMEM Cycle 2 — Cycle 1 weekly diff + case study
- `docs/_journey/2026-05-12_smem_cycle4.md` — SMEM Cycle 4 — Cycle 3+4 weekly diff + audit
- `docs/_journey/2026-05-12_smem_cycle5.md` — SMEM Cycle 5 — Cycle 4 case study + weekly diff
- `docs/_journey/2026-05-12_smem_cycle6.md` — SMEM Cycle 6 — Cycle 5 case study + weekly diff (architecture pivot + Iron Law 임계)
- `docs/_journey/2026-05-12_smem_cycle7.md` — SMEM Cycle 7 — Cycle 6 case study + v02 milestone 영구 기록
- `docs/_journey/2026-05-12_smem_cycle8.md` — SMEM Cycle 8 — Cycle 7 case study + v03 milestone + KPI
- `docs/_meta/doc-wiki/Index.md` — Doc Wiki Index -- 9 Topic SSOT
- `docs/_meta/doc-wiki/Log.md` — Doc Wiki Event Log
- `docs/_meta/doc-wiki/_schema.md` — Doc Wiki Schema -- Stream-Owned Topic Wiki
- `docs/_meta/doc-wiki/back-office.md` — Back Office Wiki SSOT
- `docs/_meta/doc-wiki/command-center.md` — Command Center Wiki SSOT
- `docs/_meta/doc-wiki/devops-ci.md` — DevOps CI Wiki SSOT
- `docs/_meta/doc-wiki/game-engine.md` — Game Engine Wiki SSOT
- `docs/_meta/doc-wiki/gap-writing.md` — Gap Writing Wiki SSOT
- `docs/_meta/doc-wiki/lobby.md` — Lobby Wiki SSOT
- `docs/_meta/doc-wiki/meta-memory.md` — Meta Memory Wiki SSOT
- `docs/_meta/doc-wiki/qa-e2e.md` — QA E2E Wiki SSOT
- `docs/_meta/doc-wiki/spec-gap-registry.md` — Spec Gap Registry Wiki SSOT
