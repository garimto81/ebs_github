...
| 요소 | 설명 | 예시 |
|------|------|------|
| **진행률 바** | n/52 시각적 표시 | ████████░░░░ 32/52 |
| **숫자 표시** | 스캔 완료 / 전체 | "32/52 카드 등록됨" |
| **최근 스캔 카드** | 마지막으로 스캔된 카드 표시 | "As (Ace of Spades)" |
| **누락 카드 목록** | 아직 스캔되지 않은 카드 | 4 suit × 미스캔 rank 목록 |
| **중복 경고** | 이미 스캔된 카드 재스캔 시 | "⚠ As 이미 등록됨" |
| **에러 표시** | 스캔 실패 시 | "✗ 카드 읽기 실패 (신호 약함)" |

### 4.2 Mock 모드 UI

Mock 모드에서는 진행률이 즉시 100%로 표시되며, "자동 등록 완료" 메시지가 나타난다.
...
| 영향 대상 | 이 문서와의 관계 |
|----------|----------------|
| `BS-04-00-overview.md` | RFID 전체 흐름에서 덱 등록 단계 |
| `BS-04-02-card-detection.md` | 등록된 덱 기반으로 카드 감지 |
| `BS-04-03-manual-fallback.md` | Mock 자동 등록과 수동 입력의 관계 |
| `BS-06-00-triggers.md §2.2` | DeckRegistered, DeckRegistrationProgress 이벤트 정의 |
| `API-03-rfid-hal-interface.md §2.1` | registerDeck() 메서드 계약 |
