...
| 2026-05-13 | **3.0.5** | **Cycle 21 Wave 1 — Reports 폐기 + Players/HandHistory DB 연동** (사용자 결정, 2026-05-13). 본 PRD 의 narrative spine `5 화면 시퀀스 + 4 진입 시점` 은 변경 0 — Reports 는 본 PRD 의 narrative 에 없음 (보조 화면). cascade 변경 영향: (A) `Overview.md` `UI.md` `EBS_Lobby_Design/README.md` 의 Reports 매핑 정정 + Hand History `lib/features/hand_history/` 독립 격상. (B) 신규 contract `Players_HandHistory_API.md` v1.0 (BO 2.2 APIs) — `GET /api/v1/hands` + `GET /api/v1/players` BO REST 직접 연동. 본 PRD 의 derivative-of (`Overview.md`) cascade 정합으로 last-updated/version 만 갱신. 후속: W2 backend impl + W3 frontend impl. additive only — narrative 변경 0 |
