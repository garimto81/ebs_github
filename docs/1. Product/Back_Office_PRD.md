...
CC 가 운영자 액션을 처리할 때, BO 와 Engine 양쪽에 동시에 dispatch 합니다 (Foundation Ch.5 §B.4). **Engine 응답을 진실로 받아들이고, BO WS 응답은 audit 참고값으로 사용**합니다. 둘이 다르면? Engine 이 이깁니다.
...
| 2026-05-05 | 1.1.0 | EBS 미션 재선언 cascade (SG-033). §7.1 SLO 표 헤더 "목표" → "**운영 메트릭**" + "표기 주의" 박스 추가 (NFR 보존, 핵심 가치 아님 명시). §7.2 챕터명 "100ms 약속의 의미" → "**안정된 동기화의 의미**". 본문 "운영자" → "**CC 오퍼레이터**" (역할 분리). Foundation Ch.1 Scene 4 새 미션 ("완벽한 번역가") 정합. 속도 KPI 표현을 운영 메트릭(NFR)으로 강등, EBS 핵심 가치는 정확성·안정성·단단한 HW. |
