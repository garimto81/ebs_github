...
### 4.4 핸드 사이의 전환 — multi-hand state

방송은 한 핸드로 끝나지 않습니다. 한 판이 끝나고 다음 판이 시작되는 사이, BO 는 두 가지 일을 합니다 — **(가)** 직전 핸드를 완료(close) 처리하고, **(나)** 다음 핸드의 셋업을 준비합니다. 이 전환은 12 시간 방송 동안 800~1,200 회 반복됩니다.

```mermaid
flowchart LR
    H1["Hand N<br/>(진행 중)"] --> Done["Hand N close<br/>(승자/팟 확정)"]
    Done --> Btn["Dealer Button<br/>시계 방향 1칸 회전"]
    Btn --> Setup["Hand N+1 setup<br/>(블라인드 재배치)"]
    Setup --> H2["Hand N+1<br/>(시작)"]
```

#### 4.4.1 BO 가 보관하는 multi-hand state

```
  +----+--------------------+------------------------------+
  |  # | 필드               | 의미                          |
  +----+--------------------+------------------------------+
  | 1  | currentHandNumber  | 현재 진행 중인 핸드 번호       |
  | 2  | dealerSeat         | 현재 딜러 버튼 좌석 (0~8)      |
  | 3  | sbSeat / bbSeat    | SB / BB 좌석 (dealerSeat 파생) |
  | 4  | handHistory[]      | 직전 N개 핸드 요약 리스트       |
  | 5  | nextHandReady      | 다음 핸드 준비 완료 여부        |
  +----+--------------------+------------------------------+
```

multi-hand state 는 BO DB 의 `tables` 테이블에 컬럼 단위로 보관되며, 핸드 사이의 전환 단계에서 갱신됩니다. `handHistory[]` 는 별도 `hands` 테이블의 최근 row 를 reverse chronological 로 조회한 결과입니다 (정본 schema 는 `2. Development/2.2 Backend/Database/Schema.md` 참조).

#### 4.4.2 POST /next-hand endpoint

핸드 전환의 트리거는 CC 오퍼레이터의 명시 액션 또는 자동 진행 모드입니다. 어느 경우든 BO 의 `POST /api/v1/tables/{id}/next-hand` 엔드포인트가 단일 진입점입니다.

| 입력 | 의미 |
|------|------|
| `tableId` | 전환 대상 테이블 ID |
| `prevHandId` | 직전 핸드 ID (멱등성 보장 — 동일 prevHandId 두 번 호출 시 두 번째는 no-op) |
| `triggerSource` | `cc_operator` / `auto_demo` / `system_recovery` (audit 용) |

| 처리 | 단계 |
|------|------|
| 1 | 직전 핸드 close (승자/팟/통계 확정) |
| 2 | dealerSeat 시계 방향 1칸 회전 (탈락한 좌석 skip) |
| 3 | sbSeat / bbSeat 재계산 (Heads-up 예외 적용) |
| 4 | 새 hand row 생성 (currentHandNumber++) |
| 5 | WS broadcast `hand_started` (Lobby + Overlay 구독) |

> **멱등성**: 같은 `prevHandId` 로 두 번 호출되어도 hand 가 중복 생성되지 않습니다. 네트워크 재전송 / CC crash 복구 시나리오에서 안전합니다.

#### 4.4.3 자동 진행 모드 (auto_demo)

`HAND_AUTO_SETUP=true` 환경에서는 한 핸드의 정상 종료 (showdown 또는 전원 fold) 직후 자동으로 `POST /next-hand` 가 호출됩니다. 데모/검증용이며, 정규 방송 운영에서는 CC 오퍼레이터의 명시 액션이 항상 트리거입니다 (감사 가능성 보장).
...
| 2026-05-12 | 1.2.0 | **Cycle 7 #326 — multi-hand state PRD 정합 (S10-W)**. §4.4 신규 섹션 추가 — 핸드 사이의 전환, 5 필드 multi-hand state (currentHandNumber / dealerSeat / sbSeat·bbSeat / handHistory[] / nextHandReady), `POST /api/v1/tables/{id}/next-hand` 엔드포인트 명세 (5 단계 처리 + 멱등성 보장 + triggerSource audit), §4.4.3 자동 진행 모드 (auto_demo) 명세. S8 #319 v03 multi-hand engine + S7 next-hand endpoint + S3 #321 CC handNumber cascade 정합 (cycle 6 머지 후속). additive only — 기존 narrative 변경 0. derivative-of 정본 = `2. Development/2.2 Backend/Back_Office/Overview.md` (next-hand endpoint 정본 + Database/Schema.md 의 hands/tables 테이블 컬럼 정본). | PRODUCT |
