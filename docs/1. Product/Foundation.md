...
이번 챕터는 4 단계로 풀어냅니다 — §1.1 정보 비대칭성의 딜레마, §1.2 시청자에게 전달할 3 핵심 데이터, §1.3 1세대 유리판에서 2세대 RFID 로의 기술 진화, §1.4 완벽한 번역가 미션.
...
> *§1.3 의 RFID 가 카드를 정확하게 읽는다면 — §1.4 가 그 정확성의 의미를 미션으로 선언합니다.*
...
flowchart LR
    ...
    B[CC 오퍼레이터<br/>액션 입력] --> D
    C[포커<br/>규칙] --> D
    D --> E["정확하고 안정된<br/>방송 그래픽"]
```
...
센서가 읽어 들인 카드 정보, **테이블 후방 컨트롤룸에서 CC 오퍼레이터가 입력한** 베팅 액션, 그리고 복잡한 포커 규칙이 만나 **"누가 이길 확률이 가장 높은가?"** 가 결정됩니다.

> 딜러는 테이블에서 플레이어 액션을 리드·처리하는 진행자이며, 시스템 입력자는 **컨트롤룸의 CC 오퍼레이터** 입니다. 두 역할은 분리되어 있습니다.

EBS의 미션은 명확합니다. 테이블 위에서 일어나는 이 모든 물리적 상황과 아날로그 데이터를 수집하여, **카드 1장도 빠짐없이, 액션 1건도 누락없이, 단단한 장비 사슬을 통해 정확하게 방송 그래픽으로 번역해 송출** 하는 것입니다.
...
> **표기 주의** (2026-05-05): 본 NFR 수치는 **운영 안정성 측정 메트릭** 이며 EBS 핵심 가치 아니다. EBS 미션은 §Ch.1.4 — 정확성·안정성·단단한 HW 다섯 가치. 시스템 동작 기준점으로만 보존.
