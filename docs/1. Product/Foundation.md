...
reimplementability_notes: "v3.1 — 3 입력 Trinity 명시 (Ch.1 4→6 단계) + Vision Layer §7.3 신설 + Ch.9 2축 진화 정합. 게임 룰 = Engine 내장 상수 SSOT 확정. 외부 PRD 3종 동기화 필요."
---
...
![CC 오퍼레이터 컨트롤룸](images/foundation/app-command-center.png)
...
```mermaid
flowchart LR
    subgraph S1["1단계 — Trinity 모델 (현재)"]
        A1[RFID<br/>카드 인식] --> E1[Game<br/>Engine]
        B1[CC 오퍼레이터<br/>액션 입력] --> E1
        C1[(포커 룰<br/>내장 상수)] -.해석.-> E1
        E1 --> O1[방송<br/>그래픽]
    end
    subgraph S2["2단계 — 무인화 모델 (목표)"]
        A2[RFID<br/>카드 인식] --> E2[Game<br/>Engine]
        B2["CV 카메라<br/>액션 자동 인식"] --> E2
        C2[(포커 룰<br/>내장 상수)] -.해석.-> E2
        E2 --> O2[방송<br/>그래픽]
    end
```

> *FIG · 1단계 = 사람이 액션 입력, 2단계 = 카메라가 액션 인식*
...
본 Trinity 모델은 **1단계 운영 방식** 입니다. EBS 의 궁극 지향점은 **현장 EBS 의 완전 무인화** 이며, 2단계에서는 CC 오퍼레이터의 베팅 액션 입력이 **컴퓨터 비전 카메라** (§7.3 Vision Layer) 로 완전 대체됩니다. 1단계 → 2단계 전환은 1단계 완전 안정화 후 순차 진행하며, 2단계는 1단계를 완전 대체합니다 (병행 운영 없음).
...
본방송 중 운영자 시선의 85% 가 머무는 화면. 테이블 1개당 1 인스턴스 독립 실행. **1단계 입력 모델** 의 핵심 컴포넌트 (§1.4 참조). 2단계 무인화 진화 시 본 화면은 모니터링 전용으로 전환됩니다 (§7.3 / §9.2).
...
시스템 내부의 두뇌가 아무리 뛰어난 연산을 해내더라도, **그것을 시청자가 볼 수 있게 그려내거나 현실의 카드를 읽어오지 못하면 아무 소용이 없습니다.** 이번 챕터에서는 가상의 디지털 세계와 현실 세계를 연결하는 세 가지 접점을 살펴봅니다.

이번 챕터는 3 단계로 풀어냅니다 — §7.1 화면을 그리는 붓 (Overlay View, Rive → SDI/NDI), §7.2 물리적 세계와의 접점 1 — RFID 12 안테나 (1·2단계 공통), §7.3 물리적 세계와의 접점 2 — **Vision Layer** (2단계 무인화 — 6대 카메라 + CV).
...
> *§7.2 의 RFID 가 카드 인식이라면 — §7.3 이 2단계 무인화의 핵심 신규 컴포넌트인 Vision Layer 를 풀어냅니다.*

<a id="ch-7-3"></a>

<!-- FB §7.3 · Vision Layer (2단계 무인화) -->
<table role="presentation" width="100%">
<tr>
<td width="50%" valign="middle" align="left">

**§7.3 · 물리적 세계와의 접점 2 — Vision Layer (2단계)**

#### 2단계 무인화의 핵심 신규 컴포넌트

§1.6 의 2단계 진화 시 추가되는 입력 채널. 1단계 CC 오퍼레이터가 수동 입력하던 8 액션을 **6대 카메라 + 컴퓨터 비전** 이 자동 인식하여 Game Engine 에 직접 전달합니다.
...
| 2 | **탑 뷰 촬영** | **1** | 테이블 전체 (칩 트레이 / 카드 위치) | **별도 (전용)** |
| 3 | **커뮤니티 카드 촬영** | **1** | 보드 카드 | **공유** |
|   | **합계** | **6** |   |   |

#### 1단계 CC 오퍼레이터 → 2단계 CV 매핑

| 1단계 입력 | 2단계 인식 카메라 |
| --- | --- |
| 핸드 시작 / 종결 | 탑 뷰 (딜러 동작 인식) |
| 카드 배분 | 탑 뷰 + 커뮤니티 (RFID 와 cross-check) |
| 콜 / 레이즈 / 폴드 / 체크 / 올인 | 플레이어 4대 (제스처 + 칩 푸시) |

#### 정확도 목표

**사람의 수정 없이 방송에 문제 없는 상태** — 휴먼 인터벤션 0. 정량 KPI 가 아니라 운영 품질 정성 기준.
...
| RFID 12 안테나 (§7.2) | ✅ | ✅ (유지) |
| CC 오퍼레이터 입력 (§5.4) | ✅ | ❌ (제거) |
| Vision Layer 6 카메라 (본 §) | ❌ | ✅ (신규) |
| Game Engine (§6.1) | ✅ | ✅ (유지) |

> 2단계 진입 시 CC 오퍼레이터 화면은 **모니터링 전용** 으로 전환. 입력 권한 없음. 1단계 완전 안정화 후 순차 진행하며 병행 운영하지 않습니다 (§9.2 참조).
...
> *FIG · 6 카메라 → CV 추론 → Engine. 플레이어 4대 + 탑뷰 1대 = 별도, 커뮤니티 1대 = 프로덕션 공유*

```mermaid
flowchart LR
    subgraph S1["1단계 입력 채널"]
        R1[RFID 12 안테나] --> Eng1[Engine]
        CC[CC 오퍼레이터<br/>8 액션 버튼] --> Eng1
    end
    subgraph S2["2단계 입력 채널"]
        R2["RFID 12 안테나<br/>(유지)"] --> Eng2[Engine]
        CV["Vision Layer<br/>6 카메라 + CV"] --> Eng2
    end
    S1 ==>|"순차 전환<br/>(완전 대체)"| S2
```

> *FIG · 1단계 → 2단계 순차 진화. RFID 유지, CC → CV 완전 대체*

</td>
</tr>
</table>

> *§7.3 으로 입력 측 신규 무인화 채널까지 정의 — Part II 개발 대상 설명을 마칩니다. Part III 부터는 운영과 미래입니다.*
...
| **데이터 통제** | 외부 의존 | 100% 통제 |
| **입력 방식 (1단계)** | 오퍼레이터 수동 입력 | **동일 — PokerGFX 모델 채택** |
| **입력 방식 (2단계)** | (영구 수동) | **CV 무인화 — 카메라 6대 + 컴퓨터 비전 (§7.3)** |
...
| **Y2** | **CV 무인화 (완전 대체)** | RFID + Vision Layer 6 카메라 (§7.3) |
...
2. **운영의 완전한 무인화 (현장 EBS 완전 무인화)** — 1단계 (Y1) 의 CC 오퍼레이터 수동 입력을 **2단계 (Y2) Vision Layer (6대 카메라 + 컴퓨터 비전, §7.3)** 가 완전 대체합니다. 메커니즘은 막연한 "AI 가 처리" 가 아니라 **CV 카메라 6대로 액션 / 제스처 / 칩 푸시 / 보드 카드 자동 인식**. 정확도 목표는 휴먼 인터벤션 0 — 사람 수정 없이 방송에 문제 없는 상태.
