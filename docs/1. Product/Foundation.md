...
reimplementability_notes: "v5.0 — Ultrathink Architecture cascade: Ch.4 신규 Scene 3 (Architecture Stack 7-Layer INHERIT/ADAPT/REPLACE) + Scene 4 (Gen 3 DDD/CQRS target) + 7-app vs 4-component 매핑 + §B.1 4 primitives footnote + §B.3 Field-based batch + §A.3 다중 skin + §A.4 DRM 직교 + §C.2 RFID 보안 회피. PokerGFX 정본 (archive/legacy-repos/ebs_reverse, 3000줄) 직접 인용 16+ 위치. v4.5 = Phase B cascade (SSOT 100% 정합). v4.4 = Phase A cascade. v4.3 = C.2 RFID 이미지. v4.2 = A.1/A.2 스크린샷 + Vision Layer. v4.1 = Ch.4 3 그룹 6 기능. v4.0 = Graphic Novel 재설계."
---
<p align="center">
...
flowchart TD
    ...
    C["CC 오퍼레이터<br/>6 키 (N·F·C·B·A·M)"] --> Eng
...
칩 더미의 작은 움직임 + 침묵 속 손짓 — 센서로 자동 판별 불가. 그래서 **컨트롤룸의 CC 오퍼레이터**가 모니터로 테이블 영상을 보면서 **6 키 (N · F · C · B · A · M)** 를 누른다 — 5-Act 시퀀스 (Hand Start → Deal → Bet → Showdown → Hand End) 단계에 따라 키 의미가 동적으로 변경된다 (CC_PRD v4.0).
...
| **B** | Bet (사이즈 입력 모달) | Bet Act |
| **A** | All-in | Bet Act |
| **M** | More (Deal/Showdown/HandEnd 보조) | Deal/Showdown/Hand End Act |

> *딜러는 테이블 진행자, CC 오퍼레이터는 컨트롤룸 입력자. 두 역할은 물리적으로 분리된다.*

> 💡 **5-Act = L6 Mental Model** (운영자 인지 추상화). L2 Engine FSM 은 §B.1 + Command_Center.md Ch.6 (10-state + runItMultiple + 게임 클래스 분기). L5 Frontend Display 는 PokerGFX AT Stateless Terminal 패턴 차용. Architecture 전체 stack = Ch.4 Scene 3 "7-Layer" 참조.
...
> *Chapter 2 가 시청자가 보는 결과물 — 9 카테고리 그래픽을 해부한다.*
...
### Scene 1 / 9 핵심 카테고리 (사용자 표 v1.0.0 SSOT)

![오버레이 anatomy](images/foundation/overlay-anatomy.png)

EBS 는 카메라 원본 영상 위에 실시간으로 포커 데이터를 덧그린다. 현장에서 발생하는 물리적 상황을 즉시 반영하는 **9 핵심 카테고리** 가 우리가 직접 생성하는 결과물이다.

**v2.0 갱신 (2026-05-13, Cycle 16 Wave 2)** — 기존 "8 그래픽" 분류 → **9 카테고리 (사용자 표 v1.0.0)** 로 동기화. 핵심 변경:
- 기존 "플레이어 정보 + 플레이어 위치" 2 항목 → 새 **#1 플레이어 대시보드** (Name + 국적 + 포지션 SB/BB/D + 칩스택) 1 항목으로 통합
- 기존 "아웃츠" → **#4 실시간 승률 통계 (Equity)** 안으로 흡수 (Equity 가 outs 의 상위 개념)
- 신규 **#6 이벤트 브랜딩** (WSOP/APT 로고) + **#7 블라인드 정보 창** (블라인드 레벨 + 앤티) + **#9 필드 현황판** (참가자/ITM/FT) 추가

| # | 카테고리 (사용자 표 verbatim) | 트리거 / 데이터 소스 |
|:---:|---|---|
| 1 | **플레이어 대시보드** (Name + 국적 + 포지션 SB/BB/D + 칩스택) | DB (이름·국가) + Engine (position, stack 자동 계산) + **WSOP LIVE 브레이크 chip count push 동기화 (SSOT)** + RFID (좌석) |
| 2 | **핸드 그래픽** (RFID 홀카드) | RFID 센서 카드 감지 즉시 (private card) |
| 3 | **액션 인디케이터** (체크/벳/레이즈/폴드) | CC 오퍼레이터 액션 입력 (Check/Bet/Raise/Fold 4 종) |
| 4 | **실시간 승률 통계** (Equity) | Engine 몬테카를로 계산 — 카드 공개마다 갱신 (기존 "아웃츠" 흡수) |
| 5 | **보드/커뮤니티 영역** (Flop/Turn/River 공용 카드) | RFID 보드 카드 인식 (Flop=3장, Turn=4장, River=5장) |
| 6 | **이벤트 브랜딩** (WSOP/APT 로고) | DB Brand Pack (sponsor logos / tournament names) |
| 7 | **블라인드 정보 창** (블라인드 레벨 + 앤티) | DB Blind Structure 스케줄 + Engine 레벨 카운터 |
| 8 | **중앙 팟 정보** (총 베팅 칩) | Engine 누적 베팅 자동 계산 (Main + Side) |
| 9 | **필드 현황판** (참가자/ITM/FT) | DB 총 entries + Engine 남은 인원 / ITM 컷 / FT 도달 |

> *이 9 카테고리가 EBS 가 책임지는 시청자 화면 영역의 전부다.*

> **정합 cross-reference**: RIVE_Standards.md Ch.2 (v0.8.0) 의 9 카테고리 SSOT 와 1:1 매핑. 정의 범위 통일 — Foundation = "EBS 직접 생성 (실시간 트리거)" / RIVE_Standards = "Rive 자산 측 분류". 같은 9 카테고리 두 시각.

> *#1 chipstack sync contract: `docs/2. Development/2.2 Backend/APIs/WSOP_LIVE_Chip_Count_Sync.md` / state machine: `docs/2. Development/2.5 Shared/Chip_Count_State.md`*

> **Auxiliary (시청자 비노출)**: Clock (Hand/Level Clock) + Operator Marks 는 사용자 표 9 카테고리에 포함 안 됨 — Clock 은 JSON 자막 채널, Operator Marks 는 운영자 도구 viewport. RIVE_Standards Ch.20 / Ch.22 참조.
...
화면에는 9 카테고리 외에도 화려한 그래픽이 많다. 리더보드, 선수 프로필, 탈락 위기 경고, 자막 — **하지만 이것들은 EBS 가 만들지 않는다**.
...
> *이 세 가지를 모두 충족하는 9 핵심 카테고리만이 EBS 의 온전한 개발 대상이다.*
...
| Scene | 핵심 |
|---|---|
| **1. 9 카테고리** | 플레이어 대시보드 / 핸드 그래픽 / 액션 인디케이터 / 실시간 승률 / 보드·커뮤니티 / 이벤트 브랜딩 / 블라인드 정보 / 중앙 팟 / 필드 현황판 |
...
| 기능 | 그룹 | 담당 팀 | 스택 |
|---|---|:---:|---|
| **Lobby** (5분 게이트웨이 + WSOP LIVE 거울) | 조작 | team1 | Flutter Web + Rive |
| **Command Center** (1×10 가로 그리드 + 6 키 동적 매핑) | 조작 | team4 | Flutter Desktop + Rive |
...
### Scene 3 / Architecture Stack — 7-Layer

EBS = "기존 PokerGFX 패러다임의 자체 구현" (§C.3 line 904). 그 패러다임은 역공학 분석 결과 **7 층 구조**로 분해된다. 각 층은 **INHERIT / ADAPT / REPLACE** 결정으로 EBS 에 매핑된다.

```
┌─────────────────────────────────────────────────────────────┐
│ Layer 7: SKIN / THEME                                       │
│   PokerGFX: INTEGRATED .skn (99+ fields, AES 암호화)        │
│   EBS:      MODULAR .riv (Rive Manager, §A.3)               │
│   결정:     REPLACE — 다중 skin 자연 지원                   │
├─────────────────────────────────────────────────────────────┤
│ Layer 6: OPERATOR MENTAL MODEL                              │
│   PokerGFX: 5-Act (Setup/Deal/Action/Showdown/Complete)     │
│   EBS:      Foundation 5-Act (Ch.1 Scene 3 ⓑ)               │
│   결정:     INHERIT — 직접 차용                             │
├─────────────────────────────────────────────────────────────┤
│ Layer 5: FRONTEND DISPLAY                                   │
│   PokerGFX: AT (ActionTracker) — Stateless Input Terminal   │
│             "AT는 결과를 표시한다. 서버가 모든 것을 처리"   │
│   EBS:      Flutter CC (game logic 보유 X, Engine 결과 표시)│
│   결정:     ADAPT — Stateless 패턴 차용                     │
├─────────────────────────────────────────────────────────────┤
│ Layer 4: WIRE PROTOCOL                                      │
│   PokerGFX: net_conn 113+ commands + GameInfoResponse 75+   │
│             FIELD-based batch (state enum 아님)             │
│   EBS:      REST + WS (§B.3) — Field-based batch 패턴 차용  │
│   결정:     REPLACE — 자체 구현, 패턴 차용                  │
├─────────────────────────────────────────────────────────────┤
│ Layer 3: BACKEND SERVICE                                    │
│   PokerGFX: 10 services (Phase 2) + DDD/CQRS (Phase 3)      │
│   EBS:      Engine + BO — Gen 3 target (Scene 4 참조)       │
│   결정:     ADAPT — Gen 3 채택                              │
├─────────────────────────────────────────────────────────────┤
│ Layer 2: ENGINE GAME STATE                                  │
│   PokerGFX: GameTypeData 79 fields + 8-state FSM            │
│             + runItMultiple 분기 + Draw flag/counter + Stud  │
│   EBS:      Engine GameState (CC Ch.6 10-state)             │
│   결정:     ADAPT — 자체 spec                               │
├─────────────────────────────────────────────────────────────┤
│ Layer 1: HARDWARE                                           │
│   PokerGFX: RFIDv2 + boarssl TLS (vendor-locked, 15 취약점) │
│   EBS:      자체 RFID (ST25R3911B + ESP32, §C.2)            │
│   결정:     REPLACE — 보안 취약점 회피                      │
└─────────────────────────────────────────────────────────────┘
```

> 💡 **INHERIT 1 (L6) + ADAPT 3 (L5/L3/L2) + REPLACE 3 (L7/L4/L1) = 7 결정**.
>   본 프로젝트 인텐트 "PokerGFX 패러다임의 자체 구현" 의 architecture 차원 명시.
>   각 layer 의 PokerGFX 정본 인용: `C:/claude/ebs-archive-backup/07-archive/legacy-repos/ebs_reverse/docs/02-design/pokergfx-reverse-engineering-complete.md`

---

### Scene 4 / Architecture Generation — Gen 3 (DDD/CQRS) target

PokerGFX 역공학에서 발견된 **3-generation 아키텍처 진화**:

| Generation | 특징 | PokerGFX 위치 |
|:----------:|------|---------------|
| **Gen 1** God Class | `main_form` 329 methods + 398 fields, 거의 모든 비즈니스 로직 집중 | 레거시 |
| **Gen 2** Service Interface | GameTypes 디렉토리 26 파일, 10 interfaces + 11 implementations | 점진적 분해 |
| **Gen 3** DDD/CQRS | Features 디렉토리 58 파일, FluentValidation + MediatR | 최신 |

→ **PokerGFX = Gen 1/3 hybrid 공존** (점진적 마이그레이션 중).
→ **EBS target = Gen 3 (DDD/CQRS)** 명시. §B.3 통신 매트릭스의 "CC = Orchestrator, 병행 dispatch" = Gen 3 패턴 정합.

> 💡 상세 마이그레이션 가이드: `docs/2. Development/2.5 Shared/Architecture_Generations.md`

---

### Scene 5 / PokerGFX 7-Application Ecosystem vs EBS 4-Component

PokerGFX = 단일 앱이 아닌 **7 application ecosystem**. EBS = **4 component** 로 의도적 단순화.

| PokerGFX App | 내부 키 | EBS 대응 | 매핑 |
|------|---------|----------|------|
| GfxServer | `pgfx_server` | **BO + Overlay** | ✅ 직접 대응 |
| ActionTracker | `pgfx_action_tracker` | **Command Center** | ✅ 직접 대응 |
| HandEvaluation | `hand_eval_wcf` | **Engine** | ✅ 직접 대응 (자체 구현) |
| ActionClock | `pgfx_action_clock` | — | ❌ EBS 미정의 (Y2 후보) |
| StreamDeck | `pgfx_streamdeck` | — | ❌ EBS 미정의 (Y2 후보) |
| Pipcap | `pgfx_pipcap` | — | ❌ 의도된 out-of-scope (포스트프로덕션) |
| CommentaryBooth | `pgfx_commentary_booth` | — | ❌ 의도된 out-of-scope (EBS 배제 정책) |
| — | — | **Lobby** | ✨ EBS 신규 (5분 게이트웨이) |

> 💡 EBS = PokerGFX 5 컴포넌트 대응 + Lobby 신규 + 2 의도된 out-of-scope (Pipcap/CommentaryBooth).
>   ActionClock / StreamDeck = Y2 단계 후보 (Vision Layer 도입 시 검토).
...
| **3. 7-Layer Architecture** | INHERIT 1 + ADAPT 3 + REPLACE 3 (PokerGFX 패러다임 자체 구현 차원) |
| **4. Gen 3 DDD/CQRS** | EBS target generation 명시 |
| **5. 7-app vs 4-component** | EBS 의도적 단순화 + Lobby 신규 + out-of-scope 2 |
...
> *Series → Event → Flight → Table 4 단계 WSOP LIVE 거울. 시프트 시작 5분 안에 룰·좌석·RFID 검증 → CC 위임. (S2 작업 중인 실제 스크린샷)*
...
> 💡 **MODULAR Skin 패턴** (Ch.4 Scene 3 L7): Rive Manager 는 **다중 skin 자연 지원**. Lobby = B&W refined minimal skin, CC = Broadcast Dark Amber skin 이 **별도 .riv 파일로 공존**. PokerGFX 의 INTEGRATED `.skn` (단일 binary AES) 과 패러다임 차이. 상세: `RIVE_Standards.md` Ch.5.5 "INTEGRATED vs MODULAR".
...
#### Hole Card Visibility — 4 단 방어 (운영자 부정 방지)

CC 운영자가 시청자보다 먼저 홀카드를 볼 수 있다면 부정 가능. 4 단 방어 (CC_PRD v4.0 D7 흡수):
...
| 4 물리 영역 | CC 모니터 = 시청자 / 딜러 시야 차단 부스 (실내 디자인 강제) |

> 💡 **DRM 직교 관계**: EBS 4단 방어 ≠ PokerGFX 4계층 DRM (역공학 분석 §10). PokerGFX = **제품 라이선스 보호** (Email/Password + Offline Session + KEYLOK USB + License 시스템). EBS = **운영 무결성** (시청자보다 운영자가 먼저 홀카드 못 보게). 두 모델은 보호 대상이 다른 **직교 관계** — EBS 가 PokerGFX DRM 미차용은 의도된 선택. 상세: `docs/2. Development/2.5 Shared/Security_Posture.md`.
...
핸드 종료 시 cycle 따라 다음 게임 자동 전환. Engine 내장 변수.

| 모드 | 순환 게임 | 베팅 구조 |
|------|----------|----------|
| HORSE | Hold'em / Omaha / Razz / Stud / Stud Hi-Lo (5 종) | FL (모두 고정) |
| 8-Game | NLHE / PLO / Razz / Stud / Stud Hi-Lo / 2-7 Triple Draw / Limit Hold'em / Omaha 8/B (8 종) | NL / PL / FL 혼재 |

> 💡 **Mixed Game 전환 단위 = PokerGFX 4 primitives composition**:
>   1. `SendGameVariant` (수동 변경) — 113+ commands 중 하나
>   2. `GAME_VARIANT_LIST` (사용 가능 variant 목록 전송)
>   3. `auto_blinds_type` enum (every_hand / new_level / with_strip)
>   4. `pl_dealer` field (round detection — dealer 좌석 회귀 검출)
>
> EBS 는 이 4 primitives 를 조합하여 운영자 설정 (Lobby Settings 부록 D.4):
> - **every_hand** (default) = 매 핸드 종료 시 SendGameVariant
> - **every_round** = pl_dealer 한 바퀴 회귀 시
> - **new_level** = blind_level 증가 시
>
> 정본 인용: archive `complete.md` line 2872 (`auto_blinds_type`) + line 1574-1645 (113+ commands).
...
> 💡 **Wire format = Field-based batch** (PokerGFX GameInfoResponse 75+ fields 패턴 차용):
>   - State enum 송신이 아닌 **75+ fields 한 번에 batch 업데이트**
>   - 9 카테고리 (블라인드 / 좌석 / 베팅 / 게임 / 보드 / 상태 / 디스플레이 / 특수 / 드로우)
>   - 클라이언트 (CC / Lobby / Overlay) 가 fields 비교하여 변경 사항 도출
>   - state 단순화 가능: Frontend 가 자체 UI 분기, Engine 은 fields 만 송출
>
> 상세 schema: `docs/2. Development/2.2 Backend/APIs/Game_State_WS_Schema.md`
> 정본 인용: archive `complete.md` line 1647-1662 (GameInfoResponse 75+ fields).
...
### B.5 / Chip Count Sync 흐름 (WSOP LIVE 브레이크 권위 시점)

브레이크 시 딜러가 각 테이블의 칩 카운트를 입력 → WSOP LIVE 가 EBS 로 push → 9 카테고리 SSOT 갱신.
브레이크 = WSOP LIVE 가 stack 의 **권위 시점** (그 외 시간은 Engine 이 자동 계산).
...
POST /api/wsop-live/chip-count-snapshot
   │ X-WSOP-Signature (HMAC-SHA256)
   │ Idempotency-Key
   │
   ▼
EBS BO (S7) → chip_count_snapshots table
   │
   ├──→ WS publish: chip_count_synced
   │       │
   │       ▼
   │   CC seat_cell stack 갱신 (S3)
   │   Overlay 플레이어 대시보드 갱신 (S3/S8)
   │   Lobby table chip total 갱신 (S2)
   │
   └──→ Engine reconcile (S8) — drift 감지 + log
```

> *상세 contract*: [APIs/WSOP_LIVE_Chip_Count_Sync.md](../2.%20Development/2.2%20Backend/APIs/WSOP_LIVE_Chip_Count_Sync.md)
> *state machine*: [2.5 Shared/Chip_Count_State.md](../2.%20Development/2.5%20Shared/Chip_Count_State.md)
> *WS event*: [APIs/WebSocket_Events.md](../2.%20Development/2.2%20Backend/APIs/WebSocket_Events.md) — `chip_count_synced`
...
> 💡 **EBS 자체 RFID = PokerGFX 보안 취약점 회피** (역공학 분석 §10.7 15 취약점):
>   - PokerGFX `InsecureCertValidator` (CRITICAL, MITM 가능) — EBS 는 자체 시스템이라 TLS 사용 X
>   - PokerGFX **WiFi 비밀번호 평문 전송** (HIGH) — EBS = USB 직결, WiFi 비사용
>   - PokerGFX **펌웨어 서명 미검증** (HIGH) — EBS = 자체 HW 펌웨어, 자체 서명 가능
>
> 본 프로젝트 인텐트 "자체 RFID 시스템 구축" 의 보안 차원 정당화.
> 정본 인용: archive `complete.md` line 1903-1905 + line 2118-2134.
...
AI 분석 결과는 1 단계 CC 오퍼레이터의 6 키 (N·F·C·B·A·M) 입력과 **동일한 형태**로 Game Engine 에 전달된다 (CC_PRD v4.0). Engine 입장에서는 입력 출처만 다르고 처리 로직 동일.
...
flowchart LR
    subgraph S1["1단계 (현재)"]
        ...
        CC["CC 오퍼레이터<br/>6 키 (N·F·C·B·A·M)"] --> Eng1
    end
...
| §2.1 | 9 핵심 카테고리 (사용자 표 v1.0.0) | Ch.2 / Scene 1 |
...
| §5.1 | Lobby (5분 게이트웨이 + WSOP LIVE 거울) | Ch.5 §A.1 |
