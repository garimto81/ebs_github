...
EBS는 "마우스로 더블 클릭해서 실행하는 프로그램 하나" 가 아닙니다. 서로 다른 역할을 맡은 **여섯 개 기능 구성요소**가 톱니바퀴처럼 맞물려 돌아가는 시스템입니다. 다만 이 여섯 개가 각각 별도 프로그램으로 설치되는 것은 아닙니다 — **설치 관점으로는 소프트웨어 3종 + 하드웨어 1종** 입니다.
...
1. **로비:** 모든 테이블의 상태를 한눈에 내려다보는 관제탑 화면입니다. 참가 선수 명단을 등록하고, 어떤 포커 규칙을 적용할지 설정하며, 방송 그래픽의 전반적인 설정을 통제하는 관리자 전용 공간입니다.
...
|-----------|-----------|-----------|--------------|
| **EBS Desktop App** | 로비 + 커맨드 센터 + 오버레이 뷰 | Flutter Desktop + Rive | §5.0 런타임 모드에 따라 단일 프로세스 (탭) 또는 다중 프로세스 (다중창) |
| **Game Engine** | 게임 엔진 | Pure Dart | 별도 서비스 (Docker 컨테이너 또는 `dart run`) |
| **Backend Server (BO)** | 백오피스 서버 | FastAPI + SQLite/PostgreSQL | 별도 서비스 |
| **RFID Hardware** | 카드 인식 하드웨어 | ST25R3911B + ESP32 | 물리 장비 (USB 연결) |

즉 **기능 6개 ↔ 설치 3 소프트웨어 + 1 하드웨어**. 로비 / 커맨드 센터 / 오버레이 뷰가 같은 Flutter 바이너리 안에 있다는 사실은 §5.0 (2 런타임 모드) 에서 자세히 풀어냅니다.

**개발팀 주의 (혼란 방지 노트)**:

- **팀 분할은 기능 단위** — team1(로비), team2(BO), team3(게임 엔진), team4(커맨드 센터 / 오버레이). 팀 수 ≠ 설치 단위 수.
- **빌드 산출물은 설치 단위** — team1 + team4 는 동일 Flutter 바이너리 `EBS Desktop App` 을 공동 소유합니다. 코드는 기능별로 폴더 분할되지만 빌드는 하나로 묶입니다.
- 상세 소유권 규칙은 `docs/2. Development/2.5 Shared/team-policy.json` 을 참조하십시오.

### Ch.5 — 사용자가 만지는 것: 관제탑과 조종석

현장의 스태프들이 시스템을 조작하기 위해 마주하는 프론트엔드 화면은 크게 세 가지로 나뉩니다. 전체 상황을 지휘하는 **로비**, 시스템 규칙을 통제하는 **세팅**, 그리고 쉴 새 없이 버튼을 눌러야 하는 **커맨드 센터**입니다.
...
* **4단계 (완전체 구축):** 목표했던 22종의 모든 게임 규칙을 통합하고, 그래픽 편집기와 데이터를 관리하는 백오피스까지 완성하여 메인 시스템 구축을 마무리합니다.
