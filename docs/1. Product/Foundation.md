...
reimplementability_notes: "2026-04-22 회의 결정 반영 — Ch.5 §5.0 2 런타임 모드 신설(D2: 탭 기본/다중창 옵션), Ch.6.3 프로세스 모델 명시(D5), Ch.6.4 실시간 동기화 정책 신설(D5: SG-002 해소), Ch.7.1 배경 config flag 명시(D4), Ch.8.5 복수 테이블 아키텍처 신설(D1: N PC + 중앙 서버 BO+DB). F2 BS_Overview 용어 정의 + F3 Spec_Gap SG-002 DONE 전환 후 최종 PASS 재판정."
---
...
* **Ch.4 개발 대상 오버뷰: 6개의 퍼즐 조각**

    * 시스템을 구성하는 5 App + 1 Hardware 요약
...
EBS는 마우스로 더블 클릭해서 실행하는 단일 프로그램이 아닙니다. 서로 다른 역할을 맡은 **5개의 소프트웨어 애플리케이션**과 **1개의 하드웨어 장치**가 정교하게 맞물려 돌아가는 거대한 톱니바퀴입니다. 전체 시스템은 크게 세 가지 그룹(조작부, 두뇌부, 출력부)으로 나눌 수 있습니다.
...
1. **로비:** 모든 테이블의 상태를 한눈에 내려다보는 관제탑 웹페이지입니다. 참가 선수 명단을 등록하고, 어떤 포커 규칙을 적용할지 설정하며, 방송 그래픽의 전반적인 설정을 통제하는 관리자 전용 공간입니다.
...
5. **오버레이 뷰:** 게임 엔진이 계산해 낸 결과값을 받아 화려한 애니메이션 그래픽으로 렌더링하는 애플리케이션입니다. 만들어진 그래픽은 방송국의 표준 송출 규격인 SDI나 NDI를 통해 최종 출력됩니다.
6. **카드 인식 하드웨어:** 테이블 천 아래에 매립되어 전파를 쏘는 12개의 안테나와 전자 기판입니다. 칩이 내장된 카드를 0.1초 만에 감지하여 컴퓨터가 이해할 수 있는 데이터로 변환해 주는 물리적 장비입니다.

우리가 개발해야 할 대상은 바로 이 **여섯 개의 독립된 퍼즐 조각**입니다. 다음 장부터는 이 조각들이 각각 어떻게 생겼고 어떤 기능을 품고 있는지 그룹별로 묶어 상세히 해부해 보겠습니다.
