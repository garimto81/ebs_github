...
이번 챕터는 4 단계로 풀어냅니다 — §1.1 정보 비대칭성의 딜레마, §1.2 시청자에게 전달할 3 핵심 데이터, §1.3 1세대 유리판에서 2세대 RFID 로의 기술 진화, §1.4 0.1초 번역가 미션.
...
| 구분 | 축구 중계 | 포커 중계 |
| --- | --- | --- |
| 핵심 정보 | 공개 (점수, 공 위치) | **비공개** (카드 뒤집힘) |
| 그래픽 역할 | 눈에 보이는 정보를 **정리** 하여 표시 | 보이지 않는 정보를 **생성** 하여 표시 |
...
> *§1.3 의 RFID 가 카드를 0.1초에 읽는다면 — §1.4 가 그 0.1초의 의미를 미션으로 선언합니다.*
...
```mermaid
flowchart LR
    A[RFID<br/>카드 인식] --> D[Game<br/>Engine]
    B[딜러<br/>액션 입력] --> D
    C[포커<br/>규칙] --> D
    D --> E["100ms 이내<br/>방송 그래픽"]
```
...
이 미션이 다음 챕터의 모든 설계를 지배합니다.

</td>
</tr>
</table>

> *§1.4 의 미션이 명확해졌다면 — Ch.2 에서 그 결과물 (시청자가 보는 화면) 을 해부합니다.*
...
![EBS 영역 = 실시간 / 현장 / 센서](images/foundation/wsop-2025-paradise-overlay.png)
...
이번 챕터는 4 단계로 풀어냅니다 — §3.1 송출 파이프라인 4 단계, §3.2 현장 프로덕션 장비 사슬, §3.3 보이지 않는 데이터 공급자 역할, §3.4 0.1초와 1시간의 시간차.
...
왜일까요? **서울에서 영상을 자르고 평가하여 수동으로 그래픽을 덧입히는 후편집 (C 구간) 과정** 에서 대부분의 지연이 발생하기 때문입니다. 즉, EBS는 극강의 실시간 시스템이지만, 방송 생태계의 편집 파이프라인 특성상 대중에게는 자연스러운 시간차를 두고 공개됩니다.
...
이번 챕터는 4 단계로 풀어냅니다 — §6.1 두뇌 Game Engine (22 게임 + 21 OutputEvent), §6.2 뼈대 Backend (BO) 3 핵심 임무 + 프로세스 모델, §6.3 통신 매트릭스 (From → To 경로), §6.4 실시간 동기화 (DB SSOT + 100ms WS push).
...
> *§6.3 의 매트릭스가 통신 경로를 정의했다면 — §6.4 가 어떻게 실시간 100ms 동기화를 보장하는지 답합니다.*
...
EBS 가 향하는 두 축. §9.2 의 5 단계 로드맵이 이 두 목적지로 수렴합니다.
