...
| 날짜 | 항목 | 내용 |
|------|------|------|
| 2026-04-15 | v41.1.0 — 용어 정리 | Action Tracker → Command Center 교체 (L529 이미지 캡션, prd-hand-sequence.html L261). Console → Settings 편입 반영 (L617 이미지 캡션, L619 본문, L897 "다이얼로그" → "페이지"). 역사 인용·PokerGFX 고유명사·CR done 은 보존. `docs/2. Development/2.1 Frontend/Console_UI.md` 는 `Settings/Legacy_Console_UI.md` 로 이동 (Settings 폴더 하위 역사 기록) |
...
| **Backend (BO)** | FastAPI + SQLModel + WebSocket | REST 66+ EP, WSOP LIVE 동기화, 3채널 WS, Auth/RBAC, GE API-07. API 카탈로그 요약 (아래) | Team 2 |
| **Command Center** | Flutter + Riverpod (Dart) | 테이블별 1 인스턴스, 핸드 생명주기, RFID HAL 통합 | Team 4 |
| **Game Engine** | Pure Dart package, Event Sourcing | 22종 게임 규칙, 실시간 Win%/Equity, Pot/Showdown 판정. OutputEvent sealed class 18종 발행 (아래 참조) | Team 3 |
| **Overlay** | Flutter + Rive (.riv) | .gfskin 씬 로딩 → 애니메이션 렌더링 → SDI/NDI 출력. OutputEventBuffer 소유 (Team 4) | Team 4 |

#### Backend API 카탈로그 (API-01 / API-06 요약)

> 상세: `docs/2. Development/2.2 Backend/APIs/Backend_HTTP.md` (API-01) · `Auth_and_Session.md` (API-06)

**Base URL**: `http://{bo_host}:{bo_port}/api/v1` (dev 8000, prod 443 HTTPS)
...
| `Idempotency-Key` | UUIDv4 또는 ULID | 조건부 | mutation 권장, 24h 유지 (CCR-003) |
| `X-Request-ID` | UUIDv4 | 선택 | 분산 추적용 correlation ID |

**설계 원칙**:

| Part | 원칙 | 설명 |
|------|------|------|
| I. REST | RESTful | 리소스 중심 URL, HTTP 메서드로 CRUD 매핑 |
| I. REST | JWT 인증 | 모든 요청에 `Authorization: Bearer {token}` (인증 API 제외) |
| I. REST | RBAC | 역할(Admin/Operator/Viewer) 기반 접근 제한 |
| II. WSOP sync | 폴링 기반 | WSOP LIVE Push(Webhook) 미가정, BO 주기적 Pull |
| II. WSOP sync | 캐싱 우선 | WSOP LIVE 데이터는 BO DB 캐싱, 클라이언트는 항상 BO DB 조회 |
| II. WSOP sync | `source` 필드 | 엔티티마다 `source`: `api` / `manual` 구분 |
| II. WSOP sync | 독립 운영 | API 연동 실패 시에도 수동 입력으로 EBS 단독 운영 |

**주요 확장 (CCR)**:
- CCR-003: Idempotency-Key 헤더 표준화 + 멱등성 응답 계약
- CCR-010: `POST /tables/rebalance` saga 구조 (`saga_id`, `steps`, 207/500)
- CCR-015: `GET /tables/{table_id}/events` replay 엔드포인트 (WebSocket seq gap 복구)

#### OutputEvent 카탈로그 (API-04 요약)

> 상세: `docs/2. Development/2.3 Game Engine/APIs/Overlay_Output_Events.md` (API-04) — 18종 전체 시그니처는 `team3-engine/ebs_game_engine/lib/core/actions/output_event.dart`

team3 Game Engine 이 발행하고 team4 Overlay 가 소비하는 **in-process Dart OutputEvent** 18 종. CC 와 Overlay 는 동일 Flutter 앱이므로 네트워크 오버헤드 없음. BO/Lobby 로의 네트워크 이벤트는 API-05 WebSocket Events 분리 담당.

| # | Event | 발행 시점 | 소비자 영향 |
|---|-------|-----------|-------------|
| OE-01 | `StateChanged` | HandFSM phase 전환 시 | Phase 배너 + 씬 전환 |
| OE-02 | `ActionProcessed` | 플레이어 액션 확정 | 액션 라벨, seat glow |
| OE-03 | `PotUpdated` | pot 변동 | pot 숫자 롤링 |
| OE-04 | `EquityUpdated` | 보드 오픈·플레이어 폴드 | Equity Bar 업데이트 |
| OE-05 | `CardRevealed` | 홀카드/보드 공개 | Rive 카드 reveal/slide |
| OE-06 | `HandCompleted` | 핸드 종료 | Winner 하이라이트 + pot sweep |
| OE-07..18 | (나머지 12종) | — | Rive 애니메이션 트리거 |

#### Security Delay Buffer (0~120초)

방송 지연 버퍼로 홀카드 선노출 차단 + Backstage/Broadcast 이중 출력 (CCR-036). OutputEventBuffer 구현 소유는 **Team 4** (`team4-cc/lib/overlay/output_event_buffer.dart`). Team 3 harness 는 OutputEvent 를 즉시 emit 하며 buffer 미보유 (Pure Dart 계산 엔진 원칙).

> 각 앱의 내부 구조(레이어, DI, 모듈 경계, 상태 관리 상세)는 팀별 `docs/2. Development/2.{1..4} {팀}/` 문서 참조. Team 2의 `docs/2. Development/2.2 Backend/` 가 현재 가장 상세한 레퍼런스이며, Team 1/3/4는 각각 해당 섹션에서 확장된다.
...
![Command Center — 운영자 입력 화면](./References/images/prd/app-command-center.png)
...
![EBS Settings — 메인 화면](./References/images/prd/app-settings-main.png)

> 미리보기 화면에서 오버레이 출력을 실시간으로 확인하며, 하단 탭(Outputs / GFX / Display / Rules / Stats / Preferences)에서 모든 설정을 제어한다. **이것이 EBS Settings이다.**
...
| **Settings** ⚙ | 오버레이·출력·게임 규칙·통계 설정 (Admin 전용 페이지) | 방송 준비 |
...
#### 네비게이션 구조 — 3계층 + Player 독립 레이어

> 상세: `docs/2. Development/2.1 Frontend/Lobby/Overview.md` (BS-02) / `Event_and_Flight.md` (BS-02-02)

Lobby 는 **WSOP LIVE Staff App** 구조를 그대로 계승한다: **Series → Event(Day) → Table** 3계층 + **Player 독립 레이어**. Player 는 Table 종속이 아닌 독립 화면으로 분리되어 어느 계층에서도 검색/관리 가능하다 (2026-04-13 전환).

```
Lobby (웹)
  ├─ 3계층 네비게이션 (Series → Event → Table)
  │   └─ Event+Flight 통합 화면 (accordion 펼침)
  ├─ Player 독립 레이어 (Table 종속 아님)
  ├─ [Settings ⚙] → Settings 페이지
  └─ [Enter CC] → Command Center (Flutter 앱, 테이블당 1개)
```

#### Lobby ↔ Command Center 관계 (1:N)

| 항목 | Lobby (웹) | Command Center (Flutter) |
|------|------------|-------------------------|
| 기술 | 웹 프론트 (Quasar) | Flutter 앱 (테이블당 1개) |
| 역할 | 설정·관리·모니터링 허브 | 실시간 방송 입력 |
| 개수 | **1개** (모든 테이블의 관제) | **테이블당 1개** (N 인스턴스) |
| 데이터 | REST API + WebSocket 구독 | DB 로드 + 핸드 데이터 발행 |

- Lobby 에서 [Launch] 로 새 CC 인스턴스 생성
- 각 CC 는 독립 동작 (테이블 1 = CC 1 = Overlay 1)
- Lobby 는 모든 활성 CC 상태를 WebSocket 으로 실시간 모니터링
- 앱 전환: CC 실행 후에도 Lobby 는 브라우저에 그대로 열려 있음 (동시 사용)

#### EventFlightStatus enum (BS-02-02)
...
보조 필드: `isRegisterable`, `dayIndex`, `is_pause` (accordion 메타)
...
### Settings ⚙ — 설정 페이지 (Admin 전용, 글로벌 단위)

> 상세: `docs/2. Development/2.1 Frontend/Settings/Overview.md` (BS-03-00)

Settings 는 Lobby 의 **별도 하위 페이지**이며, **글로벌 단위**로 모든 CC 인스턴스에 동일하게 적용된다 (2026-04-09 결정, 테이블별 Settings 없음). Console PRD v9.7 의 5탭 + Preferences 다이얼로그를 행동 명세 **6섹션**으로 매핑한다.

#### 6섹션 구조

| 섹션 | BS 문서 | 컨트롤 수 | 역할 |
|------|---------|:---------:|------|
| **Outputs** | BS-03-01 | 13 | 송출 파이프라인 (해상도, NDI/RTMP/SRT/DIRECT, Fill & Key) |
| **GFX** | BS-03-02 | 14 | 그래픽 배치/카드/애니메이션 + SE 진입 |
| **Display** | BS-03-03 | 17 | 수치 표시 형식 (통화, 정밀도, BB 모드) |
| **Rules** | BS-03-04 | 11 | 게임 규칙 + 플레이어 표시 |
| **Stats** | BS-03-05 | 15 | Equity, Outs, Leaderboard, Score Strip |
| **Preferences** | BS-03-06 | 9 | 테이블 인증, 진단, 내보내기 |
...
```
Admin Settings 변경 → BO REST API PUT /configs/{category}/{key}
  → BO DB configs UPDATE → WebSocket ConfigChanged 이벤트 → 모든 CC 수신
```

적용 시점: IDLE 상태 = 즉시 / 핸드 진행 중 = 다음 핸드 시작 시 (핸드 무결성 보호).

#### LOCK/CONFIRM/FREE 연동

| 분류 | Settings 영향 | 예시 |
|:----:|-------------|------|
| **LOCK** | 변경 불가 (비활성) | Game Type, Max Players |
...
> 상세: `docs/2. Development/2.5 Shared/Authentication.md` (BS-01) + `docs/2. Development/2.2 Backend/APIs/Auth_and_Session.md` (API-06)

EBS 는 **방향별 인증 2-스택**을 운영한다 (2026-04-15 G5 결정):

| 방향 | 프로토콜 | 자격증명 | 토큰 TTL |
|------|---------|---------|---------|
| **내부 UI → BO** (Lobby, CC) | JWT Bearer | email + password + TOTP(옵션) | Access 1~12h / Refresh 48h (환경별 차등) |
| **BO → WSOP LIVE (outbound)** | OAuth 2.0 client_credentials | Basic base64(client_id:secret) | access_token `expires_in` (~3600s), 만료 30초 전 선제 재발급 |

#### Access Token TTL (환경별 차등 — CCR-006)

| 환경 | Access TTL | Refresh TTL |
|------|:---------:|:----------:|
| dev | 1h | 24h |
| staging · prod | 2h | 48h |
| prod-live | 12h | 48h |
...
CC 는 **독립 로그인 없음**. Lobby 에서 로그인 후 테이블 선택 → Lobby-Only Launch 패턴으로 CC 실행. CC 는 Lobby 에서 전달받은 토큰으로 BO 에 인증.

#### 2FA + 자동 잠금 (CCR-048, WSOP LIVE GGPass 정렬)

- 2FA Level: Off / Low / Medium / High (4단계)
- 자동 잠금: 10회 연속 실패
- Refresh Token TTL: 48h 공통

#### Rate Limiting (CCR-052)

카테고리별 요청 한계 + 응답 헤더(`X-RateLimit-*`) + IP whitelist. OWASP + WSOP LIVE 준거.
...
> 기술 설계 상세는 `docs/2. Development/2.2 Backend/` 참조.
