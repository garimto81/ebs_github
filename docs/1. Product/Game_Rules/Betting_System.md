...
confluence-page-id: 3834085681
confluence-parent-id: 3812360338
confluence-url: https://ggnetwork.atlassian.net/wiki/spaces/~71202036ff7e0a7684471195434d342e3315ed/pages/3834085681/Betting
last-updated: 2026-05-12
reimplementability: PASS
reimplementability_checked: 2026-05-12
reimplementability_notes: "PRD-GAME-04 완결 본문 + Confluence 발행 대상. v03 multi-hand 4 섹션 보강 (§4-8 ante_override, §5-1c straddle 회전, §5-3a 홀수 칩, §7-5 engine spec) — derivative-of: docs/2. Development/2.3 Game Engine/Rules/Multi_Hand_v03.md"
related-cycle: 8
related-issue: 344
---
...
| 2026-05-12 | v03 multi-hand 보강 | §4-8 ante_override 신설, §5-1c straddle 핸드 간 회전, §5-3a 홀수 칩 처리, §7-5 straddle_seat 엔진 명세 (cycle 8 #344, S8 PR #319 handoff) |
...
### 4-8. Ante 금액 변경 — Ante Override

핸드 사이에 Ante 금액 또는 유형을 즉시 변경할 수 있습니다. 토너먼트 블라인드 레벨 상승, 캐시 게임 합의, 운영자 직권 정정 등에 사용합니다.

**동작 규칙:**

| 입력 | 다음 핸드 동작 |
|------|---------------|
| Ante 변경 이벤트(amount, type?) 발생 | 다음 핸드부터 즉시 새 금액/유형 적용 |
| 이벤트 없음 | 기존 Ante 그대로 (정상 cycle) |
| amount ≤ 0 | 이벤트 무시 (안전장치) |

**영구성:** 변경은 다음 Ante 이벤트가 들어올 때까지 **계속 유지**됩니다. (자동 reset 없음 — Tournament Mode는 별도 룰)

**예시 시나리오:**
- 토너먼트 Blind Level 2 → 3 상승: Ante 100 → 150
- 캐시 게임 합의: BB Ante (1명 대납) → Standard Ante (전원 부담) 전환
- 잘못 설정한 Ante 즉시 정정 (한 핸드만 정상 후 다시 정상 금액으로 두 번째 override)

> **엔진 명세 (정본 SSOT)**: `AnteOverride { amount, type? }` 이벤트. 입출력 계약은 [Multi_Hand_v03.md §2](../../2.%20Development/2.3%20Game%20Engine/Rules/Multi_Hand_v03.md) 참조.
...
### 5-1c. 핸드 간 Straddle 이동 — Multi-hand Rotation

여러 핸드를 연속으로 플레이할 때 Straddle 자리는 딜러 버튼처럼 함께 회전합니다.

**규칙:**

| 활성 자리 수 | Straddle 처리 | Straddle 활성 여부 |
|:-----------:|--------------|:------------------:|
| 3명 이상 | 다음 자리(빈 좌석 건너뜀)로 **1칸 시계 방향 이동** | 유지 |
| 2명 (헤즈업) | **자동 무효화** (`null`) | `false` (자동 disable) |
| 1명 이하 | rotation 자체 무효 | 변경 없음 |

> **헤즈업 시 무효화 이유**: 헤즈업에서는 딜러=SB가 되며, Straddle을 더해도 행동 순서 의미가 모호해집니다. **PokerGFX/WSOP 표준 동작**입니다.

**예시 — 6인 테이블 4핸드 회전 (Straddle 자리 = 4번째 자리부터):**

| 핸드 | Dealer | SB | BB | Straddle |
|:----:|:------:|:--:|:--:|:--------:|
| 1 | seat 0 | seat 1 | seat 2 | seat 3 |
| 2 | seat 1 | seat 2 | seat 3 | seat 4 |
| 3 | seat 2 | seat 3 | seat 4 | seat 5 |
| 4 | seat 3 | seat 4 | seat 5 | seat 0 (한 바퀴!) |

**헤즈업 진입 시나리오:**

```
3인 테이블 (seat 0/1/2 활성), 핸드 1: dealer=0, SB=1, BB=2, Straddle=null
  ↓ seat 0 sitting out → 활성 2명만 남음
핸드 2: dealer=1 (=SB), BB=2, Straddle=null (headsup 진입, straddle 자동 무효)
```

> **엔진 명세 (정본 SSOT)**: §7-5 straddle_seat 명세 + [Multi_Hand_v03.md §1](../../2.%20Development/2.3%20Game%20Engine/Rules/Multi_Hand_v03.md) 참조.
...
### 5-3a. 팟 분할 정밀도 — 홀수 칩 + Side Pot RIT

Run It Twice는 팟을 50/50으로 나누지만, **칩 수가 홀수**면 1칩 불일치가 발생합니다.

**WSOP Rule 73 — 홀수 칩 흡수:**

| 총 팟 | Board 1 (먼저 dealt) | Board 2 |
|:-----:|:-------------------:|:-------:|
| 1000 (짝수) | 500 | 500 |
| **1001 (홀수)** | **501** | 500 |
| 1003 | 502 | 501 |

> **Board 1에 +1칩 흡수**가 토너먼트/캐시 공통 표준입니다.

**보드 구성 — flop 공유 + turn/river 새로 깔기:**

```mermaid
flowchart LR
    A["River 직후<br/>All-in 합의"] --> B["Board 1<br/>기존 flop+turn+river"]
    A --> C["Board 2<br/>flop 공유<br/>+ 새 turn/river"]
    B --> D["Board 1 승자<br/>1/2 + 홀수칩"]
    C --> E["Board 2 승자<br/>1/2"]
```

**Side Pot이 여러 개일 때:**
- Main Pot, Side Pot 1, Side Pot 2 **각각 독립 50/50 분할**
- 각 팟마다 홀수 칩 처리 별도 적용 (각 Board 1이 흡수)
- 자격자(Eligible Set)는 §1b-3 일반 공식 그대로

**예시 — 3인 All-in + Side Pot 1개:**

| 팟 | 총액 | Board 1 | Board 2 | 자격자 |
|----|:----:|:-------:|:-------:|:------:|
| Main Pot | 901 | 451 | 450 | A, B, C |
| Side Pot 1 | 600 | 300 | 300 | B, C |
| **합계** | 1501 | 751 | 750 | — |

각 보드의 승자는 자격자 중에서 별도 평가합니다. A는 Side Pot 1을 받을 수 없습니다 (§1b-3 일반 공식).

> **엔진 명세 (정본 SSOT)**: 분할 알고리즘은 [Multi_Hand_v03.md §3.2](../../2.%20Development/2.3%20Game%20Engine/Rules/Multi_Hand_v03.md) 참조.
...
### 7-5. straddle_seat 엔진 명세 — Multi-hand Rotation 통합

> S8 (Game Engine) cycle 6 v03 구현 (#310 / PR #319) handoff. 본 외부 PRD ↔ engine SSOT 갈등 시 **engine SSOT (Multi_Hand_v03.md) 우선**.

#### 7-5-1. straddle_seat 정의

| 항목 | 값 |
|------|-----|
| 타입 | `int?` (active seat index, `null` 허용) |
| 활성 조건 | `straddleEnabled == true && straddleSeat != null` |
| 포함 범위 | UTG Straddle / Mississippi Straddle / Button Straddle 전체 (§5-1, §5-1b 참조) |

#### 7-5-2. 핸드 간 회전 규칙 (`ManualNextHand` 처리 시점)

| 활성 seat 수 | straddleSeat 처리 | straddleEnabled |
|:------------:|------------------|:---------------:|
| ≥ 3 | next non-sittingOut seat 으로 1칸 회전 | 유지 |
| 2 (headsup) | `null` 로 무효화 | `false` (자동 disable) |
| ≤ 1 | rotation 자체 무효 | 변경 없음 |

#### 7-5-3. 금액 규칙

| 항목 | 값 |
|------|-----|
| `straddleAmount` | `2 × bbAmount` (기본값, engine 구현과 일치) |
| Double/Triple Straddle | **Cycle 8 범위 외**. 추후 cycle 에서 다중 `straddleSeats` list 로 확장 |

#### 7-5-4. 액션 순서

| 단계 | 행동 순서 |
|------|----------|
| Pre-Flop | MP → CO → BTN → SB → BB → **Straddle (마지막, 옵션 보유)** |
| Flop 이후 | SB → BB → Straddle → MP → CO → BTN (시계 방향, Straddle 위치 일반화) |

#### 7-5-5. 정합 검증 (next-hand response)

`POST /api/session/{sessionId}/next-hand` 응답에 v03 추가 필드:

| 필드 | 타입 | 의미 |
|------|------|------|
| `straddleEnabled` | bool | 회전 후 활성 여부 (headsup 진입 시 `false`) |
| `straddleSeat` | int? | 회전 결과 seat index (headsup 시 `null`) |
| `anteAmount` | int | override 적용 후 값 |
| `anteType` | int | override 적용 후 type |
| `runItTimes` | int? | RIT 활성 시 2, 기본 `null` |

> **정본 SSOT (engine)**: [Multi_Hand_v03.md §1, §4](../../2.%20Development/2.3%20Game%20Engine/Rules/Multi_Hand_v03.md). 위 표는 외부 통합용 요약이며, 엣지 케이스 / 입출력 계약 / 코드 위치는 모두 engine SSOT 가 정본.
