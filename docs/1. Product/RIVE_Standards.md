...
purpose: |
  ...
  v0.8.0 (2026-05-13, Cycle 16 Wave 2) — **9 시청자 카테고리 (사용자 표 v1.0.0) 를 화면 SSOT 로 채택**. 기존 Part IV 11 chapter 본문 보존 + Clock (Ch.20) + Operator Marks (Ch.22) 는 Auxiliary (시청자 비노출) 로 재분류.
  v0.9.0 (2026-05-17, Ultrathink Architecture cascade) — Foundation v5.0 Ch.4 Scene 3 (7-Layer L7 Skin) cascade. 신규 sub-sections: (a) Ch.5.5 PokerGFX INTEGRATED vs EBS MODULAR Skin 비교, (b) Ch.22.5 ConfigurationPreset 99+ 필드 매핑 reference, (c) Ch.23 Ticker 시스템 PRD 명세 (신규 chapter), (d) Ch.26.5 transition_type 5종 매핑. PokerGFX 정본 (archive `complete.md`) 직접 인용.
---
...
- [Ch.2 — 그래픽 요소 9 카테고리 한눈에 (v0.8.0 — 사용자 표 SSOT) + Rendering / JSON DB 분리](#ch2)
...
1. **무엇이 보이는가?** — 9 시청자 카테고리 (사용자 표 v1.0.0) + 2 Auxiliary (Clock / Operator) — Part IV 본문 11 chapter 로 풀이
2. **어떻게 만들어지는가?** — Rive 런타임 + Variable Binding (Part II)
3. **어디에서 데이터가 오는가?** — 다섯 작가별 매핑 (Part III)

<a id="ch2"></a>

## Ch.2 — 그래픽 요소 9 카테고리 한눈에 (v0.8.0 — 사용자 표 SSOT) + Rendering / JSON DB 분리

본 Part 의 끝에서 — Part IV 본문 들어가기 전에 — **시청자 화면 9 카테고리** 를 한 페이지로 펼친다. 시청자가 화면에서 보게 되는 모든 그래픽은 다음 9 안에 들어간다.

**v0.8.0 SSOT pivot (2026-05-13, Cycle 16 Wave 2)** — 시청자 화면 = **9 카테고리** (사용자 표 v1.0.0 그대로). 기존 v0.7.0 의 "Player Info Identity / Player Info Stack" 2 카테고리는 → 새 #1 **플레이어 대시보드** 로 통합. 기존 v0.7.0 의 "Cards (Hole + Community)" 1 카테고리는 → 새 #2 **핸드 그래픽 (홀카드)** + 새 #5 **보드/커뮤니티 영역** 으로 분리.

기존 v0.7.0 의 #9 시계 (Clock) + #11 운영자 전용 표식 → 사용자 표에 없음. **Auxiliary (시청자 비노출, 내부/운영자 전용)** 으로 재분류. Part IV 본문 chapter 는 보존 (Ch.20 = JSON Output Only, Ch.22 = Operator-only viewport).

### 9 카테고리 — 두 책임 영역 분리

| 영역 | 책임 | 9 카테고리 분포 |
|------|------|---------------|
| **A. EBS Rive Rendering Area** (highly modular) | EBS 가 실시간 Rive Overlay 로 직접 렌더링 | 플레이어 대시보드 (#1) + 핸드 그래픽 (#2) + 액션 인디케이터 (#3) + 실시간 승률 (#4) + 보드/커뮤니티 (#5) + 블라인드 정보 창 (#7) + 중앙 팟 정보 (#8) |
| **B. JSON DB Output Only** (NO EBS Rendering) | EBS 가 JSON DB 로만 출력. 외부 그래픽 시스템 / 방송 자막 시스템이 자체 렌더링 | 이벤트 브랜딩 (#6) + 필드 현황판 (#9) |
| **C. Auxiliary** (Viewer-Invisible) | 시청자 화면 비노출. 운영자/내부 진단 | Clock (Ch.20 → JSON Output Only) + Operator Marks (Ch.22 → Operator-only viewport) |

> **분리 원칙**: A 영역은 카메라 위 Rive Overlay 의 실시간 합성 — Lego-Block (Ch.25) 으로 load/unload. B 영역은 EBS 가 데이터를 JSON 으로 송출하기만 함 — 그래픽 형태는 EBS 책임 밖. C 영역은 시청자 화면에 노출되지 않는 운영/진단 채널.
...
![Overlay 9 카테고리 해부](images/foundation/overlay-anatomy.png)

> *FIG · 9 카테고리 위치와 정체 (해부 도식 — Auxiliary 2 종 제외)*

</td>
</tr>
</table>

### 9 카테고리 표 (사용자 표 v1.0.0 verbatim + 범용 역할 + 데이터 소스)

| # | 카테고리 (사용자 표 verbatim) | 범용 역할 | 영역 | 데이터 소스 매핑 | Part IV chapter |
|---|------------------------------|----------|:----:|------------------|:---------------:|
| 1 | **플레이어 대시보드** (Name + 국적 + 포지션 SB/BB/D + 칩스택) | 좌석별 선수 식별 + 자산 표시 | **A** | DB (`name`, `country`) + Engine (`position`, `stack`) + RFID (`seat_id`) | Ch.12 + Ch.13 |
| 2 | **핸드 그래픽** (RFID 홀카드) | 좌석별 손패 (private card) 시각화 | **A** | RFID (카드 정체) + Engine (`is_revealed`, `is_folded`) | Ch.14 (Hole 부분) |
| 3 | **액션 인디케이터** (체크/벳/레이즈/폴드) | 현재 액션 4 종 시각 표식 | **A** | Command Center (`CHECK`/`BET`/`RAISE`/`FOLD`) → Engine 정합 | Ch.16 |
| 4 | **실시간 승률 통계** (Equity) | 핸드 강도 + Equity 백분율 | **A** | Engine (몬테카를로 계산 결과) | Ch.15 |
| 5 | **보드/커뮤니티 영역** (Flop/Turn/River 공용 카드) | 공용 카드 (community card) 시각화 | **A** | RFID (카드 정체) + Engine (공개 시점: Flop=3장, Turn=4장, River=5장) | Ch.14 (Community 부분) |
| 6 | **이벤트 브랜딩** (WSOP/APT 로고) | 토너 / 스폰서 로고 / 이벤트 브랜드 | **B** | DB (Brand Pack: sponsor logos, tournament names) — **JSON DB Output Only** | Ch.21 |
| 7 | **블라인드 정보 창** (블라인드 레벨 + 앤티) | 현재 레벨의 Small Blind / Big Blind / Ante | **A** | DB (Blind Structure 스케줄) + Engine (현재 레벨 카운터) | Ch.18 |
| 8 | **중앙 팟 정보** (총 베팅 칩) | Main Pot + Side Pot 누적 | **A** | Engine (메인 + 사이드 팟 계산) | Ch.17 |
| 9 | **필드 현황판** (참가자/ITM/FT) | 토너 전체 진행 상태 (남은 인원 / ITM 진입 / FT) | **B** | DB (총 entries) + Engine (남은 인원, ITM 컷, FT 도달) — **JSON DB Output Only** | Ch.19 |

> **A 영역 모든 카테고리는 Rive Asset (.riv 파일) 이 그래픽 형태를 제공**. B 영역은 EBS 가 JSON 만 송출하므로 그래픽 형태는 외부 책임.

### 액션 인디케이터 4 종 명세 (#3, 사용자 표 verbatim)

| 액션 | 트리거 | Rive Trigger Variable |
|------|--------|----------------------|
| **체크** (Check) | Command Center `CHECK` 버튼 | `play_check_animation` |
| **벳** (Bet) | Command Center `BET <amount>` | `play_bet_animation` + Engine `bet_amount` |
| **레이즈** (Raise) | Command Center `RAISE <amount>` | `play_raise_animation` + Engine `bet_amount` |
| **폴드** (Fold) | Command Center `FOLD` | `play_fold_animation` + `is_folded=true` |

> **사용자 표 4 종 정합 (v1.0.0)** — Check/Bet/Raise/Fold. **Call 액션은 사용자 표에서 명시 제외**. Game Engine 의 Call 액션은 내부 계산상 존재하나 시각 표식은 미반영 (또는 "Bet 매칭" 패턴으로 통합 — Cascade Note: 2.3 Game Engine 명세 확인 필요).
> New Hand / Deal 은 visual X (시청자 화면 미반영, 운영자 흐름 제어 전용).

### Auxiliary (시청자 비노출) 보존 사유

| 보조 카테고리 | Part IV chapter | 보존 사유 |
|--------------|:---------------:|----------|
| **Clock** (Level Clock + Hand Clock) | Ch.20 (JSON Output Only) | 외부 그래픽 시스템 / 방송 자막 시스템이 자체 렌더링. 사용자 표 9 카테고리 = 화면 직접 노출만 — Clock 은 별도 자막 채널 |
| **Operator Marks** (운영자 전용 표식) | Ch.22 (Operator-only viewport) | 운영자 도구 화면 (CC / Console) 에서만 노출. 송출 화면 비노출 — 사용자 표 정합 |

각 카테고리는 Part IV 의 별도 챕터에서 — 그래픽 모양, 5 데이터 소스에서 받는 정확한 변수, 변화의 순간 — 으로 풀어 다룬다. **B 영역 (Ch.19/20/21) 은 JSON 스키마 정의 중심으로 풀이. Auxiliary (Ch.20/Ch.22) 는 viewer-invisible 노트 명시.**
...
## Ch.5.5 — PokerGFX INTEGRATED vs EBS MODULAR Skin 비교 (v0.9.0)

Foundation §A.3 (Rive Manager) + Ch.4 Scene 3 (7-Layer L7) cascade. PokerGFX 의 INTEGRATED `.skn` 패러다임과 EBS 의 MODULAR `.riv` 패러다임의 차이.

| 측면 | PokerGFX (INTEGRATED `.skn`) | EBS (MODULAR `.riv`) |
|------|------------------------------|----------------------|
| 파일 형식 | 단일 binary `.skn` (AES 암호화) | 개별 `.riv` 파일 다수 |
| 암호화 | AES (SKIN_PWD + SKIN_SALT, ConfuserEx 보호) | 미적용 (Rive 자체 보안) |
| 서버 인증 | `skin_auth_result` enum (no_network/permit/deny) | Rive Manager Validate 단계 |
| 다중 톤 지원 | 다중 `.skn` 파일 swap (운영 시 교체) | 다중 `.riv` 자연 공존 |
| 무결성 | `skin_crc` CRC32 검증 | git SHA + Rive Manager preview |
| 99+ 필드 위치 | binary 내부 ConfigurationPreset | Rive 파일 + EBS DB Brand Pack 분리 |
| 외부 인계 | binary 패키지 | 개별 `.riv` 파일 + Rive Editor 작업물 |

### Lobby ↔ CC 디자인 톤 = MODULAR 의 자연 결과

PokerGFX 패러다임에서 "다중 톤" = 다중 `.skn` 파일 swap. EBS = 다중 `.riv` 동시 공존 가능 → **Lobby B&W refined minimal + CC Broadcast Dark Amber 가 별도 skin 으로 공존**.

| 위치 | Skin |
|------|------|
| Lobby (5분 게이트웨이 + WSOP LIVE 거울) | `lobby-bw-minimal.riv` 외 |
| Command Center (라이브 운영) | `cc-broadcast-amber.riv` 외 |
| Overlay | 시청자 표시용 별도 `.riv` |

> 💡 **이는 모순이 아니라 의도된 컨텍스트 분리**. 운영자가 "준비 단계 (Lobby B&W) → 라이브 (CC amber)" 전환 시 색 변화 = mental state shift 신호.
>   Command_Center.md Ch.11.3 cross-ref.

### PokerGFX 정본 인용

- `complete.md` line 2138-2349 — ConfigurationPreset 99+ fields + `.skn` 구조
- `complete.md` line 2097-2114 — `skin_auth_result` enum + 인증 흐름
- `complete.md` line 2308-2349 — skin_crc 무결성 + 로딩 흐름
...
## Ch.22.5 — PokerGFX ConfigurationPreset 99+ 필드 → Rive 매핑 (v0.9.0)

Foundation §A.3 cascade + Ch.5.5 INTEGRATED vs MODULAR. PokerGFX 의 ConfigurationPreset 99+ 필드 (정본 `complete.md` line 2193-2287) 가 EBS Rive 어떤 변수로 매핑되는지 reference.

### 22.5.1 매핑 카테고리 (7 그룹)

| 카테고리 | PokerGFX 필드 수 | EBS Rive 매핑 위치 |
|---------|:--:|------------------|
| 레이아웃 | 10 | Rive Artboard layout + Ch.27 z-layer |
| 표시 설정 | 11 | Rive State Machine (show/hide trigger) |
| 전환 효과 | 4 | Ch.26.5 transition_type 5종 매핑 참조 |
| 통계 설정 | 11 | Ch.23 Ticker 시스템 + Brand Pack 통계 |
| 칩 표시 정밀도 | 8 | Rive text variable (cp_pl_stack 등) |
| 통화/금액 | 4 | EBS DB Brand Pack (currency_symbol 등) |
| 로고 에셋 | 3 | Brand Pack panel_logo / board_logo / strip_logo |

### 22.5.2 레이아웃 필드 매핑 (10)

| PokerGFX 필드 | 타입 | EBS Rive 변수 |
|---------------|------|---------------|
| `board_pos` | enum (left/centre/right) | `board_layout_pos` State |
| `gfx_vertical` | bool | `is_vertical_mode` boolean |
| `gfx_bottom_up` | bool | `bottom_anchor_align` boolean |
| `gfx_fit` | bool | (auto by Rive Artboard fit) |
| `heads_up_layout_mode` | enum 3 | `heads_up_mode` State |
| `heads_up_layout_direction` | enum 2 | `heads_up_direction` boolean |
| `heads_up_custom_ypos` | int | `heads_up_y_offset` number |
| `x_margin` | int | (Brand Pack 또는 Rive Artboard) |
| `y_margin_top` | int | (Brand Pack 또는 Rive Artboard) |
| `y_margin_bot` | int | (Brand Pack 또는 Rive Artboard) |

### 22.5.3 표시 설정 매핑 (11)

| PokerGFX 필드 | 타입 | EBS Rive 변수 |
|---------------|------|---------------|
| `at_show` | enum 4 (immediate/action_on/after_bet/action_on_next) | `action_show_policy` State |
| `fold_hide` | enum 2 | `fold_hide_policy` boolean |
| `fold_hide_period` | int (ms) | `fold_hide_delay_ms` number |
| `card_reveal` | enum 6 (immediate/after_action/end_of_hand/never/showdown_cash/showdown_tourney) | `card_reveal_policy` State |
| `show_rank` | bool | `show_rank_label` boolean |
| `show_seat_num` | bool | `show_seat_number` boolean |
| `show_eliminated` | bool | `show_eliminated_marker` boolean |
| `show_action_on_text` | bool | `show_action_on_text` boolean |
| `rabbit_hunt` | bool | `rabbit_hunt_enabled` boolean |
| `dead_cards` | bool | `show_dead_cards` boolean |
| `indent_action` | bool | `indent_action_text` boolean |

### 22.5.4 통계 + 칩 표시 + 로고 매핑

상세는 `complete.md` line 2237-2286 정밀 인용. Brand Pack (Ch.6.3) + Ticker 시스템 (Ch.23) 으로 분산.

### 22.5.5 활용 가이드

- **외부 디자이너 작업 시**: PokerGFX 99+ 필드 → 본 매핑표 → Rive 변수 명명 일관성 확보
- **운영자 설정 시**: Lobby Settings 부록 D (6 탭 79 필드) ↔ ConfigurationPreset 99+ 필드 cross-ref
- **PokerGFX 정본 인용**: archive `complete.md` line 2193-2287

---

<a id="ch23"></a>

## Ch.23 — Ticker 시스템 PRD 명세 (v0.9.0 신규)

PokerGFX 정본 (line 2573, 2245-2252) 에서 발견된 **Ticker 시스템** 의 EBS PRD 정의. 시청자 화면 9 카테고리 (Ch.2) 의 보조 시스템.

### 23.1 Ticker 정의

**Ticker** = 시청자 화면 하단 또는 측면에 표시되는 **횡스크롤 정보 띠**. 통계 / 사이드 정보 / 스폰서 메시지 등을 순환 표시.
...
```
   ...
   │                                                    │
   ├────────────────────────────────────────────────────┤
   │ ◀ VPIP 32%  P3  ·  Avg Stack 125k  ·  ITM 18      │ ← Ticker
   └────────────────────────────────────────────────────┘
```

### 23.2 Ticker 종류

PokerGFX 정본 `ticker_*` 필드 분석:

| Ticker | PokerGFX 필드 | EBS 데이터 소스 | 활성 조건 |
|--------|--------------|---------------|----------|
| **VPIP Ticker** | `ticker_stat_vpip` | EBS DB (Player 통계) | Lobby Settings 토글 |
| **PFR Ticker** | `ticker_stat_pfr` | EBS DB | 토글 |
| **AGR Ticker** | `ticker_stat_agr` | EBS DB | 토글 |
| **WTSD Ticker** | `ticker_stat_wtsd` | EBS DB | 토글 |
| **CumWin Ticker** | `ticker_stat_cumwin` | EBS DB | 토글 |
| **Brand Ticker** | (Brand Pack) | EBS DB Brand Pack | 항상 (스폰서 표시) |

### 23.3 Ticker 와 Auxiliary 분리

- **Ticker = 시청자 노출** (PGM 채널)
- **Operator Marks (Ch.22) = 운영자 전용** (OPS 채널)
- 두 시스템 명확히 분리
...
| 데이터 | 소스 | 갱신 빈도 |
|--------|------|----------|
| 플레이어 통계 (VPIP/PFR/AGR/Wtsd/CumWin) | EBS DB (Lobby.md Ch.7 PlayerInfoResponse 5종) | 매 핸드 종료 |
| Brand 메시지 | Brand Pack (`panel_logo`, sponsor names) | 정적 (시즌 단위) |
| ITM / FT 카운트 | Engine (참가자 수 - 탈락자 수) | 매 탈락 |

### 23.5 Rive Variable 매핑

```
ticker_text[i]        ← 표시할 다음 정보 (DB query 결과)
ticker_visible        ← 활성/비활성 (Lobby Settings)
ticker_scroll_speed   ← Brand Pack 설정
ticker_position       ← 화면 하단/상단/측면 (ConfigurationPreset)
```

### 23.6 PokerGFX 정본 인용

- `complete.md` line 2245-2252 — auto_stat_* 11 필드 (vpip/pfr/agr/wtsd/cumwin + ticker variant)
- `complete.md` line 2573 — `ticker_edit`, `ticker_stats_edit` WinForms (PokerGFX 편집 UI)
...
### 26.5 transition_type 5종 매핑 (PokerGFX 정본 cascade, v0.9.0)

PokerGFX 정본 `complete.md` line 2812 의 `skin_transition_type` enum 5 값을 EBS Rive 의 Entry/Exit 패턴과 매핑.

| transition_type | PokerGFX enum 값 | EBS Rive 구현 | 사용 예시 |
|----------------|:--:|---------------|----------|
| `global` | 0 | (시스템 전역 설정 — 모든 transition 에 적용) | Rive Project default |
| `fade` | 1 | Opacity Timeline 0→1 (Entry) / 1→0 (Exit) | Player Card 등장/탈락 |
| `slide` | 2 | Translation Timeline (X 또는 Y 이동) | 액션 표식 slide-in |
| `pop` | 3 | Scale Timeline 0→1.2→1 (탄성) | Pot 카운트업 강조 |
| `expand` | 4 | Scale Timeline 0→1 (선형) + Opacity | 카드 등장 |

> 💡 PokerGFX 의 `trans_in` / `trans_out` (ConfigurationPreset 99+ 필드 중 4 transition 필드) 가 위 enum 값 중 하나로 설정. EBS Rive 는 매 그래픽 요소마다 transition_type 변수 선언 권장.

#### 26.5.1 활용 가이드 (외부 디자이너)

- Rive Editor 에서 각 요소의 Entry/Exit Timeline 작성 시 본 5종 패턴 중 선택
- ConfigurationPreset 의 `trans_in_time` / `trans_out_time` 필드 (ms) 가 Timeline 길이 결정
- 통일된 transition 명명으로 외부 인계 시 디자이너-개발팀 인지 일치

#### 26.5.2 PokerGFX 정본 인용

- `complete.md` line 2231-2235 — ConfigurationPreset `trans_in`, `trans_out` 4 필드
- `complete.md` line 2812 — `skin_transition_type` enum 5종
