...
- [Ch.20 — 시계](#ch20) — Hand Clock + Level Clock
- [Ch.21 — 브랜딩](#ch21) — 로고·워터마크·광고
- [Ch.22 — 운영자 전용 표식](#ch22) — 운영자만 보는 영역

**Part V — 살아 움직이는 법**

- [Ch.24 — State Machine](#ch24) — Rive 의 상태 전이
- [Ch.25 — 게임 흐름과 그래픽 전이](#ch25) — 5 단계 + 21 OutputEvent → Rive 트리거 매핑
...
| **Command Center** | **Command Center** | "운영자 결정 작가" — 6 키 → Action 객체 → Rive Variable / Engine | Flutter Desktop + Rive |
| **RFID** | **RFID Hardware** (12 안테나) | 카드 정체 작가 | ST25R3911B + ESP32 (USB) |
...
| **Command Center** | ✅ (Rive 사용) | "Rive 출력자" — 6 키 입력 + 자체 Overlay 렌더 인스턴스 (1 테이블 = 1 인스턴스) |
...
![실제 방송 송출 화면 reference](../../1.%20Product/images/foundation/wsop-2025-paradise-overlay.png)
...
![Overlay 11 카테고리 해부](../../1.%20Product/images/foundation/overlay-anatomy.png)
...
| 9 | 시계 | Engine (Hand Clock + Level Clock 카운트) |
| 10 | 브랜딩 | DB (브랜드 패키지) |
...
CC 는 **6 물리 키 (N · F · C · B · A · M)** 만 사용하며, 각 키의 의미는 **5-Act 시퀀스** 의 현재 단계에 따라 동적으로 변경된다 (Foundation §3, CC_PRD v4.0).

#### 6 물리 키 (Foundation §3 정합)
...
| **B** | Bet (사이즈 모달) | Bet Act — Bet / Raise / All-in 사이즈 입력 진입 |
| **A** | All-in | Bet Act |
| **M** | More | Deal / Showdown / HandEnd Act (각 Act 진행) |

#### 5-Act 시퀀스

```
   Hand Start (N)  ->  Deal (M)  ->  Bet (F/C/B/A)  ->  Showdown (M)  ->  Hand End (M)
```

- **Hand Start Act**: N 키 = NewHand (Engine 한 핸드 컨텍스트 초기화)
- **Deal Act**: M 키 = 홀카드 딜 / 보드 카드 딜 진행
- **Bet Act**: F (Fold) / C (Check or Call) / B (Bet 사이즈 모달, Bet · Raise · All-in 분기) / A (All-in)
- **Showdown Act**: M 키 = 카드 공개 / hand_strength 산출
- **Hand End Act**: M 키 = 정산 + 다음 핸드 준비

> **시각 표식 (Ch.16) ↔ 물리 키 분리**: 시청자는 화면에서 FOLD/CHECK/BET/CALL/RAISE/ALL-IN 등 명시적 라벨을 본다. 그러나 운영자가 누르는 물리 키는 6 개뿐 — 컨텍스트가 키 의미를 결정한다.

### 8.2 입력이 변수로 가는 길

운영자의 한 키 입력이 — 화면의 그래픽 한 조각으로 변환되는 길. 아래 예시는 Bet Act 컨텍스트의 RAISE 입력.

```
   운영자 (Bet Act): [B] 키 -> 사이즈 모달 [30000] -> [Enter]
                |
                v
   +---------------------+
   |  Command Center     |
...
> *FIG · 운영자 (Bet Act) [B]→사이즈 모달 30000→Enter → Action Object (RAISE seat:3 amount:30000) → Engine → Rive Variables → 화면 highlight*
...
테이블 아래 깔린 **12 안테나** (10 좌석 + 보드 중앙) 가 카드의 NFC 태그를 읽어 가져오는 단 한 가지 데이터 (Foundation §13 정합):
...
```
   ...
   |                                              |
   +----------------------------------------------+
              ASCII MOCK · 12 안테나 배치 (10 좌석 + 보드 중앙 영역 2)
```

![실제 테이블 RFID 안테나 배치](images/rive/Gemini_Generated_Image_4tazf54tazf54taz.webp)

> *FIG · 실제 테이블 RFID 안테나 12 개 배치 (ST25R3911B + ESP32, USB 통신)*
...
### 16.1 8 시각 표식 ↔ 6 물리 키 매핑

시청자는 화면에서 다음 8 가지 시각 표식을 본다. 그러나 운영자가 누르는 물리 키는 **6 개 (N·F·C·B·A·M)** 뿐 — Engine 이 컨텍스트로 시각 라벨을 결정한다 (Foundation §3, Ch.8.1 참조).

| 시각 표식 | 표시 | 색상 (1 차 가설) | 운영자 키 (5-Act 컨텍스트) |
|----------|------|:----------------:|---------------------------|
| **NEW HAND** | "딜링 중..." 모달 | 흰색 | **N** (Hand Start Act) |
| **DEAL** | 홀카드 등장 (Flip) | — | **M** (Deal Act) |
| **FOLD** | 카드 회색 + 좌석 dim | 회색 | **F** (Bet Act) |
| **CHECK** | "CHECK" 텍스트 | 청색 | **C** (Bet Act, 베팅 無) |
| **BET** | 칩 → 베팅라인 + "BET" 라벨 | 황색 | **B** (Bet Act, 사이즈 모달) |
| **CALL** | 칩 → 베팅라인 매칭 | 청색 | **C** (Bet Act, 베팅 有) |
| **RAISE** | 칩 → 베팅라인 + "RAISE" 라벨 | 적색 | **B** (Bet Act, 사이즈 모달, 이미 베팅 有) |
| **ALL-IN** | 전체 스택 → "ALL-IN" 박스 | 흑색 + 진동 | **A** (Bet Act) |

> 색상은 1 차 가설. 외부 디자이너 / Brand Pack 별 override.
> 6 키 ↔ 시각 표식 매핑은 Foundation §3 (CC 6 키) + 5-Act 시퀀스 정합.
...
| `rfid_status[1..12]` | RFID (안테나 진단, Foundation §13) |
...
| 11 | `AllInDeclared` | `is_all_in[seat]`, `on_action[seat]` (ALL-IN) | ALL-IN 박스 (Ch.16.2) |
...
```
  ...
  [ ] 4. RFID
       [ ] 12 안테나 신호 확인 (10 좌석 + 보드 중앙)
       [ ] Mock 카드로 카드 인식 검증
  [ ] 5. Command Center
       [ ] 6 키 (N · F · C · B · A · M) 응답 확인 (Foundation §3)
       [ ] 5-Act 시퀀스 (Hand Start → Deal → Bet → Showdown → Hand End) 흐름 검증
  [ ] 6. Game Engine
       [ ] 게임 종목 룰 검증 (Hold'em / Omaha / Mixed 등)
       [ ] 팟 / 사이드 팟 계산 검증
       [ ] Equity 계산 검증
  [ ] 7. 합류 확인
       [ ] 11 카테고리 모두 5 데이터 소스의 변수 채워짐
       [ ] State Machine 전이 검증
       [ ] Brand Pack 적용 검증
  [ ] 8. 채널 분리
