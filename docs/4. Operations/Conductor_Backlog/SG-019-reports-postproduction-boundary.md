...
status: DONE
resolved: 2026-05-03
resolved-by: conductor (Mode A 자율 — Foundation §1.2 scope 권한, V9.4 정합)
owner: conductor
created: 2026-04-21
promoted: 2026-04-26
affects_chapter:
...
reimplementability_notes: "Conductor 자율 결정 완료 (2026-05-03). 대안 1 채택: Reports/Insights = 실시간 운영 지표 한정 (Foundation §1.2 EBS Core 정의 정합). 포스트프로덕션 (Adobe 영역) 은 EBS scope 외. Reports/Overview.md + Foundation §1.2 cross-ref 명시. 외부 개발팀 인계 가능 SSOT 확정"
---
...
> ✅ **DONE** — V9.4 AI-Centric Mode A 자율 진행. 사용자 도메인 질문 0회 (Foundation §1.2 scope 권한 = conductor 자체).

**채택**: 대안 1 — Reports/Insights = 실시간 운영 지표 한정

**이유**:
- Foundation §1.2 EBS Core 정의 (실시간 라이브) 정합
- MEMORY `project_architecture_v33.md` "EBS = 실시간 라이브" 원칙 일치
- 포스트프로덕션 (Adobe Premiere/AE 영역) 은 EBS scope 외 — 명확한 경계

**Reports 시간축 경계** (publisher cascade 권고):

| 영역 | EBS scope? | Reports 다룸? |
|------|:---------:|:------------:|
| 라이브 운영 (테이블/딜러 통계) | ✅ EBS Core | ✅ |
| 토너먼트 진행 지표 (현재 chip lead, 평균 stack) | ✅ EBS Core | ✅ |
| 일별/주별 운영 KPI 집계 | ✅ EBS Core | ✅ |
| 방송 후 종합 분석 (시청률, SNS 반응) | ❌ 포스트프로덕션 | ❌ |
| 영상 편집/하이라이트 제작 | ❌ Adobe 영역 | ❌ |

**영향 (publisher cascade 권고)**:
- `Foundation.md §1.2`: "Reports = 실시간 운영 지표 한정, 포스트프로덕션 = EBS scope 외" 명시
- `Reports/Overview.md`: 첫 단락에 시간축 경계 + 포스트프로덕션 cross-ref
- `BS_Overview §1`: "Reports (EBS) ≠ 포스트프로덕션 분석" 용어 등재
