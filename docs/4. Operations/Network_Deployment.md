...
| Lobby Web | 3000 | HTTP (nginx) | Docker 컨테이너 (`ebs-lobby-web`). 사용자 브라우저 접속 — `/api`, `/ws` 를 BO 로 reverse proxy |
| Command Center | — | Desktop app (Flutter) | Windows native 앱 (RFID 시리얼 하드웨어 접근 필수). BO + Engine 에 연결 |
...
```bash
docker compose --profile web up -d   # BO + Engine + Redis + Lobby Web + CC Web
# Lobby 접속: 브라우저 http://localhost:3000/
# CC 개발용 Desktop: cd team4-cc/src && flutter run -d windows --dart-define=DEMO_MODE=true
```
...
### Team1 Lobby (Flutter Web via Docker nginx)

**정규 배포**: Docker 컨테이너 `ebs-lobby-web` (port 3000). nginx 가 `/api`, `/ws` 를 `bo:8000` 으로 reverse proxy. 브라우저는 동일 origin 사용 — CORS 불필요.
...
**개발자 디버깅 전용** (배포 아님): `flutter run -d chrome --dart-define=EBS_BO_HOST=<lan-ip>`

상세: `docs/2. Development/2.1 Frontend/Deployment.md`
