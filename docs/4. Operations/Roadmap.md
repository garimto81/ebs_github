...
### 4. 최우선 FAIL 5건 (기획 공백/모순, SG-001 해소 후 현재)

| # | 항목 | 유형 | 상태 |
|:--:|------|:----:|:----:|
| 1 | Foundation Ch.7 / team4 Overlay — ENGINE_URL 표준 + graceful 대기 (동일 공백) | B | SG-002 추적 |
| 2 | team3 API-04 — 구현 21종 vs 문서 18종 | C | Overlay_Output_Events.md §6.0 정정 완료 (2026-04-15). 문서 일관성만 유지 |
| 3 | team3 순수 Dart 규칙 vs harness `dart:io` | C | BS_Overview §1 + team3 CLAUDE.md 예외 명시 (2026-04-20) — **부분 해소** |
| 4 | Shared EBS_Core.md — 참조되지만 파일 부재 | B | SG-005 로 추적 (Foundation Ch.6 으로 병합 결정) |
| 5 | Settings 6탭 / `.gfskin` / RFID codemap — Type B 공백 (Agent B 재분류) | B | SG-003 / SG-004 / SG-006 로 추적 |

~~Foundation Ch.5, team1 Engineering, BS_Overview §1 Tech Stack (SG-001 resolved)~~
...
| SG-002 | Engine 의존 계약 (ENGINE_URL + graceful) | PENDING | conductor |
| SG-003 | Settings 6탭 스키마 | PENDING | conductor + team1 |
| SG-004 | .gfskin ZIP DATA-07 포맷 | PENDING | conductor + team1 |
| SG-005 | Foundation Ch.6 시스템 연결 도식 (EBS_Core 병합) | PENDING | conductor |
