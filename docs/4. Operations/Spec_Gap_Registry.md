...
| SG-022 | scope_clarification | Foundation §5.0 / BS_Overview §1 | **SUPERSEDED 2026-04-27 저녁** | ~~2026-04-27 오전 — A 채택: 단일 Desktop 바이너리 (Lobby 포함)~~. **같은 날 저녁 multi-service Docker SSOT cascade 로 SUPERSEDE** (Lobby = Web lobby-web :3000 / CC = Web cc-web :3001 또는 Desktop / Overlay = Desktop). 자세히: `Conductor_Backlog/SG-022-deprecation.md`, memory `project_multi_service_docker_2026_04_27`. 2026-05-04 SG-030 cascade 로 재확인 |
...
| SG-030 | spec_contradiction_typeC | 2.1 Frontend/Lobby/Overview.md L73 + Foundation §5.0/§5.1 + BS_Overview §1.5 | **DONE** | 2026-05-04 — design SSOT (`Lobby/References/EBS_Lobby_Design/` HTML/React 230KB) 가 Web 가정으로 작성, multi-service Docker SSOT 와 정합. 사용자 의사결정 (Lobby_Modification_Plan_2026-05-04 §F1 — SUPERSEDE 권한 인정) 으로 옵션 A 채택: Overview.md L73 + Foundation §5.0/§5.1 + BS_Overview §1.5 표 정정 + 4 SSOT 정합화. 후속 cascade: B-LOBBY-{TOPBAR,SERIES,ALERTS}-001 + B-LOBBY-TOPBAR-WS-CONTRACT (team2 협의). `Conductor_Backlog/SG-030-lobby-overview-flutter-vs-web.md` |
