...
owner: conductor
tier: internal
last-updated: 2026-05-13 (Cycle 16 — SG-040/SG-041 9 카테고리 재편 audit, Cycle 15 SG-038/039 등재 위 추가)
...
| ID | type | category | status | note |
|----|------|----------|:------:|------|
| SG-040 | spec_gap + spec_inconsistency | overlay-categories | PENDING | User 9 카테고리 vs Foundation Scene 1 (8 그래픽) 부정합. Foundation Scene 1 = Branding/Blind/Field 명시 제외 → 9 카테고리 재정의 필요. RIVE Ch.2 #3 카드 (홀+커뮤 단일) / #1+#2 분리 — User 9 와 분류 base 충돌. Cycle 16 (2026-05-13). 해소 = Cycle 17 S10-W Foundation §Ch.2 MAJOR rewrite + RIVE_Standards §Ch.2 MAJOR restructure. (rebase 재할당: 작업 당시 SG-038 등재 시도 → 충돌 회피로 SG-040) |
| SG-041 | spec_drift | overlay-widgets | PENDING | overlay/layer1 widget 3종 누락 — Branding STUB (logoPath 파라미터만), Blind STUB (overlay widget 부재, backend 완전), Field MISSING (DB total_entries만, ITM/FT API+widget 전무). carry-over B-356 (OE-12~21 번호 매핑). Cycle 16 (2026-05-13). 해소 = SG-040 spec 정합 후 team4 implementation PR (S2 stream). (rebase 재할당: 작업 당시 SG-039 → SG-041) |
