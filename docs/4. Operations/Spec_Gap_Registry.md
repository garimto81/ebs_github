...
| SG-026 | quality_gates | docs/4. Operations/Roadmap.md §"Production Quality Gates" + 후속 Backlog | **DONE** (gate 명시), **PENDING** (95% coverage 도달 + OWASP audit + 100ms SLA 측정 = B-Q10/Q11/Q12) | 2026-04-27 — B-Q7 ㉠ 채택 (Production-strict). 95%+ coverage, 99.9% uptime, p99<200ms, OWASP Top 10, WCAG 2.1 AA, 한+영. 잔여: B-Q10 (95% coverage 도달 plan, **17%p gap multi-turn**), B-Q11 (OWASP audit), B-Q12 (100ms SLA 측정 framework). **현재 team2 baseline = 261 tests / 78% coverage** (Session 2 Phase 1 audit 정정 — 이전 "90%" 는 stale 기록). 95% 도달 = 약 683 stmts 추가 커버 = 70-140 unit tests = Session 2.1~2.7 multi-turn |
