...
## 4. 현재 Drift (2026-05-12 Cycle 5 P10 path-aware scope 분리)

### 4.1 계약별 요약

| 계약 | D1 | D2 | D3 | D4 | Total | 핵심 조치 |
|------|:--:|:--:|:--:|:--:|:-----:|-----------|
| REST API | 0 | 43 | 2 | 132 | 175 | 2026-05-11 **Cycle 2 closure correction**: spec 정합 ✅ (S7 PR #232 `Backend_HTTP §5.17.5 + §5.17.11` 신설) but detector **D3 2 잔여** (매칭 실패 — SG-010 본질적 한계). 보고-reality 정합 갱신 (이전 Cycle 2 표 D3 0/0 잘못된 보고). D4 130→132 |
...
| Settings | 0 | 87 | 5 | 51 | 143 | 2026-05-12 **Cycle 5 P10 path-aware scope 분리 적용**: D2 93→**87 (-6 false positive 제거)** ✅. `_CC_SETTINGS_PATHS = {CC/Settings.md}` 신규 — CC Settings.md (BS-03, BO Config 스코프) 가 team1-frontend UI 영역과 다른 도메인이므로 detector 비교 대상에서 제외. 제거된 6건: `rfid_mode`(*_mode 1건), `cap_bb_multiplier`, `allow_run_it_twice`, `ante_override`, `shot_clock_seconds`, `straddle_enabled_seats` (모두 BO Config 전용 key). 잔여 *_mode 4건 (`bets_mode`/`chipcount_mode`/`pot_mode`/`vertical_mode`) = team1 `Settings/UI.md` 진성 spec→code drift (Display/Resolution 탭 dropdown 미구현 — scanner correctness 보존). KPI 88 대비 -1 초과 달성. Cycle 4 P3 교훈 (false positive 인정 + scanner 한계 명시) 적용 — *_mode suffix 광범위 억제 회피. |
...
| SG-010 | tooling | meta | **IN_PROGRESS** | spec_drift_check.py 정밀화 (Settings, Schema, WebSocket). **F2 WebSocket detector 정밀화 완료 (2026-04-20)**, **P6 Settings detector 정규화 완료 (2026-04-20)** — camelCase/snake_case/dotted namespace/frontmatter 지원. D4 +39. **P9 Settings prefix-aware filtering 완료 (2026-05-12 Cycle 4, issue #269)**: `gfx.*`/`overlay.*` graphics scope 제외 → settings D2 109→93 (-16 false positive). **P10 path-aware scope 완료 (2026-05-12 Cycle 5, issue #283)**: `_CC_SETTINGS_PATHS = {CC/Settings.md}` — BO Config 스코프 (configs 테이블 backend key) 가 team1 UI form 필드와 다른 도메인이므로 spec source 에서 제외. D2 93→87 (-6, KPI 88 초과 달성). 제거: `rfid_mode` + 5 BO Config keys (`cap_bb_multiplier`/`allow_run_it_twice`/`ante_override`/`shot_clock_seconds`/`straddle_enabled_seats`). 잔여 *_mode 4건 = team1 UI.md 진성 D2 (scanner correctness 보존, Cycle 4 P3 교훈). 잔여 후속: **P11** (api/websocket detector spec walker §X.Y.Z 깊은 트리 매칭 — Cycle 2 closure 발견) |
