...
| 2026-05-10 | v1.4 | **CC 기획 vs UI 정합성 감사 — 기획 §12 표만 stale, 실제 코드는 v4.0 100% 정합 확인.** 사용자 지적 "기획과 ui 구조가 달라". 코드 정독 결과 `at_01_main_screen.dart` + `widgets/`에 V1~V13 모두 통합됨. KeyboardHintBar phase-aware 매핑(C/B/N) 정상. §3.0 4 영역 위계, 1×10 그리드, 6 키, KeyboardHintBar, MiniTableDiagram, CcStatusBar, FLOP/TURN/RIVER street labels 모두 ✅. **정정 사항**: Overview §12.1 V1~V13 모두 ✅ 표기 + §12.3 Phase A~G 모두 ✅. 코드 변경 0줄 (구현은 이미 완료, 문서 sync만). |
| 2026-05-10 | v1.5 | **QA 자동화 아키텍처 구축 + 풀 e2e 자동 실행 100% 성공.** 사용자 지적: "사용자가 좌석 추가+START HAND+액션 실행해야 확인 가능하면 QA 실패. 아키텍처 설계하여 진행". 패치 3건: (1) `at_00_login_screen.dart:initState` — `CC_AUTO_LOGIN_*` dart-define 자동 connect hook, (2) `at_01_main_screen.dart:initState` — `CC_AUTO_DEMO=quickHand` dart-define 자동 demo 활성+seed+play hook, (3) `production.json` — QA 자격증명 + 시나리오 임베드. 결과: navigate 1회로 자동 connect → /main 진입 → Demo Mode 활성 → 3 players seed → quickHand 시나리오 12 step 자동 재생 → **HandFSM IDLE → SETUP → PRE_FLOP → FLOP → TURN → RIVER → SHOWDOWN → COMPLETE 풀 사이클 완료**. 스크린샷 21에 Community board 5장 (A♥K♠7♦Q♣2♠), Pot 250, Hand #1 COMPLETE, V1~V13 모든 visual uplift 활성. B-211 (E2E 풀 핸드) IMPLEMENTED, B-217 (Flutter Web e2e) PARTIAL_DONE (Demo Mode 자동화로 우회, Flutter Driver는 별도). |
| 2026-05-10 | v1.6 | **CC SeatCell 인라인 편집 검증 + Demo path actionOn/seat status 누락 발견.** 사용자 지적 #1: "기획 문서/HTML에 각 요소 클릭 → 수정 설계되어 있음" → 코드 검증 결과 **이미 완전 구현** (`seat_cell.dart` line 660-696 `onTap` 연결 + line 154/197/237/347/392/436/476 `showDialog`. 7-row 모두 button — POS/CTRY/NAME/STACK/BET/LAST. CARDS만 D7 가드로 onTap 없음). Playwright snapshot으로 `e21~e26` 7개 button 노출 확인 + NAME 클릭 → "Player name: Alice" textbox 다이얼로그 정상 작동 검증 (스크린샷 22). 사용자 지적 #2: "CC 로직 설계도 불완전 (empty 위치 액션 호출)" → 코드 분석 결과 **`ws_provider.dart` `dispatchLocalDemoEvent`의 HandStarted/ActionPerformed에서 `setActionOn` + seat status sync 누락** 확인. 광범위 영향(prod WS path 공유)이라 별도 사이클로 진행 — 신규 **B-220** 등록 (HIGH/team4+team2). 권장 path: stub_engine → EngineOutputDispatcher 통일 (§1.1.1 정합). |
| 2026-05-10 | v1.7 | **B-220 IMPLEMENTED — CC actionOn EMPTY 좌석 가드 + folded 시각 구분.** 패치 3건: (1) `seat_provider.dart:setActionOn` — guard로 EMPTY/folded/busted 좌석 차단 + nextActiveAfter fallback, (2) `seat_provider.dart` 신규 helper 2개 (`markFolded`, `nextActiveAfter`), (3) `ws_provider.dart` `_dispatchIncomingEvent` 보강 — HandStarted에서 first_action_seat 또는 dealer 기준 derive, ActionPerformed에서 fold→activity 갱신 + next_action_seat 또는 nextActiveAfter로 자동 propagation, HandEnded에서 actionOn null. 결과 (스크린샷 23): **S4 (Bob) folded → 회색 dimmed + LAST 행 "FOLD" 표시**, Players 2/3, EMPTY 좌석 glow 0건. v1.5의 EMPTY 좌석 ACTING glow 버그 해소. prod WS path도 동일 dispatcher 사용이므로 publisher payload에 `next_action_seat` 보내면 Engine SSOT와 일치. |
