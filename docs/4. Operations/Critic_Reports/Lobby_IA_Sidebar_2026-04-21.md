...
■ Settings                    (사이드바 병치 + 헤더 ⚙ 유지)
  ...
  ├─ Rules
  └─ Stats
■ Insights                    (= user "reports" 재명명 — 실시간 한정)
  ├─ Live Dashboards
  └─ Realtime Reports
...
섹션 수 9개. 런타임에 `nav_sections.feature_flag` 로 가시성 제어 → role 별 노출 축소 가능.
...
| Insights | ReportInstance | template_id, params_json, run_at, result_ref | `report_templates` (§5.2) |
...
| SG-016 | ia_missing | 2.1 Frontend/Lobby | Reports/Insights 섹션 미정의 (UI.md 에 사이드바 항목 없음) |
...
4. **"Reports = 실시간만"** 판정 — 오버엔지니어링 위험. "실시간 지표 + 당일 종료 후 요약" 까지 포함으로 완화 가능. 결정은 team2 Backend + Conductor.
