...
> 이 문서는 PokerGFX Server v3.2.985.0의 역공학 분석 결과를 바탕으로, 동일한 기능을 제공하는 자체 시스템을 처음부터 구축하기 위한 통합 개발 기획서입니다.
...
포커 방송은 실시간으로 최대 10명의 카드, 액션, 베팅 금액, 승률을 화면에 표시해야 합니다. 현재 이 역할을 수행하는 유일한 상용 시스템이 PokerGFX Server인데, 다음 세 가지 문제가 반복됩니다.

| 문제 | 영향 |
|------|------|
| 라이선스 비용 | 매년 갱신 비용 발생, 예산 통제 불가 |
| 기능 제약 | 한국어 UI, 커스텀 통계 등 요청 시 외부 업체 일정에 종속 |
| 외부 의존성 | 버그 수정, 업데이트 모두 업체 판단에 따라 지연 |

이 프로젝트의 목표는 **PokerGFX와 동일한 방송 경험을 제공하는 자체 시스템**을 구축하여, 전체 기술 스택을 내재화하고 자유롭게 확장할 수 있는 기반을 만드는 것입니다.
...
| 제외 항목 | 이유 |
|----------|------|
| KEYLOK USB 동글 DRM | 자체 JWT 라이선스로 대체 |
| ConfuserEx 난독화 | 역공학 방어 불필요 (자체 소유) |
| Dotfuscator 변조 감지 | 동일 이유 |
| Newtonsoft.Json | System.Text.Json으로 교체 |
| WinForms UI | WPF/Avalonia로 전면 재설계 |
...
역공학 분석으로 확인된 PokerGFX Server v3.2.985.0의 실제 규모:
...
원본의 God Class 구조를 현대적 아키텍처로 재설계합니다.
...
KEYLOK USB 동글은 제거합니다. 자체 시스템이므로 하드웨어 DRM이 불필요합니다.
