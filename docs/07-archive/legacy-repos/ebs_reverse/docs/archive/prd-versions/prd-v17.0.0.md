...
PokerGFX는 이 문제를 해결하는 라이브 포커 방송 전용 그래픽 시스템이다. 테이블에 내장된 RFID 리더가 카드를 전자적으로 인식하고, 게임 엔진이 다수의 포커 변형 규칙과 승률을 실시간으로 연산하며, GPU 렌더러가 방송 화면 위에 그래픽을 합성한다. 카드가 테이블에 놓이는 순간부터 방송 화면에 표시되기까지 200ms 이내.
...
시스템은 서버 엔진 1대(GfxServer)와 클라이언트 앱 2개(ActionTracker, HandEvaluation)로 구성된다. 각 구성요소의 역할은 Section 4에서 상세히 다룬다.
...
> Part I에서 파악한 핵심 개념이 실제 시스템에서 어떤 구조로 구현되는지, 아키텍처, 생태계, 서비스 인터페이스, 서버 구성을 살펴본다.
...
![3계층 아키텍처](images/prd/prd-3layer-architecture.png)
...
![시스템 모듈 구조](images/prd/prd-6module-overview.png)
...
카드 한 장이 테이블에 놓이는 순간부터 시청자 화면에 표시되기까지 200ms 이내로 완료된다. RFID 감지(~50ms) → 카드 매핑(~30ms) → 게임 엔진 상태 갱신 + 핸드 평가/승률(~50ms, 병렬) → GPU 렌더링(~70ms) → 방송 출력. 단계별 상세 타임라인은 Section 9에서 다룬다.
...
**External 모드** (간접 관여): 서버의 Live Data Export 기능(Section 4 참조)을 통해 게임 데이터를 외부 시스템에 전달하고, 프로덕션 팀이 이를 가공하여 자막/그래픽을 별도 제작한다. 이 워크플로우는 핸드가 종료되는 시점마다 본방송(Live) 내에서 실시간으로 처리된다. External 모드는 Live Data Export 라이선스(Professional+)가 필요하다.
...
> **기존 프로토콜**: 벤치마크 PokerGFX는 TCP 바이너리 프로토콜(AES-256 CBC 암호화, Length-Prefixed 직렬화)로 통신한다. 아래 5개 서비스 영역은 99개 명령어를 기능별로 분류한 논리적 그룹이다.
>
> **프로토콜 상세**: 포트 구성, 암호화, 프로토콜 스택 등은 기술 설계 문서를 참조한다. → `docs/02-design/features/pokergfx.design.md`
...
> Part II에서 다룬 시스템 구조 내에서 "RFID가 카드를 감지한다"는 전제가 반복적으로 등장했다. 이제 그 카드 인식 기술이 어떻게 진화했고, 하드웨어가 어떻게 배치되며, 인식 흐름이 어떻게 동작하는지를 상세히 살펴본다.
...
> 카드가 인식된 후, 게임 엔진이 22개 포커 변형의 규칙, 베팅, 핸드 평가, 통계를 처리하는 과정을 다룬다.
...
> 핸드 히스토리를 포함한 데이터 출력 경로(LiveApi, live_export, HAND_HISTORY)와 라이선스 게이팅 상세는 Section 4를 참조한다.
...
| **합계** | **99개** | 11개 카테고리 (내부 전용 명령 ~31개 별도) |
...
| ID | 기능 |
|----|------|
| OUT-001 | Dual Canvas 출력 (Venue + Broadcast) |
| OUT-002 | NDI 출력 |
| OUT-003 | HDMI 출력 |
| OUT-004 | SDI 출력 |
| OUT-005 | 출력 해상도 설정 |
...
| ID | 기능 |
|----|------|
| SYS-001~010 | 서버 포트, Discovery, 라이선스, RFID(12대), 상태모니터, 카드테스트, 네트워크, 암호화, 출력, 스킨 경로 |
...
| 보안 딜레이 | `images/web/trustless-security.jpg` | habwin.com |
...
> **Version**: 17.0.0 | **Updated**: 2026-02-18
