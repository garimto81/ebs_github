...
동시에 현장의 게임 공정성을 보호한다. Dual Canvas 아키텍처가 현장용 화면(카드 숨김)과 방송용 화면(카드 공개)을 물리적으로 분리하여, 시청자에게는 모든 것을 보여주면서도 현장에는 아무것도 유출하지 않는다.

시스템은 서버 엔진 1대와 7개 클라이언트 앱으로 구성된다. 게임 진행을 제어하는 Action Tracker, 시청자에게 보이는 Viewer Overlay, 테마를 편집하는 Skin Editor, 그리고 해설자/PD/보조용 앱까지 — 방송에 참여하는 모든 역할에 전용 인터페이스를 제공한다.
...
| 동시 접속 클라이언트 | 7+ |
| 지원 게임 변형 | 22개 |
| 기능 커버리지 | P0 85 · P1 48 · P2 18 = 151개 |
...
**Stream Deck 매핑**: 위 단축키 중 빈도가 높은 6개를 물리 버튼에 할당한다. 운영자는 화면을 보지 않고도 손끝 감각으로 GFX 숨김, 카메라 전환, 핸드 시작을 실행할 수 있다.
...
151개 기능을 하나의 표로 관리하는 이유: 원본 PokerGFX는 8개 탭 × 수십 개 기능이 산재하여, "무엇이 구현되었고 무엇이 남았는지"를 파악하기 어렵다. 이 추적표는 **사용 단계별**로 재분류하여, MVP 범위 선정(P0), 후속 릴리스 계획(P1/P2), 개발 진척 측정의 단일 기준선 역할을 한다. Feature ID는 부록 C의 원본과 동일하다.
...
| 6 | GFX 운영자 | Dual Canvas 확인 | Live + Delayed 캔버스 모두 동작 |
| 7 | GFX 운영자 | Trustless 모드 | Live Canvas에 홀카드 숨김 확인 |
...
| **Action Tracker** | GFX 운영자용 터치스크린 게임 진행 앱 |
...
> **Version**: 7.0.0 | **Updated**: 2026-02-16
