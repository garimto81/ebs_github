...
- **Small Blind (SB)**: 딜러 버튼 바로 왼쪽. 보통 Big Blind의 절반
...
- 3rd Street에서 **가장 낮은 공개 카드** 보유자가 소액 의무 베팅
- 이후 시계 방향 진행
...
[7][2] 오프슈트 = Hold'em **최약 시작 핸드**입니다. 이 핸드로 팟을 따면 → 다른 **모든 플레이어로부터 사이드벳을 수취**합니다.

**규칙:**
- 참여자 전원 사전 합의 (보통 1x~5x BB)
...
> 이 섹션은 소프트웨어 구현자를 위한 기술 참조입니다.

### 7-1. 베팅 라운드 State Machine
...
### 7-2. 액션 Validation Rule Table

| 액션 | 선행 조건 (Precondition) | 거부 조건 (Reject) |
|------|------------------------|-------------------|
| **Fold** | 항상 가능 | — |
