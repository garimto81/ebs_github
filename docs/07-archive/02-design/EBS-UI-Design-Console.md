...
> | 원본 장 | 설계 문서 | 내용 |
> |---------|----------|------|
> | 3장 | `docs/02-design/ebs-command-center.design.md` | Command Center 설계 (기존 ActionTracker → CC로 이름 변경, 순수 커맨드 입력 전용) |
...
> <img src="../01-pokergfx-analysis/03_Reference_ngd/web_screenshots/ggpoker-gto-broadcast.gif" alt="GGPoker + GTO Wizard — 포커 방송 오버레이 (Glassmorphism + 네온 강조)" style="max-width: 960px;">
>
> *GGPoker 방송 오버레이: Glassmorphism 카드 UI, 네온/글로우 이벤트 강조, Bold 타이포 핵심 수치 강조*

> <img src="../01-pokergfx-analysis/03_Reference_ngd/web_screenshots/ggpoker-gto-thumbnail.png" alt="GGPoker GTO Wizard — 실시간 분석 오버레이 썸네일" style="max-width: 960px;">
>
> *GTO Wizard 실시간 분석 오버레이: 확률/Equity 표시, 상태별 동적 UI 전환*

> <img src="../01-pokergfx-analysis/03_Reference_ngd/web_screenshots/wsop-paradise-2025-rfid-broadcast.png" alt="WSOP Paradise 2025 — RFID 기반 방송 오버레이 (올인 상황)" style="max-width: 960px;">
>
...
## 2장. EBS App — 3화면 구조 (Flutter + Rive)

PokerGFX의 776x660px WinForms 6탭 구조 + 별도 ActionTracker EXE를 **단일 Flutter 앱의 3화면 구조**(Lobby + Console + Command Center)로 재설계한다.

### 앱 화면 구조

| 화면 | 역할 | 진입 경로 | 기술 |
|------|------|----------|------|
| **Lobby** | 이벤트 내 모든 테이블(Feature+General) 목록, Setup(게임 설정/플레이어 등록), 테이블 선택. **2단계 개발**: 1단계=직접 수정, 2단계=WSOPLIVE 데이터 연동 (대회→이벤트→테이블 3-depth) | 로그인 후 진입점 | Flutter |
| **Console** | 오버레이 그래픽/출력 설정 (5탭: Outputs/GFX/Display/Rules/Stats) | Lobby에서 [Console ⚙] | Flutter |
| **Command Center** (CC) | 선택한 테이블의 게임 커맨드 입력 전용 (New Hand → Deal → 베팅 → Showdown) | Lobby에서 테이블 [Enter] | Flutter |

> **기존 ActionTracker 기능의 재분배**: PokerGFX ActionTracker의 기능을 Lobby(Setup: 게임 설정, 플레이어 등록, 좌석 배치)와 Command Center(커맨드: 핸드 사이클, 베팅 액션)로 분리. Command Center는 순수 커맨드 입력만 담당한다.
...
>
> <img src="../01-pokergfx-analysis/02_Annotated_ngd/ebs-main-window.png" alt="EBS Console 메인 윈도우 Annotation — Menu Bar, Preview, Info Bar 요소 매핑" style="max-width: 960px;">
>
...
3. GFX 탭에서 레이아웃(Board Position, Player Layout), 스킨 설정
4. Register Deck으로 새 덱 등록 → 카드 RFID 매핑 확인
5. [◄ Lobby]로 Lobby 복귀 → 테이블별 Setup(게임 유형, 블라인드, 플레이어 등록)
6. 테이블 [Enter]로 Command Center 진입 → 방송 시작

**긴급 상황**:
- 오버레이 오류 → Hide GFX (즉시 숨김) → 문제 해결 → Hide GFX 토글 복원
- 카드 인식 오류 → Reset Hand → 현재 핸드 전체 초기화
- 다른 테이블 전환 필요 → [◄ Lobby]로 Lobby 복귀 → 다른 테이블 선택
...
| File | Exit |
| View | System Log |
| Tools | Save GFX Configuration, Load GFX Configuration |
...
```
[Table: Final Table] ── [RFID ●][CPU ▐▐▐][GPU ▐▐] ── [Lock][Skin][Reset][Deck][Lobby][Hide]
```
...
| 우측 — Quick Actions | Lock Toggle, Load Skin, Reset Hand, Register Deck, Lobby, Hide GFX — 항상 접근 가능한 핵심 버튼 **6개** |
...
운영: Lobby 화면으로 이동한다. Console에서 Lobby로 돌아가 테이블 목록을 확인하거나, 다른 테이블의 Command Center로 전환할 때 사용한다. 현재 Command Center가 활성 상태이면 먼저 Lobby를 경유하여 테이블을 변경한다.
...
**Tab Content (가변)**: 전폭 활용. 스크롤 정책은 §2.4 서두 참조. Console은 **오버레이 표시 방식**만 제어하며, 게임 데이터(블라인드 값, 플레이어, 스택)는 Lobby의 Setup에서 설정하고, 게임 커맨드는 Command Center에서 입력한다.
...
| 핸드 번호 | 하단 상태 바 | 제거 (Command Center에서 관리) |
| 딜레이 카운터 | 하단 상태 바 | 제거 (송출 딜레이 장비에서 관리) |
| 단축키 가이드 | 하단 상태 바 | 제거 (키보드 단축키로 대체) |

### 2.7 Outputs 탭 기능 상세

> <img src="../01-pokergfx-analysis/02_Annotated_ngd/ebs-tabbar-outputs.png" alt="Outputs Tab Bar" style="max-width: 960px;">
...
> <img src="../01-pokergfx-analysis/02_Annotated_ngd/ebs-outputs-tab.png" alt="Outputs 탭 요소 매핑" style="max-width: 960px;">
>
...
> <img src="../01-pokergfx-analysis/02_Annotated_ngd/ebs-tabbar-gfx.png" alt="GFX Tab Bar" style="max-width: 960px;">
...
> <img src="../01-pokergfx-analysis/02_Annotated_ngd/ebs-gfx-tab.png" alt="GFX 탭 요소 매핑" style="max-width: 960px;">
>
> *번호 = annotation 박스. 각 요소의 상세는 아래 테이블 참조.*
...
| Never | 3 | 수동 공개만 허용 | Command Center에서 수동 조작 |
...
> <img src="../01-pokergfx-analysis/02_Annotated_ngd/ebs-tabbar-display.png" alt="Display Tab Bar" style="max-width: 960px;">
...
> <img src="../01-pokergfx-analysis/02_Annotated_ngd/ebs-display-tab.png" alt="Display 탭 요소 매핑" style="max-width: 960px;">
>
> *번호 = annotation 박스. 각 요소의 상세는 아래 테이블 참조.*
...
> <img src="../01-pokergfx-analysis/02_Annotated_ngd/ebs-tabbar-rules.png" alt="Rules Tab Bar" style="max-width: 960px;">
...
> <img src="../01-pokergfx-analysis/02_Annotated_ngd/ebs-rules-tab.png" alt="Rules 탭 요소 매핑" style="max-width: 960px;">
>
> *번호 = annotation 박스. 각 요소의 상세는 아래 테이블 참조.*
...
게임 진행 규칙 중 오버레이와 Command Center에 영향을 미치는 설정을 관리한다. 방송 시작 전 세팅하고 핸드 진행 중 변경하지 않는 것이 원칙이다.
...
> <img src="../01-pokergfx-analysis/02_Annotated_ngd/ebs-tabbar-stats.png" width="720" alt="Stats 탭바">
...
> <img src="../01-pokergfx-analysis/02_Annotated_ngd/ebs-stats-tab.png" width="720" alt="Stats 탭 요소 매핑">
>
> *번호 = annotation 박스. 각 요소의 상세는 아래 테이블 참조.*
...
| ID | 요소 | 기능 | 기본값 |
|:--:|------|------|--------|
| 1 | Table Name | 테이블 식별 이름 (TextField + Update 버튼). Lobby에서 이 이름으로 테이블을 표시한다 | "Table 1" |
| 2 | Table Password | 테이블 접속 비밀번호 (TextField, 마스킹 + Update 버튼). RBAC 로그인과 별도로 테이블 수준 보호. 빈 값이면 비밀번호 없이 접속 허용 | 빈 문자열 (비밀번호 없음) |
| 3 | PASS / Reset | 테이블 정보 초기화 버튼 2개. PASS = 비밀번호만 초기화, Reset = 테이블명 + 비밀번호 전체 초기화 | — |

**동작**: Table Name은 Info Bar의 테이블 라벨(M-02t)과 Lobby의 테이블 카드에 표시되는 이름과 동기화된다. Lobby에서 테이블 선택 시 Table Name으로 식별하며, Password가 설정되어 있으면 진입 시 인증한다. Update 버튼을 눌러야 변경이 서버에 반영된다 (실시간 반영이 아닌 명시적 커밋 — 실수 방지).
...
Console 5탭 + Settings 다이얼로그의 설정이 Command Center와 오버레이에 어떻게 전파되는지를 요약한다. Console은 **사전 세팅 도구**이므로 방송 시작 전에 설정을 완료하고, 방송 중에는 Command Center와 오버레이가 이 설정을 참조하여 동작한다.

| 소스 | Console 설정 | CC 영향 | 오버레이 영향 |
|------|-------------|---------|-------------|
...
| GFX 탭 | Reveal Players (6) | CC에서 카드 공개 시점 연동 | 홀카드 공개 시각 효과 |
| GFX 탭 | How to Show Fold (7) | CC 폴드 시 시각 전환 | 폴드 플레이어 Graphic 제거 타이밍 |
...
| Settings | Table Name/Password (1~2) | RBAC 로그인 시 사용 | — |
...
|------|------|-----------|----------|----------|
| 2026-03-31 | v10.1.0 | Lobby 2단계 개발 전략 추가 (1단계 직접 수정 / 2단계 WSOPLIVE Staff 3-depth 연동), WSOPLIVE 참조 이미지 링크. PRD v33.1.0 동기화 | TECH | WSOPLIVE Staff 화면 참조 기반 Lobby 로드맵 |
| 2026-03-31 | v10.0.0 | **Flutter/Rive + Lobby + Command Center 전면 재설계**: (1) 2장 제목 "EBS Console"→"EBS App 3화면 구조", Lobby+Console+CC 화면 구조 테이블 추가, (2) "Launch AT" 버튼(M-14)→"Lobby" 버튼으로 변경, (3) Menu Bar에서 "Launch Action Tracker" 제거, (4) 운영 워크플로우 Lobby 경유 방식으로 재설계, (5) 교차 참조 AT→CC 전환, (6) Settings Table Name/Password를 RBAC+Lobby 체계로 변경, (7) 분리 문서 ebs-action-tracker→ebs-command-center로 이름 변경 | TECH | PRD v33.0.0 아키텍처 전환(Flutter+Lobby+CC) 동기화. AT 기능을 Lobby(Setup)와 CC(커맨드)로 분리 |
...
| 2026-03-11 | v9.0.0 | 3~6장(Command Center, 오버레이 그래픽, 화면 전환, 제약 조건) 별도 설계 문서로 분리 — PRD에는 교차 참조만 유지 | TECH | 설계 내용 방대, 개발 시 별도 진행하기에 문서 분리 |
