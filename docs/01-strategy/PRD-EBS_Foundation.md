...
![축구 중계 vs 포커 중계 — 핵심 차이](../../images/prd/prd-sport-vs-poker.png)
...
![52장 전체 카드 — 4무늬 × 13숫자 그리드](../../images/prd/card-deck.png)
...
![플레이어 시점(앞면) vs 상대 시점(뒷면)](../../images/prd/card-private.png)
...
![테이블 중앙 — 모든 플레이어가 공유하는 5장](../../images/prd/card-shared.png)

## 7장 중 최고 5장으로 승부

내 비공개 카드 **2장**과 공유 카드 **5장**, 합해서 **총 7장**이 놓인다. 이 중 가장 강한 **5장 조합**을 만든 사람이 이긴다.

![7장 중 5장 선택 — 조합 비교](../../images/prd/card-five-selection.png)
...
![베팅 액션 시각화](../../images/prd/card-betting-actions.png)
...
![Late Night Poker (1999) — 최초의 홀카드 방송](../../images/web/late-night-poker-1999-framed.png)
> *1999년 영국 Channel 4의 Late Night Poker. 투명 테이블 아래 카메라로 홀카드를 촬영한 최초의 방송. (출처: Channel 4)*

![WPT 홀카메라 — 테이블 유리판 아래 카메라](../../images/web/hole-card-cam-history-framed.png)
...
![RFID 태그가 내장된 포커 카드](../../images/web/rfid-live-poker-event-framed.png)
...
![WSOP 방송 프로덕션 전체 파이프라인](../../images/prd/prd-streaming-architecture.png)
...
![EBS 기술 아키텍처 — 입력에서 방송 출력까지](../../images/prd/prd-ebs-technical-architecture.png)
...
![RFID 테이블 3D 단면도 — 안테나 배치](../../images/prd/rfid-vpt-3d-crosssection.png)
...
![Action Tracker — 운영자 입력 화면](../../images/prd/app-action-tracker.png)
...
![카드 배분 — 각자 비공개 2장](../../images/prd/card-holecards.png)

카드 2장이 들어오면 → 보드에 공개된 카드와 조합하여 → 가능한 모든 경우를 계산한다.

![핸드 랭킹 — 기본 조합](../../images/prd/card-rankings.png)
...
![방송 프로덕션 현장](../../images/web/pokercaster-broadcast-setup-framed.png)

#### 전용 케이블로 보내는 방법

방송국에서 쓰는 전용 동축 케이블로 보내는 방법이다. 지연이 **10ms**로 가장 빠르다.

![BNC 커넥터 — SDI 케이블의 연결 단자](../../images/web/bnc-connector-framed.png)
...
![EBS 4단계 프로세스 — 카드 인식에서 방송 출력까지](../../images/prd/prd-ebs-4steps.png)

> ① 카드 인식 → ② 게임 엔진 → ③ 화면 생성 → ④ 방송 출력. 전체 파이프라인이 **100ms 이내**에 완료된다.

이 모든 과정을 하나의 소프트웨어에서 제어한다:

![EBS Console — 메인 화면](../../images/prd/app-console-main.png)
...
![Texas Hold'em 게임 진행 4단계](../../images/prd/card-stages.png)
...
![EBS 화면 구성 — Lobby, Command Center, Settings](../../images/prd/prd-3screens.png)
...
![포커 방송 하루 — 준비부터 종료까지](../../images/prd/prd-broadcast-day.png)
...
![5-Phase 로드맵 타임라인](../../images/prd/prd-5phase-timeline.png)
...
> 상세: `docs/api/API-03-rfid-hal-interface.md`, `docs/02-behavioral/BS-04-rfid/`
