...
- **영향팀**: [team4]
- **변경 대상 파일**:
  - `Behavioral_Specs/Overview.md` (legacy-id: BS-06-00)
  - `Holdem/Lifecycle.md` (legacy-id: BS-06-01)
  - `Holdem/Betting.md` (legacy-id: BS-06-02)
  - `Holdem/Blinds_and_Ante.md` (legacy-id: BS-06-03)
  - `Holdem/Showdown.md` (legacy-id: BS-06-07)
  - `Holdem/Exceptions.md` (legacy-id: BS-06-08)
  - `Event_Catalog.md` (legacy-id: BS-06-09)
- **변경 유형**: modify
...
- **계약 영향**: `contracts/api/API-02-backoffice.md` 또는 `WebSocket_Events.md` (legacy-id: API-05)에 `SET_TOURNAMENT_HEADS_UP` 이벤트 추가 필요 → **후속 CCR 필요**
...
- **계약 영향**: `Overlay_Output_Events.md` (legacy-id: API-04)에 PotUpdated 스키마 확장 → 후속 CCR
...
- `Overlay_Output_Events.md` (legacy-id: API-04): OutputEvent 카탈로그에 `ManagerRuling`, `DealCommunityRecovery` 소비 계약 추가 필요
...
1. `Holdem/Betting.md` (legacy-id: BS-06-02) §4.4 "Under-raise 처리 (Rule 95)" 신설
2. `Holdem/Betting.md` (legacy-id: BS-06-02) §4.5 "Raise Cap 적용 (Rule 100)" 표 추가
3. `Holdem/Betting.md` (legacy-id: BS-06-02) §5.3 "All-in Below Minimum Raise (Rule 96)" 신설
4. `Behavioral_Specs/Overview.md` (legacy-id: BS-06-00) Ch1 state fields에 `tournament_heads_up: bool` 추가
5. (별도 세션) `betting_rules.dart` 코드 수정 + 3개 신규 TC

### Phase 2 — P1 HIGH (5건)
1. `Holdem/Blinds_and_Ante.md` (legacy-id: BS-06-03) §5.2 "Heads-up 전환 시 Button 조정 (Rule 87)" + §5.3 딜링 순서
2. `Holdem/Lifecycle.md` (legacy-id: BS-06-01) §9.2 "Bomb Pot — Button Freeze & Opt-Out (Rule 28.3.2)"
3. `BS-06-00-REF.md` Ch1.9 "Mixed Game Rotation & Button Freeze"
4. `Holdem/Exceptions.md` (legacy-id: BS-06-08) §3.4 "Four-Card Flop 복구 (Rule 89)"
5. `Holdem/Showdown.md` (legacy-id: BS-06-07) §7 "핸드 보호 & 복구 (Rule 71/110)" + `BS-06-09` ManagerRuling/DealCommunityRecovery 이벤트 신설

### Phase 3 — P2 MEDIUM (6건)
1. `Holdem/Betting.md` (legacy-id: BS-06-02) §1.1 "베팅 입력 방식 (Rule 56 참조)"
2. `Holdem/Showdown.md` (legacy-id: BS-06-07) §6.2 "Muck 카드 재판정 (Rule 109)"
3. `BS-06-00-REF.md` Ch7.5 "Pot Size Display Policy (Rule 101)"
4. `Holdem/Lifecycle.md` (legacy-id: BS-06-01) §8.4 "Run It Multiple vs Rabbit Hunting (Rule 81)"
5. `Holdem/Blinds_and_Ante.md` (legacy-id: BS-06-03) §7 "Missed Blind 복귀 규정 (Rule 86)"
6. `Holdem/Exceptions.md` (legacy-id: BS-06-08) §5 "Deck Change 절차 (Rule 78)"
...
`Overlay_Output_Events.md` (legacy-id: API-04)에 `ManagerRuling`, `DealCommunityRecovery` 이벤트 소비 계약 추가를 위한 후속 CCR-DRAFT 작성. Conductor 검토 → `ccr_promote.py` 자동 promotion.
