...
reimplementability_notes: "status=PENDING, 89 code-only endpoints 3분류 진행 중"
---
...
> **2026-04-20 업데이트 (Agent D)**: 12건 모두 개별 SG 파일로 승격 완료. 각 파일에 default 제안 + 결정 옵션 포함.

| # | Endpoint | 승격 slug | 판정 필요 사항 | 상태 |
|:-:|----------|----------|---------------|------|
| b1 | `GET /api/v1/audit-events` | [SG-008-b1-audit-events.md](SG-008-b1-audit-events.md) | 감사 이벤트 공개 범위 + RBAC | ✓ 파일 생성 (default: 옵션 1 Admin-only public) |
| b2 | `GET /api/v1/audit-logs` | [SG-008-b2-audit-logs-read.md](SG-008-b2-audit-logs-read.md) | 로그 조회 RBAC (admin only? operator 범위?) | ✓ 파일 생성 (default: 옵션 1 별도 리소스 Admin only) |
| b3 | `GET /api/v1/audit-logs/download` | [SG-008-b3-audit-logs-download.md](SG-008-b3-audit-logs-download.md) | 다운로드 포맷 + rate limit | ✓ 파일 생성 (default: 옵션 1 sync streaming NDJSON, 90일 제한) |
| b4 | `GET /auth/me` | [SG-008-b4-auth-me.md](SG-008-b4-auth-me.md) | self-info endpoint — 반환 필드 결정 | ✓ 파일 생성 (default: 옵션 1 확장 필드 + no-cache) |
| b5 | `POST /auth/logout` | [SG-008-b5-auth-logout.md](SG-008-b5-auth-logout.md) | 세션 무효화 범위 | ✓ 파일 생성 (default: 옵션 1 current session + `?all=true` opt-in) |
| b6 | `POST /api/v1/sync/mock/seed` | [SG-008-b6-sync-mock-seed.md](SG-008-b6-sync-mock-seed.md) | mock endpoint 운영 환경 노출 여부 | ✓ 파일 생성 (default: 옵션 1 dev/staging only env 가드) |
| b7 | `DELETE /api/v1/sync/mock/reset` | [SG-008-b7-sync-mock-reset.md](SG-008-b7-sync-mock-reset.md) | 동상 — b6 과 쌍 | ✓ 파일 생성 (default: 옵션 1 b6 과 동일 정책) |
| b8 | `GET /api/v1/sync/status` | [SG-008-b8-sync-status.md](SG-008-b8-sync-status.md) | WSOP LIVE 연동 관측 — 공개/내부 결정 | ✓ 파일 생성 (default: 옵션 1 Public authenticated + Admin detail) |
| b9 | `POST /api/v1/sync/trigger/{source}` | [SG-008-b9-sync-trigger.md](SG-008-b9-sync-trigger.md) | 수동 sync 트리거 RBAC | ✓ 파일 생성 (default: 옵션 1 Admin only + concurrent reject) |
| b10 | `POST /api/v1/events/{event_id}/undo` | [SG-008-b10-event-undo.md](SG-008-b10-event-undo.md) | Undo 기능 기획 자체 부재 | ✓ 파일 생성 (default: 옵션 3 Phase 1 미지원 — 삭제) |
| b11 | `POST /api/v1/tables/{table_id}/launch-cc` | [SG-008-b11-table-launch-cc.md](SG-008-b11-table-launch-cc.md) | CC 원격 launch 필요성 | ✓ 파일 생성 (default: 옵션 1 Deep-link 전환, 코드 삭제) |
| b12 | `GET /api/v1/reports/{report_type}` | [SG-008-b12-legacy-reports.md](SG-008-b12-legacy-reports.md) | SG-007 대체 완료 후 deprecate | ✓ 파일 생성 (default: 옵션 1 즉시 삭제 — 사용처 검증 후) |
...
## (a) 77건 Backend_HTTP §5.17 일괄 편입 완료 (2026-04-20, Agent G)

77건 모두 `Backend_HTTP.md §5.17 CRUD 완결성 편입` 섹션에 리소스별 테이블로 편입. 편입 방식:

- §5.17.1 Users (4) / §5.17.2 Competitions (4) / §5.17.3 Series (5) / §5.17.4 Events (5)
- §5.17.5 Flights + Clock 제어 (20) / §5.17.6 Tables + Seats (7) / §5.17.7 Players (5)
- §5.17.8 Hands (4) / §5.17.9 BlindStructures flat (5) / §5.17.10 PayoutStructures flat (5)
- §5.17.11 Skins (5) / §5.17.12 Decks (6) / §5.17.13 Configs (2) / §5.17.14 Reports (6) / §5.17.15 Settings (1)

편입 원칙:
- 각 endpoint 는 method/path/용도/RBAC/status code 만 명시 (패턴 + 테이블 방식)
- Request/Response schema 는 `team2-backend/src/routers/<res>.py` Pydantic 모델 참조 (OpenAPI 자동 생성)
- 새 리소스·새 동작 추가 없음 — 기존 §5.2~5.16 의 CRUD 대칭 완결만
- 엄밀한 full spec 은 Phase 2 production 이전. 프로토타입 단계는 본 수준으로 "재구현 가능성" 확보

**참조**: `Backend_HTTP.md §5.17` — 리소스별 테이블 + §5.17.16 편입 총괄표

## 수락 기준

- [x] 상위 30개 샘플 분류 결과가 Conductor 승인 (v2.0 문서)
- [x] 나머지 59개 (실제 89개 전체) 자동 분류 완료 (v3.0, subagent B 2026-04-20)
- [x] (a) 기획 추가 77건 Backend_HTTP.md §5.17 일괄 편입 (v5.0, Agent G 2026-04-20)
- [x] (b) 판정 필요 12건이 SG-008-b1 ~ b12 로 개별 승격 (v4.0, 2026-04-20 Agent D)
- [ ] 각 b1~b12 의 옵션 결정 (decision_owner: conductor. default 제안 채택 여부)
- [ ] 최종 `python tools/spec_drift_check.py --api` 의 D3 = (b) 미결정 12건만 남음 (주의: scanner 의 spec 측 매칭은 dotted path/표기 변형에 여전히 불완전. §5.17 편입 후 D3 수치는 자동 감소하지 않을 수 있음 — `Backend_HTTP.md §5.17` 를 신뢰 소스로 삼는다)
...
| 2026-04-20 | **v4.0 (b) 파일 승격 완료** | Agent D 가 SG-008-b1~b12 12개 파일 생성. 각 파일에 default 제안 + 결정 옵션 + 수락 기준 포함. 본 마스터에 파일 링크 추가 |
| 2026-04-20 | **v5.0 (a) 편입 완료** | Agent G 가 Backend_HTTP.md §5.17 신설 — 77건 리소스별 CRUD 테이블 일괄 편입 완료. 엄밀한 full spec 은 Phase 2 production 이전 |
