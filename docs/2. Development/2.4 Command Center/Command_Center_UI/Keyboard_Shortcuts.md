...
| 2026-05-07 | v4 cascade | CC_PRD v4.0 정체성 정합 — **8 키 (N·D·F·C·B·R·A) → 6 키 (N·F·C·B·A·M)** 통합. D 키 폐기 (createSession auto), R 키 → B 키 통합 (BET/RAISE auto-switch), M 키 신설 (Menu/Manual/Miss Deal). §"v4.0 6 키 단축키 표준" 신설. SSOT: `docs/1. Product/Command_Center_PRD.md` v4.0 §Ch.5. |
...
## v4.0 6 키 단축키 표준 (2026-05-07 신설, SSOT)

> **트리거**: `docs/1. Product/Command_Center_PRD.md` v4.0 cascade. 8 키 (N·D·F·C·B·R·A) 시대가 끝나고 **6 키 (N·F·C·B·A·M) + Ctrl+Z (UNDO)** 시대가 시작된다.
...
| **Ctrl+Z** | Undo | 항상 (현재 핸드 내) | 항상 | (HAND_COMPLETE 후 disabled) | 별도 |

### 8 키 (v1.x) → 6 키 (v4.0) 매핑

| v1.x 키 | v4.0 키 | 변환 근거 |
|---------|---------|----------|
| N (NEW HAND) | **N** (lifecycle) | 1:1, 단 HAND_COMPLETE 에서 FINISH HAND 추가 |
| **D (DEAL)** | (폐기) | createSession 시점에 auto HandStart + holecards (Engine §2.1) |
| F (FOLD) | **F** | 1:1 |
| C (CHECK/CALL) | **C** | auto-switch 룰 동일 |
| B (BET) | **B** | BET/RAISE 통합 — auto-switch |
| **R (RAISE)** | (B 통합) | biggest_bet > 0 시 B 키가 RAISE 로 자동 전환 |
| A (ALL-IN) | **A** | 1:1 |
| (신규) | **M** | Miss Deal / Manual 진입 |

### 6 키의 가치

> ★ *같은 키 = 같은 손가락 위치 = 다른 의미*. 손가락은 알파벳을 외우지 않고 *위치* 를 외운다. 8 키 → 6 키 통합으로 손가락이 외워야 할 위치 25% 감소.

### 자매 문서 정합

- `Action_Buttons.md §"v4.0 6 키 매핑"` — 본 §과 동기 SSOT
- `Hand_Lifecycle.md §"5-Act 시퀀스"` — Act × 6 키 활성 표
- `Manual_Card_Input.md` — M 키 (Manual) 진입 흐름
- KeyboardHintBar (V1, ✅ 구현 완료) — 본 §의 6 키 시각 표시

---

## [archive — v1.x] 1. 액션 단축키 (8 키)

> ⚠️ **Archive (v1.x)**: 본 §은 v4.0 §"6 키 단축키 표준" 으로 *override* 됨. D 키 폐기, R 키 → B 통합, M 키 신설. 인용 시 v4.0 표 우선.
