...
> **참조**: `BS-05-00 §6 AT 화면 카탈로그`, `BS-02-lobby §테이블 통계`, 계산식은 team3 Game Engine, 전송은 `Backend_HTTP.md §통계 엔드포인트` (legacy-id: API-01).

> **공유 소비자 (2026-04-21 신설)**: Lobby `BS-02-HH Hand_History.md` §2.3 Player Hand Stats 가 본 화면과 동일한 engine 통계 (VPIP/PFR/AGR/WTSD/W$SD) 를 재사용한다. 계산식 변경은 team3 engine 에서만 발생하며 CC AT-04 와 Lobby Hand History 양쪽에 자동 반영된다. 중복 정의 금지.
...
- `Backend_HTTP.md` (legacy-id: API-01) — 통계 엔드포인트 상세
