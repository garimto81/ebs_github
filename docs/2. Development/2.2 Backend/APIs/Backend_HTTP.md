...
> **SG-008-b11 v1.2 (2026-05-03 Web variant 복원)**: Phase 1 Korea soft-launch 가 Docker Web 배포 (lobby-web :3000 + cc-web :3001) 로 결정되어 OS deep-link (browser 미지원) 단독 적용 시 chain break. **Endpoint 재도입** + dual response (cc_url for browser, deep_link for desktop). 상세: §16.12. 근거: 9/9 E2E PASS evidence (`tools/e2e_lobby_to_cc_ws.py`, 2026-05-03).
...
| b11 | `POST /tables/{table_id}/launch-cc` | **v1.2 (2026-05-03) Web variant 복원** — endpoint + dual response (cc_url + deep_link) | §5.4 + §16.12 |
...
### 16.12 POST /tables/{table_id}/launch-cc (b11 — v1.2 Web variant)
...
| 환경 | 사용 필드 | 동작 |
|------|----------|------|
| Web (browser) | `cc_url` | `window.open(cc_url, '_blank')` |
...
**근거**: SG-008-b11 v1.2 (Phase 1 Web 배포 변형). WSOP LIVE deep-link 패턴 + EBS Web Multi-Service 배포 모델 hybrid.
