...
confluence-page-id: 3819078266
confluence-parent-id: 3811770578
confluence-url: https://ggnetwork.atlassian.net/wiki/spaces/WSOPLive/pages/3819078266/EBS+WebSocket+Events
---
...
#### 1.2.1 Foundation Ch.5 §B.4 정합 — DB SSOT + 2 채널 동기화 (2026-04-22 신설, 2026-05-08 cascade 정합)

Foundation Ch.5 §B.4 "실시간 동기화 — DB SSOT + WS push" 정책에 따라 EBS 내부 상태 동기화는 다음 2 채널 조합으로 수행된다. 본 WebSocket 프로토콜은 **실시간 push 채널** 역할을 담당한다.
...
> **참조**: Foundation Ch.5 §B.4 (상위 정책), `Back_Office/Sync_Protocol.md §1` (팀2 내부 구현 scope).
...
**§1.2.1 SSOT 분류** (Foundation Ch.5 §B.3/§B.4 정렬):
...
| `cc_session_count` | §4.2.10 참조 | CC WebSocket 연결/끊김 시 | 활성 CC 카운트 (Lobby TopBar `cc-pill` 갱신, EBS 독자) |
...
#### 4.2.10 cc_session_count (Phase 3.C, 2026-05-06)

- **Trigger**: CC 채널 WebSocket 연결 / 끊김 시 (`websocket/manager.py:_broadcast_cc_session_count`).
- **Payload**: `{ type: "cc_session_count", data: { count: <int> }, seq: 0 }`
- **Consumer**: Lobby (`activeCcCountProvider` → TopBar `cc-pill` 위젯).
- **Channel**: `lobby` 채널 broadcast (table_id `*`).
- **Envelope 주의**: 현재 구현은 `seq=0` 고정. 본 이벤트는 게임 상태 변경이 아닌 connection topology 변경이므로 `audit_events` 저장 대상 아님. 따라서 단조증가 `seq` 보장 (§2 envelope) 및 replay (§6.4) 대상에서 제외된다.
- **WSOP LIVE 대응**: EBS 독자 이벤트 (WSOP LIVE 미정의).
- **다중창/단일창 모드 영향** (Foundation Ch.5 §B.2): CC 인스턴스 수가 OS 프로세스 단위로 변하면 카운트가 즉시 갱신된다. Lobby 가 BO 경유로만 수신하므로 다중창 모드에서도 동일하게 동작 (직접 IPC 금지 원칙).

#### 4.2.11 chip_count_synced (Cycle 20, 2026-05-13)

- **Trigger**: WSOP LIVE 가 브레이크 시 딜러 입력 chip count 를 `POST /api/wsop-live/chip-count-snapshot` (webhook) 으로 push → BO 가 HMAC + Idempotency 검증 통과 후 `chip_count_snapshots` 테이블 commit → broadcast.
- **Authority window**: 브레이크 = WSOP LIVE 가 stack 의 **권위 시점**. 그 외 시간에는 Engine 이 자동 계산한 값이 SSOT. 본 이벤트 도착 시 그 snapshot 의 chip_count 가 truth 로 채택 → Engine reconcile 트리거.
- **Payload schema**:

| 필드 | 타입 | 설명 |
|------|------|------|
| `type` | str | `"chip_count_synced"` 고정 |
| `seq` | int | envelope monotonic seq (§2.1) |
| `data.table_id` | int | EBS table id |
| `data.snapshot_id` | UUID | webhook idempotency key (DB unique) |
| `data.break_id` | int | WSOP LIVE 브레이크 식별자 (해당 브레이크 동안의 모든 테이블 push 묶음) |
| `data.seats` | array | `[{seat_number: int, player_id: int, chip_count: int}]` |
| `data.recorded_at` | ISO8601 | WSOP LIVE 측 딜러 입력 시점 (권위 시점) |
| `data.received_at` | ISO8601 | BO 수신 시점 (audit) |
| `data.signature_ok` | bool | HMAC 검증 결과 (정상 케이스 true. false 면 본 이벤트 자체가 발행 안 됨) |

- **Subscribers**:
  - **CC** (S3): `seat_cell.stack` 갱신 → 9 카테고리 #1 플레이어 대시보드 chipstack 동기화
  - **Overlay** (S3/S8): 플레이어 대시보드 view 갱신 (현재 hand 진행 중이면 다음 OutputEvent 사이클에 반영)
  - **Lobby** (S2): table list 의 chip total 갱신 (테이블별 합계)
  - **Engine** (S8): reconcile loop 트리거 → drift 감지 + log (`Chip_Count_State.md` §3)
- **Channel**: `lobby` + `cc:{table_id}` + `overlay:{table_id}` 동시 broadcast (구독자 별 자동 라우팅).
- **Envelope**: 단조증가 `seq` 보장 + `audit_events` 저장 대상 (replay 가능). 게임 상태에 영향을 주는 이벤트이므로 cc_session_count 와 달리 replay 필수.
- **WSOP LIVE 대응**: 본 이벤트는 EBS 가 webhook 수신 후 발행하는 **derivative event**. WSOP LIVE 측 ↔ EBS 측 경계는 [Foundation Ch.5 §B.5](../../../1.%20Product/Foundation.md) 흐름도 참조.
- **Contract pointer**: webhook spec 본체는 [WSOP_LIVE_Chip_Count_Sync.md](WSOP_LIVE_Chip_Count_Sync.md). 본 §4.2.11 은 그 결과를 EBS 내부 소비자에게 broadcast 하는 WS 이벤트만 정의.
- **State machine pointer**: ENGINE_AUTO ↔ WSOP_AUTHORITATIVE 전이는 [Chip_Count_State.md](../../2.5%20Shared/Chip_Count_State.md) §1.
