...
다섯 명 중 누구 하나라도 빠지면 화면은 절반의 거짓이 된다.

| 빠진 작가 | 화면이 되는 것 |
|----------|---------------|
| Rive Asset 가 없으면 | 데이터는 옷을 입을 수가 없다. 숫자만 떠 있는 표 |
| EBS DB 가 없으면 | 선수는 익명의 그림자가 된다 |
| Command Center 가 없으면 | 화면은 멈춘 정물화가 된다 |
| RFID 가 없으면 | 카드는 추상이 된다 |
| Game Engine 이 없으면 | 모든 숫자가 0 이 된다 |
...
- [Ch.3 — Rive 라는 그릇](#ch3) — 왜 Rive 인가
- [Ch.4 — Rive Editor ↔ EBS 통합 두 경로](#ch4) — Export-Import vs API 직접 연동
- [Ch.5 — Variable Binding 의 마법](#ch5) — 데이터가 그래픽이 되는 순간

**Part III — 다섯 데이터 소스**

- [Ch.6 — Rive Asset](#ch6) — 그래픽의 vessel
- [Ch.7 — EBS DB](#ch7) — 영속의 기억
- [Ch.8 — Command Center 입력](#ch8) — 인간의 결정
- [Ch.9 — RFID](#ch9) — 카드의 정체
- [Ch.10 — Game Engine](#ch10) — 계산된 진리
...
| 작가 | 가져오는 것 | 비유 |
|------|------------|------|
| **Rive Asset** | 그래픽의 형태 (벡터 도형, 색, 폰트, 애니메이션) | 의상과 표정의 사전 |
| **EBS DB** | 영속 데이터 (선수 프로필, 토너 정보, 브랜드 패키지, 히스토리) | 도서관의 인명 사전 |
| **Command Center** | 운영자 결정 (FOLD / BET / RAISE / ALL-IN / 핸드 시작 / 종료) | 무대의 큐 사인 |
| **RFID** | 카드의 정체 (무늬·숫자·좌석) | 카드의 신분증 |
| **Game Engine** | 계산된 결과 (팟, 사이드 팟, Equity, 핸드 강도, 스택 변동) | 회계사의 장부 |
...
2. **어떻게 만들어지는가?** — Rive 라는 그릇 + Variable Binding (Part II)
...
> **모든 카테고리에는 Rive Asset 이 공통 의상**. 위 표에서는 그 외 작가만 표시.

각 카테고리는 Part IV 의 별도 챕터에서 — 그래픽 모양, 다섯 작가에서 받는 정확한 변수, 변화의 순간 — 으로 풀어 다룬다.
...
다섯 작가의 데이터를 화면에 옮기는 그릇이 **Rive** 다. 왜 Rive 인가.
...
## Ch.5 — Variable Binding 의 마법

Rive 가 정말 강력한 이유는 — 그래픽의 거의 모든 속성이 **변수에 묶일 수 있다**는 데 있다.
...
이 Part 는 다섯 작가 각자의 자리를 자세히 들여다본다. 각 작가가 어떤 데이터를 가지고 있고, 그 데이터가 어떤 변수를 채우는지.
...
다른 네 작가는 모두 **데이터** 만 가져온다. Rive Asset 만이 **형태** 를 가져온다. 데이터가 어떻게 보일지를 결정하는 유일한 작가다.

### 6.2 디자이너의 책임

Rive Asset 의 작가는 **외부 디자이너** 다. 디자이너가 Rive Editor 에서 — 본 문서의 Part IV 12 카테고리 각각에 대해 — `.riv` 파일을 만든다.
...
### 6.3 Brand Pack 이라는 의상장

대회마다 그래픽의 색·폰트·로고가 다르다. 그러나 `.riv` 파일을 대회마다 새로 만들지 않는다. 대신 — `.riv` 안의 색·폰트·로고를 **Brand Pack** 이라는 별도 데이터로 빼낸다.
...
> EBS DB = 도서관의 인명 사전 + 대회 안내 책자 + 디자인 가이드북.
...
운영자가 키보드로 입력하는 — 다른 어떤 작가도 알 수 없는 — 한 핸드의 **결정** 들.
...
> Command Center = 무대의 큐 사인. 큐가 떨어지는 그 순간 모든 배우가 다음 동작으로 움직인다.
...
> RFID = 카드의 신분증. 신분증을 보여줘야 카드는 비로소 자기 정체를 갖는다.
...
다른 네 작가의 데이터를 받아 — **계산해서** 새 데이터를 만든다. 가장 풍부한 출력을 가진 작가다.
...
Engine 은 자기 혼자 일하지 않는다. 다른 네 작가의 데이터에 기반한다.
...
> Game Engine = 회계사. 다른 작가들이 보낸 거래 내역을 모아 — 잔고와 손익을 계산해 화면에 띄운다.
...
다섯 작가가 **같은 변수에 글을 쓰는 일은 없다**. 각 변수는 **단 하나의 작가가 책임**진다. 그러나 한 그래픽 한 조각을 만들기 위해서는 — 여러 변수가 동시에 채워져야 한다.
...
```
   ...
   |   brand_color_*    |  EBS DB         |  (Brand Pack)
   |   (의상 자체)       |  Rive Asset     |  (.riv 파일)
   +-------------------------------------+
```

다섯 작가 모두 — 한 Player Card 의 변수를 채운다. 한 작가라도 빠지면 그 자리는 빈 변수로 남고, 그래픽은 절반의 거짓이 된다.
...
본 매트릭스는 다섯 작가가 12 카테고리에 어떻게 기여하는지를 한 번에 보여준다.

```
                      Rive   EBS    CC    RFID   Engine
                      Asset  DB
   ----------------- ------ ----- ----- ------ -------
   Player Card        의상   이름   폴드   좌석    스택
   Stack + Bet        의상   -      -      -      합산
   홀카드             의상   -      -      ID     공개시점
   커뮤니티 카드       의상   -      -      ID     공개시점
   Hand Strength      의상   -      -      -      계산
...
> 다섯 작가는 같은 책의 다른 페이지를 쓴다. 한 페이지가 비면 책은 절반의 거짓이 된다.
...
| 변수 | 작가 |
|------|------|
| 의상 (도형, 폰트, 그림자) | Rive Asset (`player_card.riv`) |
...
| 변수 | 작가 |
|------|------|
| 의상 (라인 도형, 칩 아이콘) | Rive Asset (`stack_bet.riv`) |
| `stack_amount` | Game Engine |
...
> 칩은 정지된 자산이 아니라 흐르는 화살표다 — 어디로 흘러가는가가 그 핸드의 운명이다.
...
| 변수 | 작가 |
|------|------|
| 의상 (카드 도형, 무늬 SVG) | Rive Asset (`hole_card.riv`, `community_card.riv`) |
| `card_id` (예: "AS") | RFID |
...
> 비밀이 공유로 변하는 다섯 단계 — 그것이 한 핸드의 서사다.
...
| 변수 | 작가 |
|------|------|
| 의상 (게이지 도형) | Rive Asset (`hand_strength.riv`) |
...
게이지가 100% 가 되면 — 우승 직전임을 알리는 황금색 펄싱 (Rive 의상 책임).
...
| 변수 | 작가 |
|------|------|
| 의상 (라벨 도형, 박스) | Rive Asset (`action_badge.riv`) |
...
| 변수 | 작가 |
|------|------|
| 의상 (팟 박스, 칩 그래픽) | Rive Asset (`pot.riv`) |
...
| 변수 | 작가 |
|------|------|
| 의상 | Rive Asset (`tournament_state.riv`) |
...
| 변수 | 작가 |
|------|------|
| 의상 (슬롯 위치, 회전 메커니즘) | Rive Asset (`branding.riv`) |
...
| 변수 | 작가 |
|------|------|
| 의상 (debug 패널 도형) | Rive Asset (`ops_overlay.riv`) |
...
State 의 전환은 **Rive Variable 의 값 변화** 가 trigger 한다. 다섯 작가 중 누구든 — 자기 책임 변수를 바꾸면 — Rive 가 자동으로 적절한 상태로 이동한다.
...
> State Machine = 그래픽의 일생. 변수가 바뀌면 그래픽이 다음 인생 단계로 넘어간다.
...
| 날짜 | 버전 | 변경 | 결정 근거 |
|------|------|------|----------|
| 2026-05-07 | v0.2.0 | 전면 재설계 — Voldemort 룰 적용 (시간 약속 / human error / 다른 PRD cross-ref 모두 제거). 5 데이터 소스 (Rive Asset / EBS DB / Command Center / RFID / Game Engine) 중심으로 재구조. Rive Editor ↔ EBS 통합 2 경로 (Export-Import vs API) 명시. Variable Binding 메커니즘 신설. 12 그래픽 요소를 5 작가 매핑 표로 통일. | 사용자 비평 — 서론부터 잡소리, 1 초의 약속 같은 거 거론 금지, 다른 PRD 비교 제거, 5 데이터 소스 + Rive 통합이 핵심 |
| 2026-05-07 | v0.1.0 | 최초 작성 (SUPERSEDED by v0.2.0) | — |

---

## 작성 메모

> 본 문서는 **다섯 작가의 만남** 이라는 단 하나의 thesis 로 통일되어 있다. 모든 챕터는 — 누가 쓰는가 / 무엇을 쓰는가 / 어디에 합류하는가 — 의 세 질문에만 답한다.
>
