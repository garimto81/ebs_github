...
EBS는 **3개 별도 앱 + 1개 독립 패키지**로 구성된다. WSOP LIVE 관제 허브 패턴을 참조하되, Lobby/CC 모두 Flutter 프레임워크로 구현하여 **의도적 divergence** (원칙 1 §"적용 예외": CC 와 동일 프레임워크 공유, rive 프리뷰 성능 요구, Flutter/Dart 코드 재사용). **Lobby 는 Web 배포 (Docker nginx, 브라우저 접속)**, **CC 는 Windows Desktop 앱** (RFID 시리얼 하드웨어 접근 필수).
...
> **금지**: "단일 Flutter 앱의 2개 화면" 표현. Lobby(Web 배포)와 CC(Desktop 앱)는 별도 앱이다 (동일 Flutter 프레임워크이지만 배포 타겟 다름).
