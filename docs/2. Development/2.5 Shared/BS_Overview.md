...
reimplementability_notes: "§1 Tech Stack SSOT 3중화 해소 (SG-001 채택: Flutter). 2026-04-27 SG-022 (단일 Desktop 바이너리 통합) → 같은 날 저녁 multi-service Docker SSOT 로 SUPERSEDED → 2026-05-04 SG-030 재확인 (design SSOT + 사용자 의사결정). 현재 §1.5 표 = Lobby Flutter Web (lobby-web :3000) / CC Flutter Web (cc-web :3001) 또는 Desktop / Overlay Flutter Desktop."
---
...
| 2026-04-27 | SG-022 결정 — §1 전면 재작성 (단일 Desktop 바이너리, γ 하이브리드 폐기). Lobby + Settings + Rive Manager + Command Center + Overlay 모두 단일 Flutter Desktop 바이너리로 배포. Web 빌드는 Phase 2 옵션. §1 표 "배포 형태" 컬럼 통일. 용어 구분 주의 callout 갱신. Ref: Spec_Gap_Registry SG-022, Phase_1_Decision_Queue.md (2026-04-27) **[SUPERSEDED 같은 날 저녁]** |
| 2026-05-04 | SG-030 cascade — §1.5 앱 구성 표 정정. Lobby = Flutter Web (lobby-web :3000) / CC = Web (cc-web :3001) 또는 Desktop / Overlay = Desktop. §"관계" 의 "단일 바이너리 내 라우팅" → "Web 브라우저 다중 클라이언트" 로 정합화. reimplementability_notes 갱신. Ref: `docs/4. Operations/Conductor_Backlog/SG-030-lobby-overview-flutter-vs-web.md`, `docs/4. Operations/Lobby_Modification_Plan_2026-05-04.md` |
...
| 용어 | 정의 | 기술 | 비고 |
|------|------|------|------|
| **Lobby** | 모든 테이블의 관제·설정 허브 | Flutter/Dart + Riverpod + Freezed + Dio + go_router + rive | **Flutter Web 배포** (lobby-web Docker 컨테이너 :3000, nginx). Rive Manager 허브 포함. Tech Stack SSOT (CCR-016, SG-001). ~~단일 Desktop 바이너리~~ SG-022 SUPERSEDED 2026-04-27 저녁 + SG-030 재확인 2026-05-04. 개발자 디버깅: `flutter run -d chrome` |
| **Command Center (CC)** | 게임 진행 커맨드 입력 화면. 테이블당 1개 인스턴스 | Flutter/Dart + Riverpod + Dio + rive | 구 PokerGFX Action Tracker. **Flutter Web (cc-web Docker :3001)** 또는 Flutter Desktop (RFID 시리얼 직결 운영 환경). Lobby 와 별도 Flutter 프로젝트. |
| **Overlay** | 시청자 방송 화면 그래픽 출력 | Flutter + Rive | **Flutter Desktop** (송출 직결, SDI/NDI). CC 와 동일 Desktop 바이너리 / 기술 스택. |
...
**관계**:
- Lobby : CC = **1 : N** (1개 Lobby (Web 브라우저 다중 클라이언트) 에서 N개 테이블의 CC 인스턴스 (각 테이블당 1) 관리)
