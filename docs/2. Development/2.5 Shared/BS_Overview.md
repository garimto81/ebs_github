...
EBS는 **3개 별도 앱 + 1개 독립 패키지**로 구성된다. WSOP LIVE 관제 허브 패턴을 참조하되, Lobby 는 Desktop Flutter 로 **의도적 divergence** (원칙 1 §"적용 예외": RFID HAL 시리얼 접근, CC 와 동일 스택 공유, rive 프리뷰 성능 요구).

| 용어 | 정의 | 기술 | 비고 |
|------|------|------|------|
| **Lobby** | 모든 테이블의 관제·설정 허브. Desktop Flutter 앱 | Flutter/Dart (Desktop) + Riverpod + Freezed + Dio + go_router + rive | WSOP LIVE Staff Page 역할 대응. GE 허브 포함. Tech Stack SSOT (CCR-016, SG-001 resolved 2026-04-20). |
| **Command Center (CC)** | 게임 진행 커맨드 입력 화면. 테이블당 1개 인스턴스 | Flutter/Dart (Desktop) + Riverpod + Dio + rive | 구 PokerGFX Action Tracker. Lobby 와 동일 스택 |
...
| **Graphic Editor (GE)** | 아트 디자이너 `.gfskin` Import + 메타데이터 편집 + Activate 허브. Lobby 탭(`/lobby/graphic-editor`). | Flutter/Dart + rive (프리뷰 전용) | Admin 전용. Lobby 앱 내부 기능 (별도 런타임 아님). 동일 `.riv` 바이너리 공유. (CCR-011, SG-001) |
