...
### SSOT 선언 (B-332, Foundation §6.3 §1.1.1 / §6.4 / foundation_ssot.md §11)

**통신 모델**: CC → Engine **REST stateless query** (foundation_ssot.md §11 통신 매트릭스). 서버 측 세션은 `sessionId` 로 조회되며, 매 요청은 독립적 (HTTP REST stateless 원칙). Engine 자체 in-memory state 는 sessionId-keyed snapshot 이고 client 가 cursor 로 동기화한다.
