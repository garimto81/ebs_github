...
Admin이 업로드·편집된 스킨을 **실제 방송에 활성화(activate)**하는 흐름을 정의한다. Activate 시 Backend는 `active_skin_id`를 DB에 기록하고 모든 CC/Overlay 인스턴스에 `skin_updated` WebSocket 이벤트를 broadcast한다. 다중 CC는 500ms 이내에 동시 리로드되어 시청자에게 매끄러운 전환을 제공한다.
...
activating
  ...
  ↓
broadcasting
  ↓ 서버가 skin_updated WS broadcast (GEA-05)
...
if state == "RUNNING":
  다이얼로그 표시:
    제목: "방송 진행 중입니다"
    본문: "현재 Table {id}에서 Hand {hand_id}가 진행 중입니다.
...
## 4. Broadcast — `skin_updated` WebSocket 이벤트

Backend는 Activate 성공 시 모든 구독자에게 `skin_updated` 이벤트를 발행한다. 상세 페이로드는 `API-05 §Events skin_updated` 참조.
...
WS 수신: skin_updated
  ↓
GET /api/v1/skins/{skin_id} (`.gfskin` bytes 다운로드)
  ↓
...
2. `GET /api/v1/events/replay?from_seq={last_seq}&channel=cc_event` → 놓친 이벤트 재생
...
- `API-05-websocket-events.md` — skin_updated 이벤트 (CCR-015)
