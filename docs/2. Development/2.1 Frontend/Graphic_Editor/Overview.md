...
| 페르소나 | 역할 | 사용 경로 |
|----------|------|-----------|
| **Art Designer** | `.gfskin` 제작·업로드·메타데이터 조정 | Lobby Web (브라우저) |
| **Admin** | Activate 결정, RBAC gate 통과, 배포 책임 | Lobby Web (브라우저) |
| **Operator** | 리스트 조회, 프리뷰, 메타데이터 조회 (편집 불가) | Lobby Web (브라우저) |
| **Viewer** | GE 라우트 접근 불가 | — |
