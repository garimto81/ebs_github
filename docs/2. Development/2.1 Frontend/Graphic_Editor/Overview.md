...
Graphic Editor(이하 **GE**)는 EBS Overlay가 사용하는 `.gfskin` 스킨 아티팩트를 **업로드·검증·메타데이터 편집·Activate**하는 허브다. Team 1 Lobby의 `/lobby/graphic-editor` 라우트로 구현되며, **Flutter Desktop + `rive` ^0.13** (team4 Overlay 와 동일 C++ native runtime, Rive 2026 `rive_native` bridge 기반) 기술 스택을 사용한다.
...
| 페르소나 | 역할 | 사용 경로 |
|----------|------|-----------|
| **Art Designer** | `.gfskin` 제작·업로드·메타데이터 조정 | Flutter Desktop |
| **Admin** | Activate 결정, RBAC gate 통과, 배포 책임 | Flutter Desktop |
| **Operator** | 리스트 조회, 프리뷰, 메타데이터 조회 (편집 불가) | Flutter Desktop |
| **Viewer** | GE 라우트 접근 불가 | — |
...
## 8. Rive 호환성 (Flutter `rive` 프리뷰 구현 가이드)

> 2026-04-15 초안, **2026-04-21 Flutter 전환** (Quasar/rive-js → Flutter/rive). GE 의 프리뷰 widget 및 메타데이터 cross-validation 구현에 필요한 버전·스키마·코드 예제를 고정한다.

### 8.1 지원 Rive 런타임 버전 범위

| 라이브러리 | 버전 | 채택 이유 |
|-----------|------|----------|
| `rive` (pub.dev) | **^0.13** (Overlay 와 동일) | Rive Flutter 2026 `rive_native` C++ bridge 기반. team4 Overlay 와 런타임 동일 → 프리뷰 ≡ 송출 보증. |
| 내부 `rive_native` (C++) | 0.13 에 번들 | Flutter package 의존성으로 자동 해결 |
| Rive 파일 포맷 runtime | `runtime >= 7` 만 허용 | 구버전 파일은 파싱 실패 유도 (`INVALID_RIVE_RUNTIME` 에러) |

**Breaking 업그레이드 정책**: `rive` major 업그레이드 검토는 별도 스토리. 업그레이드 전 본 문서 §8 과 `../Engineering.md` 의 `skin_updated` 처리, team4 Overlay 렌더러 간 통합 테스트 필수.
...
메타데이터 편집 화면의 프리뷰 widget 에서 상태 전환·숫자 입력·트리거를 사용자가 직접 조작할 수 있어야 한다.
...
class SkinPreview extends StatefulWidget {
  final String skinBytesPath; // ZIP 에서 추출한 .riv 로컬 경로 또는 in-memory bytes
  const SkinPreview({super.key, required this.skinBytesPath});
...
class _SkinPreviewState extends State<SkinPreview> {
  late final RiveAnimationController _ctrl;
  StateMachineController? _smc;
  SMIBool? _handFeatured;
  SMINumber? _seatCount;
  SMITrigger? _revealCards;

  void _onRiveInit(Artboard artboard) {
    // skin.json.state_machines[0].name
    _smc = StateMachineController.fromArtboard(artboard, 'MainSM');
    if (_smc == null) return;
    artboard.addController(_smc!);
    _handFeatured = _smc!.findInput<bool>('handFeatured') as SMIBool?;
    _seatCount = _smc!.findInput<double>('seatCount') as SMINumber?;
    _revealCards = _smc!.findInput<bool>('revealCards') as SMITrigger?;
  }
  ...
  Widget build(BuildContext context) {
    return Column(children: [
      SizedBox(
        height: 600,
        child: RiveAnimation.file(widget.skinBytesPath, onInit: _onRiveInit),
      ),
      // UI 에서 주입
      Switch(value: _handFeatured?.value ?? false,
             onChanged: (v) => _handFeatured?.value = v),
      Slider(value: _seatCount?.value ?? 0, min: 0, max: 10,
             onChanged: (v) => _seatCount?.value = v),
      ElevatedButton(onPressed: () => _revealCards?.fire(),
                     child: const Text('Reveal')),
    ]);
  }

  @override
  void dispose() { _smc?.dispose(); super.dispose(); }
}
```

| 입력 타입 | 주입 방법 | UI |
|----------|----------|-----|
| `boolean` (`SMIBool`) | `input.value = true/false` | Material `Switch` 또는 `Checkbox` |
| `number` (`SMINumber`) | `input.value = N` | `Slider` 또는 `TextField` keyboardType=number |
| `trigger` (`SMITrigger`) | `input.fire()` | `ElevatedButton` |

입력 목록은 `skin.json.state_machines[].inputs` 와 자동 대조하여 Widget 을 동적 생성 (하드코딩 금지). 누락된 input 은 "스킨 정의와 불일치" SnackBar/Banner 경고로 표시.

### 8.4 프리뷰 해상도 전환

| 설정 | 처리 |
|------|------|
| skin.layout_type == 'horizontal' | 프리뷰 widget `AspectRatio(aspectRatio: 16/9)` 고정, 부모 너비 채움 |
| skin.layout_type == 'vertical' | 프리뷰 widget `AspectRatio(aspectRatio: 9/16)`, 높이 600 `LogicalPixels` 기준 |
| skin.layout_type == 'both' | `SegmentedButton` 토글 "Horizontal / Vertical" 제공, 선택 시 `setState` 로 AspectRatio 재계산 |

### 8.5 리소스 정리

Rive `StateMachineController` / Artboard 는 GPU 리소스를 점유한다. `State.dispose()` 에서 반드시 cleanup:

```dart
@override
void dispose() {
  _smc?.dispose();        // StateMachineController
  _artboard?.dispose();   // 필요 시
  super.dispose();
}
```

메모리 누수 체크: Import 후 프리뷰 5회 반복 후 DevTools `Memory` / `Performance` 탭에서 Artboard·StateMachineController instance 가 leak 없이 회수되는지 확인 (QA 체크리스트).
