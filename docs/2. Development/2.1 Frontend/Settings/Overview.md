...
| 2026-04-27 | **5-level scope 채택** | SG-003 + SG-017 DONE. "글로벌 단일" 표현 폐기. Global / Series / Event / Table / User 5-level scope + Override 우선순위 (User > Table > Event > Series > Global). WSOP LIVE 정렬 (원칙 1). Phase_1_Decision_Queue.md C.1 결정. |

---

## 개요

Settings는 EBS **Lobby의 별도 하위 페이지**로, 오버레이 출력 파이프라인, 그래픽 배치, 수치 표시 형식, 게임 규칙, 통계/리더보드, 테이블 인증/진단/내보내기를 관리한다. **Settings 는 5-level scope 으로 분리** 적용된다 (§Settings Scope 참조). Console PRD v9.7의 5탭(Outputs/GFX/Display/Rules/Stats) + Settings 다이얼로그(Preferences)를 행동 명세 6섹션으로 매핑한다.

---

## Settings Scope (5-level scope, 2026-04-27 결정)

EBS Settings 는 **5-level scope** 으로 분리되어 적용된다. 하위 scope 가 상위를 override.

| Scope | 적용 범위 | 예시 키 |
|-------|-----------|---------|
| Global | EBS 인스턴스 전체 기본값 | 시스템 색상 테마, 기본 언어 |
| Series | 시리즈 (예: WSOP 2026) 단위 | 시리즈 로고, 시리즈 색상 |
| Event | 이벤트 (예: WSOP 2026 Main Event) 단위 | 이벤트 룰셋, 이벤트 디스플레이 |
| Table | 테이블 단위 | 테이블별 RFID 매핑, 테이블별 카메라 |
| User | 사용자 별 (운영자 별 personal) | `twoFactorEnabled`, 개인 단축키 |

**Override 우선순위 (가장 높은 우선)**: User > Table > Event > Series > Global

- 어느 scope 에서도 명시되지 않은 키는 Global 기본값 사용.
- 일부 키 (예: `twoFactorEnabled` per Decision SG-008-b14) 는 User scope 만 적용.
- Series / Event / Table scope 는 WSOP LIVE Confluence 패턴과 정렬 (원칙 1).

### 참조

- Spec_Gap_Registry SG-003, SG-017, SG-008-b14
- `docs/4. Operations/Phase_1_Decision_Queue.md` (2026-04-27 C.1 결정)
- MEMORY `feedback_settings_global` (2026-04-15 결정 — 5-level scope 채택으로 supersedes)

> 구 Console PRD v9.7 §2.7~§2.10 매핑은 history 로만 보존 — Lobby Settings 통합 (2026-04-15) 후 별도 파일 없음.
