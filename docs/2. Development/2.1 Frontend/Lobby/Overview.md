...
Lobby와 Command Center는 **별도 앱**이다. WSOP LIVE 는 Staff Page(웹) + WSOP LIVE(Flutter 앱) 분리 구조이나, **EBS 는 Foundation §5.1 (2026-04-21) 결정으로 두 앱 모두 Flutter 프레임워크로 통일** (원칙 1 의도적 divergence — Rive 런타임 일치, 내부 앱 개발팀 생산성, `ebs_common` Dart 패키지 재사용). 배포 형태는 Lobby = **Flutter Web** (Docker nginx, 브라우저 접속), CC = **Flutter Web** (Docker nginx) — Multi-Service Docker (2026-04-27, SG-022 폐기).

| 항목 | Lobby | Command Center |
|------|-------|---------------|
| **기술** | Flutter Web 앱 (Dart + Riverpod + Freezed + go_router + Rive) — Docker `ebs-lobby-web:3000` | Flutter Web 앱 (Dart + Rive) — Docker `ebs-cc-web:3001` |
