...
| 2026-05-04 | Type C 해소 (F1) | ~~Foundation §5.1 (2026-04-21) Flutter Desktop 통일~~ → **2026-04-27 multi-service Docker (lobby-web :3000 Web + cc-web :3001 Flutter Web)** 로 SUPERSEDE 명시. `Lobby/References/EBS_Lobby_Design/` (2026-04-29 HTML/React) 디자인 자산을 SSOT 로 인정. 사용자 의사결정 (`docs/4. Operations/Lobby_Modification_Plan_2026-05-04.md` §F1). |
...
Lobby와 Command Center는 **별도 앱**이다. WSOP LIVE 는 Staff Page(웹) + WSOP LIVE(Flutter 앱) 분리 구조이며, **EBS 도 동일 패턴 채택** — Lobby = Web (lobby-web Docker 컨테이너 :3000), CC = Flutter Web (cc-web Docker 컨테이너 :3001).

> **이력**: ~~Foundation §5.1 (2026-04-21) Flutter Desktop 통일~~ 결정은 **2026-04-27 multi-service Docker SSOT cascade 로 SUPERSEDE** 됨. cascade 근거: ① 2026-04-29 design SSOT (`Lobby/References/EBS_Lobby_Design/`, HTML/React) 가 Web 가정으로 작성 ② multi-service Docker (lobby-web :3000 + cc-web :3001 + ebs-net) 로 LAN 다중 접속 요구 충족 ③ 사용자 명시 결정 (2026-05-04 Lobby_Modification_Plan_2026-05-04.md F1 의사결정 — 디자인 자산 SUPERSEDE 권한 인정). 자세히: `docs/4. Operations/Lobby_Modification_Plan_2026-05-04.md` §F1, memory `project_multi_service_docker_2026_04_27`.

| 항목 | Lobby | Command Center |
|------|-------|---------------|
| **기술** | Web (브라우저, lobby-web Docker 컨테이너 :3000) | Flutter Web (cc-web Docker 컨테이너 :3001) |
...
| Player | Player List (독립 레이어) | Lobby 헤더 사이드바 |
...
| Game Type | Select | O | NL Hold'em | 22종 게임 — 권위: `../../2.3 Game Engine/Behavioral_Specs/Lifecycle_and_State_Machine.md` §2.6 `event_game_type` Enum + `Variants_and_Evaluation.md` §2.1 25 게임 마스터 (구 BS-06-00-triggers, 2026-04-27 통합) |
