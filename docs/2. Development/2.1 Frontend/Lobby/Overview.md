...
Lobby와 Command Center는 **별도 앱**이다. WSOP LIVE 는 Staff Page(웹) + WSOP LIVE(Flutter 앱) 분리 구조이나, **EBS 는 Foundation §5.1 (2026-04-21) 결정으로 Lobby 도 Flutter Desktop 으로 통일** (원칙 1 의도적 divergence — Rive 런타임 일치, 내부 앱 개발팀 생산성, `ebs_common` Dart 패키지 재사용).

| 항목 | Lobby | Command Center |
|------|-------|---------------|
| **기술** | Flutter Desktop 앱 (Dart + Riverpod + Freezed + go_router + Rive) | Flutter Desktop 앱 (별도 창, Dart + Rive) |
