...
| 2026-05-07 | v3 정체성 정합 | Lobby v3.0.0 cascade — 5분 게이트웨이 + WSOP LIVE 거울 정체성 반영. §개요 첫 단락에 정체성 박스 추가, §Lobby-Command Center 관계에 게이트웨이 framing 추가, §화면 계층 흐름에 4 진입 시점 카탈로그 신설. 기존 본문/changelog 변경 0 (additive only). 외부 인계 PRD 동기화 (`derivative-of: Lobby.md`). |
| 2026-05-11 | Cycle 2/3 cascade — 1 hand 자동 셋업 | Lobby v3.0.2 (§Ch.1.7 + 부록 H) 와 team1-frontend Cycle 2 wire 의 본 정본 측 반영: 본문 narrative 변경 0, derivative-of cascade 정합만 갱신 (last-updated 2026-05-07 → 2026-05-11). 실구현 코드 = `team1-frontend/lib/features/lobby/providers/hand_auto_setup_provider.dart` (Cycle 2 PR #258, 머지 commit `55795d91`). flight_status 정수 enum 정합 = `team1-frontend/lib/models/enums/flight_status.dart` ↔ `integration-tests/scenarios/60-event-flight-status-enum.http`. cascade:lobby-hand-ready broker publish seq=64. |
| 2026-05-13 | **Cycle 21 Wave 1 — Reports 폐기 + HandHistory DB 연동** | 사용자 결정 (2026-05-13). (A) Reports 탭 폐기: 4 진입 시점 ④ narrative 에서 Reports 제거 (`Hand History → Reports → Settings` → `Hand History → Settings`). 화면 표에서 `Reports.md` SSOT reference 제거. (B) Hand History 본문 보강 (§화면 6): mockup → BO REST API 직접 연동 명시 (`GET /api/v1/hands` + `GET /api/v1/hands/{id}` nested). 신규 contract `Players_HandHistory_API.md` (BO 2.2 APIs, v1.0). (C) Players 화면 보강: `GET /api/v1/players` BO REST 연동 명시. (D) Lobby Renewal cascade 보강 노트 추가. additive only — narrative 변경 0 (Reports row 제거 외). 후속: W2 backend impl + W3 frontend impl (`lib/features/hand_history/`). |
| 2026-05-15 | **Skin Editor 폐기 — Lobby Settings GFX 탭 흡수** | 사용자 결정 (2026-05-15). Graphic Editor 독립 탭 (Lobby 헤더 `[Graphic Editor]` 버튼) 폐기. 스킨 선택/활성화 기능은 **Settings GFX 탭 §6 Active Skin 관리** 에 흡수. 상세 편집(Upload/Metadata/Preview)은 EBS 범위 외 — 디자이너 Rive 공식 에디터 담당. 화면 표의 `Graphic Editor` 행 폐기 처리. 폐기 PRD: `docs/_archive/discarded-2026-05-15/skin-editor/`. |
| 2026-05-13 | **Cycle 22 Wave 2 — Lobby cascade repair (Hand History 독립 강조)** | Cycle 21 Wave 1 spec PR (`#444`) cascade 정합 후속. 본 Wave 2 는 spec 변경 0 — cascade 정합 보강만 수행. (A) §UI 화면 설계 섹션의 Reports 잔존 흔적 점검 완료 — 본문 잔존 0 (Cycle 21 narrative 제거 정합). (B) §화면 표 (line 295-305) 의 `Hand History (화면 6)` 행에 "**독립 화면으로 격상** (Cycle 21)" 명시. (C) 4 진입 시점 ④ narrative 추가 검증 — `Hand History` 가 Tools 사이드바 보조 화면이 아닌 **독립 feature** 임을 §화면 6 헤더에 cross-reference 표기. (D) derivative-of cascade 영향 파일: `docs/1. Product/Lobby.md` (외부 인계 PRD) + `docs/2. Development/2.1 Frontend/2.1 Frontend.md` — 본 Wave 2 narrative 변경 0 이므로 PRD 갱신은 S2 Cycle 23 W3 후속 인계. additive only. |
...
> **정체성 (Lobby v3.0.0 cascade, 2026-05-07)**: Lobby = **5분 게이트웨이 + WSOP LIVE 거울**.
...
> 본 정체성은 외부 인계 PRD `docs/1. Product/Lobby.md` v3.0.0 의 narrative SSOT 와 정합한다. 이전 v2.0.x "관제탑" framing 은 supersede 됨.
...
> **역할 분담 (Lobby v3.0.0 정체성, 2026-05-07)**: 운영자가 **머무는 화면 = Command Center** (한 테이블 / 한 핸드 / 한 베팅), **거치는 게이트웨이 = Lobby** (CC 진입 / 이탈 / 어긋났을 때 회귀). 1 Lobby ↔ N Command Center 의 1:N 관계는 이 정체성의 구조적 표현이다 — 운영자는 한 Lobby 화면에서 N 개 CC 를 게이트웨이 + 모니터링 한다.
...
| Player | Player List (독립 레이어) — BO REST `GET /api/v1/players` 연동 (Cycle 21, `Players_HandHistory_API.md`) | Lobby 헤더 사이드바 | `visual/screenshots/ebs-lobby-05-players.png` |
| **Hand History** (화면 6, **독립 화면 / Cycle 21**) | EBS 고유 — Hand Browser / Detail / Player Stats 3 서브메뉴, 당일 한정. **독립 feature 격상** (Reports 탭 폐기 후, `lib/features/reports/` 통합 → `lib/features/hand_history/` 독립). BO REST `GET /api/v1/hands` 연동 (Cycle 21, `Players_HandHistory_API.md`) | Lobby 사이드바 `■ Hand History` (`Hand_History.md` SSOT) | `visual/screenshots/ebs-lobby-06-hands.png` |
...
| ~~**Graphic Editor** (2026-04-15 신설)~~ **⛔ 폐기 (2026-05-15)** | ~~Skin Editor — Import/Metadata/Activate/GE 8모드 편집~~ → 스킨 선택/활성화만 Settings GFX §6 에 흡수됨 | ~~Lobby 헤더 `[Graphic Editor]` 독립 버튼~~ 제거됨 | `docs/_archive/discarded-2026-05-15/skin-editor/` |
...
>
> **변경 2026-05-15 (사용자 결정 — Skin Editor 폐기)**:
> - **Skin Editor 기능은 Lobby Settings 의 GFX 탭 (Active Skin 섹션) 에 흡수됨.** 스킨 선택·활성화만 제공. Upload/Preview/Metadata 편집 기능은 EBS 범위 외.
> - Lobby 헤더 `[Graphic Editor]` 버튼 제거. 화면 표 `Graphic Editor` 행 폐기.
> - ~~**[변경 2026-05-15]** Graphic Editor 독립 탭 전면 폐기. 스킨 선택/활성화만 Settings GFX §6 에 잔존. `Skin Editor 기능은 Lobby Settings 의 GFX 탭 Active Skin 섹션에 흡수됨`.~~
...
> **변경 2026-05-13 (Cycle 21 Wave 1, 사용자 결정)**:
> - **Reports 탭 폐기**: `Reports.md` SSOT 및 `lib/features/reports/` Flutter 통합 구현 모두 폐기. WSOP LIVE Staff App §09 Reports 영역은 EBS 운영 5분 게이트웨이 가치에서 제외.
> - **Hand History 독립화**: `lib/features/hand_history/` 독립 feature 로 격상. BO REST API (`Players_HandHistory_API.md` v1.0) 와 직접 연동 — `GET /api/v1/hands` + `GET /api/v1/hands/{id}` (nested hand_players + hand_actions).
> - **Players 화면 연동**: `GET /api/v1/players` BO REST API 연동 명시 (Player List 독립 레이어).
> - 4 진입 시점 ④ narrative: "Tables → Hand History → Reports → Settings (signout)" → "Tables → Hand History → Settings (signout)".
...
### 4 진입 시점 카탈로그 (Lobby v3.0.0 정합, 2026-05-07)
...
| **④ 모든 것이 끝날 때** | 운영 종료 — Flight Completed / 방송 종료 / 시리즈 마감 | Tables → Hand History → Settings (signout) | `Hand_History.md`, `Operations.md` |
...
> **Cycle 17 cascade — 4 핵심 필드 강제 (2026-05-13)**: Player Dashboard 에는 항상 **Name + Country (국기+ISO) + Position (D/SB/BB/UTG/MP/CO/HJ) + Stack** 4 필드가 동시 표시되어야 한다. Position 은 hand-time derived value 이며 BO 미응답 시 `—` 표시. 4 필드 누락 시 cascade 위반. 정합 코드: `team1-frontend/lib/features/players/screens/players_screen.dart` (Country/Pos/Stack 컬럼) + `team1-frontend/lib/models/entities/player.dart` (`position` 필드, `@JsonKey(name: 'position')`).

**EBS 목업:** VPIP/PFR/AGR 컬럼 추가, Feature Table Player 금색 강조, Position chip

![EBS Player List — 재설계 (Peg&Swap/Export 제거, VPIP+PFR+AGR+Feature+Position 추가)](visual/screenshots/ebs-lobby-05-players.png)

| 요소 | DB 소스 | EBS 적용 | Cycle 17 강제 |
|------|---------|:-------:|:-----------:|
| **상단 요약** | | | |
...
| **플레이어 리스트 (4 핵심 필드)** | | | |
| Place (순위) | 계산 | O | — |
| **Name** (이름) | `players.first_name + last_name` | O | ✅ **#1 핵심** |
| **Country** (국기 + ISO Alpha-2) | `players.country_code`, `nationality` | O | ✅ **#2 핵심** |
| **Position** (D/SB/BB/UTG/MP/CO/HJ) | hand-time derived (Game Engine cascade) | O | ✅ **#3 핵심** |
| **Stack** (칩스택) | `table_players.chips` | O | ✅ **#4 핵심 (실시간)** |
...
#### Position chip 색상 매핑 (Cycle 17)

| Position | 의미 | UI 색상 |
|:--------:|------|--------|
| **D** | Dealer button | `amber.shade200` 배경 / `amber.shade900` 텍스트 |
| **SB** | Small Blind | `blue.shade100` 배경 / `blue.shade800` 텍스트 |
| **BB** | Big Blind | `indigo.shade100` 배경 / `indigo.shade800` 텍스트 |
| UTG / MP / CO / HJ | 기타 포지션 | `grey.shade200` 배경 / `grey.shade800` 텍스트 |
| `null` / empty | 미할당 (방송 외 시간 또는 BO 미응답) | `—` (회색 텍스트) |

#### 국기 emoji 변환 (Cycle 17)

- 입력: ISO-3166 alpha-2 (예: `KR`, `US`, `JP`)
- 변환: 각 문자 → Regional Indicator Symbol (`0x1F1E6 + (code - 0x41)`)
- 출력: `🇰🇷`, `🇺🇸`, `🇯🇵` + ISO code 텍스트 병기
- 잘못된 입력 (길이 != 2, 또는 A-Z 범위 외): 빈 emoji (텍스트만 표시)

---

### 화면 6: Hand History (독립 feature, Cycle 21 격상)
...
>
> **독립 feature 격상 (Cycle 21 Wave 1, 2026-05-13)**: Reports 탭 폐기 결정 (사용자, 2026-05-13) 에 따라 Hand History 는 **Tools 사이드바 보조 화면이 아닌 독립 화면 / 독립 feature 로 격상**. Flutter 구현 위치 = `lib/features/hand_history/` (이전 `lib/features/reports/` 통합 폐기). UI sidebar 진입점은 `UI.md §좌측 사이드바 §Hand History` SSOT.
>
> **데이터 출처 (Cycle 21, 2026-05-13)**: prototype mockup data 가 아닌 **BO REST API** 와 직접 연동. 사용 contract: `Players_HandHistory_API.md` — `GET /api/v1/hands` (list, cursor pagination) + `GET /api/v1/hands/{id}` (nested hand_players + hand_actions). DB 스키마: `players` / `hands` / `hand_players` / `hand_actions` (`Schema.md` §players, §hands SSOT). Reports 탭 폐기로 (Cycle 21 사용자 결정) Hand History 는 `lib/features/hand_history/` 독립 feature 로 격상.
