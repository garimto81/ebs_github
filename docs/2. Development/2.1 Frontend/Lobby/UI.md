...
> **정체성 (Lobby v3.0.0 cascade, 2026-05-07)**: Lobby = **5분 게이트웨이 + WSOP LIVE 거울**. 운영자가 머무는 화면이 아니라 거치는 화면이다. 4 진입 시점 (① 처음 진입 / ② 어긋났을 때 / ③ 게임 바뀔 때 / ④ 모든 것이 끝날 때) 에 5 화면 시퀀스 (Series → Events → Flights → Tables → Players) 를 통과한다. 본 UI 정본은 그 시퀀스 각 화면의 widget·필드·상태 spec 을 정의한다. 4 진입 시점 카탈로그는 `Overview.md §4 진입 시점 카탈로그` SSOT.
...
> 근거: `Backend_HTTP.md` (legacy-id: API-01) §`POST /Tables/Rebalance` (CCR-010/CCR-020). G-05 gap 해결.
...
> **✅ CCR-017 APPLIED (2026-04-10)** — `contracts/specs/BS-01-auth/BS-01-auth.md §Permission Bit Flag` (구 BS-01-02-rbac.md, 통합됨), `BS-02-lobby/BS-02-02-event-flight.md`, `BS-02-lobby/BS-02-03-table.md`, `Settings/Rules.md` (legacy-id: BS-03-04), `contracts/data/DATA-04-db-schema.md` 5개 파일에 EventFlightStatus / isRegisterable / dayIndex / BlindDetailType / isPause / Bit Flag Permission 확장 완료. 승격본: `docs/05-plans/ccr-inbox/promoting/CCR-017-wsop-parity.md`.
...
| `BlindDetailType` enum (`Blind/Break/DinnerBreak/HalfBlind/HalfBreak`) + Late Reg 남은 시간 계산식 + Mix Game 특수 규칙(G-02) | `../Specs/BS-03-settings/`Settings/Rules.md` (legacy-id: BS-03-04) §5` | 계산식은 variant 개수와 무관. `flight.is_pause == true` 시 elapsed 증가 멈춤. |
...
| CCR | 상태 | 변경 대상 | 관련 섹션 |
|-----|------|----------|----------|
| **CCR-017** wsop-parity (EventFlightStatus / isRegisterable / dayIndex / Restricted / BlindDetailType / isPause / Bit Flag RBAC) | ✅ APPLIED | `contracts/specs/BS-01-auth/BS-01-auth.md §Permission Bit Flag` (구 BS-01-02-rbac.md 통합됨), `BS-02-lobby/BS-02-02-event-flight.md`, `BS-02-lobby/BS-02-03-table.md`, `Settings/Rules.md` (legacy-id: BS-03-04), `contracts/data/DATA-04-db-schema.md` | §9.1 ~ §9.5 전체 |
...
| **CCR-025** bs03-graphic-settings-tab (`Settings/Graphics.md` (legacy-id: BS-03-02) 시각 asset 메타 확장) | ✅ APPLIED | `Settings/Graphics.md` (legacy-id: BS-03-02) | (간접) Settings GFX 탭의 Team 4 기여 필드 |
...
|-----------|------|----------|----------|
| `CCR-DRAFT-team1-20260413-google-oauth.md` | 🔶 DRAFT | `BS-01-auth.md`, `Auth_and_Session.md` (legacy-id: API-06) | §0 Login Google OAuth |
