...
자매 영역 PRD 미존재 (Login_PRD, Settings_PRD, Graphic_Editor_PRD 부재). `Lobby_PRD.md` 만 정점 외부 PRD. → derivative-of 검증 = N/A (PRD-less sub-feature).

**결론**: PASS. 자매 영역은 Lobby_PRD 의 sub-section 으로 흡수되거나 정본 자체가 SSOT (PRD 분리 미설계).
