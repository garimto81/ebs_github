...
### 2.2 프로세스 경계 및 IPC 금지 (Foundation Ch.5 §B.3 (옛 §6.3))

Foundation Ch.5 §B.3 (옛 §6.3) 은 **앱 간 직접 IPC 금지** 를 명문화한다. 다중창 모드에서 각 앱은 별개 OS 프로세스이나 shared memory / 파이프 / 소켓으로 직접 통신하지 않는다.
...
### 6.0 DB polling + WS push 2채널 모델 (Foundation Ch.5 §B.4 (옛 §6.4), SG-002 해소)

Foundation Ch.5 §B.4 (옛 §6.4) 는 **DB 를 단일 진실(SSOT)** 로 두고, 두 가지 채널로 동기화를 수행한다:
...
> **⚠ DB polling endpoint 실제 스키마는 team2 SSOT**. Foundation Ch.5 §B.4 (옛 §6.4) 는 정책 선언, team2 가 Wave 2 에서 `docs/2. Development/2.2 Backend/APIs/` 에 발행. 본 섹션은 Lobby 관점 소비 패턴만 기술.
