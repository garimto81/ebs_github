...
| 패턴 | 의미 | 예시 |
|------|------|------|
| `X-nn` | v4 annotation 정본 ID | S-07, G-04, Y-06 |
| `X-nn.m` | 하위 세부 기능 (m = 1, 2, 3…) | G-10.1 (Sponsor Logo 1) |
| `X-nn.tm` | DataTable 컬럼 (t = table, m = 컬럼 순번) | S-01.t1 (L/R Source) |

### 2.6 Sources 탭 기능 상세

Sources 탭은 방송 입력 파이프라인을 제어한다. 3-Column 그리드 내에 **Video Input**, **Chroma Key**, **ATEM Control** 서브그룹을 배치한다.

#### Video Input (S-01, S-01.t1~S-01.t5)

비디오 소스 목록을 DataTable로 관리한다. 운영자는 NDI, 캡처카드, 네트워크 카메라를 등록하고 상태를 모니터링한다.

| 컬럼 | ID | 기능 | 조작 |
|------|:--:|------|------|
| L/R Source | S-01.t1 | 좌/우 비디오 소스 할당 (X 마커) | 클릭 토글 |
| Format/Input/URL | S-01.t2 | 소스 포맷 및 입력 경로 표시 | 읽기 전용 |
| Cycle | S-01.t3 | 소스 순환 표시 시간 (초, 0=제외) | 숫자 입력 |
| Status | S-01.t4 | ON/OFF 활성 상태 | 토글 |
| Action | S-01.t5 | Preview 토글 + Settings 버튼 | 클릭 |

**동작**: 소스가 ON으로 전환되면 Preview Area에 해당 피드가 실시간 합성된다. 소스 해상도가 출력 해상도(O-02)와 불일치하면 자동 스케일링을 적용한다.

> **[DROP]** Auto Camera Control (SV-002) — 게임 상태 기반 자동 카메라 전환 전체 배제. 카메라 전환은 프로덕션 팀(스위처)이 수동 운영한다.

#### Chroma Key (S-07, S-08)

| 요소 | ID | 기능 | 기본값 |
|------|:--:|------|--------|
| Enable | S-07 | 크로마키 활성화 체크박스 | OFF |
| Background Colour | S-08 | 배경색 ColorPicker | #0000FF (Blue) |
...
#### ATEM Control (S-11.1, S-11.2, S-12)

외부 하드웨어 스위처(Blackmagic ATEM)와 연동하여 Fill & Key 출력을 제어한다.

| 요소 | ID | 기능 |
|------|:--:|------|
| ATEM IP | S-12 | 스위처 IP 주소 입력 (TextField) |
| Switcher Source | S-11.1 | ATEM 입력 소스 선택 (Dropdown) |
| ATEM Control Enable | S-11.2 | 스위처 제어 활성화 (Checkbox) |
...
| 요소 | ID | 기능 |
|------|:--:|------|
| Live Video/Audio/Device | O-07~O-09 | Live 출력 3개 드롭다운 (DeckLink/NDI 채널 선택) |
| Live Key & Fill | O-06.1 | Fill & Key 듀얼 출력 활성화 (외부 키잉 장치 선택 시) |
...
#### Fill & Key (O-11.1, O-16, O-11.2)

Fill & Key 모드 활성화(O-06.1) 시에만 표시되는 서브그룹.

| 요소 | ID | 기능 | 기본값 |
|------|:--:|------|--------|
| Fill & Key Color | O-11.1 | Key 신호 배경색 | #FF000000 (투명 블랙) |
| Fill/Key Preview | O-16 | Fill 신호와 Key 신호 나란히 미니 프리뷰 (듀얼) | — |
| DeckLink Channel Map | O-11.2 | Fill/Key → DeckLink 물리 포트 매핑 | — |

**동작**: O-16의 듀얼 미니 프리뷰는 Fill(오버레이 합성 영상)과 Key(흑백 알파 마스크)를 나란히 보여준다. 운영자는 Key 신호에서 오버레이 영역이 올바르게 마스킹되는지 실시간으로 확인할 수 있다. O-11.2에서 DeckLink 포트를 잘못 매핑하면 Fill/Key 신호가 뒤바뀌므로 설정 확인 알림을 제공한다.
...
|----|-------------|---------|-------------|
| Outputs | Video Size (O-02) | — | 렌더링 캔버스 해상도 결정 |
| Outputs | Frame Rate (O-04) | — | 오버레이 렌더링 fps 결정 |
| Sources | Chroma Key (S-07~S-08) | — | Preview + 출력 배경색 |
...
**Version**: 3.9.0 | **Updated**: 2026-03-06
