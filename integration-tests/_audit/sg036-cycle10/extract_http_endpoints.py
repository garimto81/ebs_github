...
Cycle 10 fix: properly handle `{{path_var}}` as path params (not host prefix).

Outputs JSON: list of {file, method, path_raw, path_normalized}
- path_normalized: {{var}} → {} for path params, then strip leading host {}
...
def normalize(path: str) -> str:
    # strip query string
    p = path.split("?", 1)[0]
    # Convert {{var}} → {} (treat all handlebars as path param placeholders)
    p = VAR_RE.sub("{}", p)
    # Strip leading host {} (the very first {} is the URL prefix like {{host}})
    while p.startswith("{}"):
        p = p[2:]
    # collapse path params {var} → {}
    p = PARAM_RE.sub("{}", p)
