...
> 참조: HTML 목업 `ui-design/reference/action-tracker/mockups/` 로 대칭 검증.
...
**배치 규칙**:
- **D(Dealer)**: 하단 중앙
- **D 왼쪽 (시계방향)**: S1(SB) → S2(BB) → S3 → S4 → S5
- **D 오른쪽 (반시계방향)**: S10 → S9 → S8 → S7 → S6
- **좌우 대칭**: D를 중심축으로 왼쪽 5석, 오른쪽 5석
...
> **화면 2(좌석 상세 패널)는 제거되었다.** 대신 좌석 위젯의 각 요소를 직접 탭하여 수정한다.
...
> **변경**: Quick preset, 슬라이더 **제거**. 키패드 + '000' 버튼으로 아주 심플하게 설계.
...
| 요소 | 타입 | 설명 |
|------|------|------|
| Amount Field | TextField (mono) | 직접 입력, Roboto Mono |
| Numpad | 4×4 그리드 | 0-9, C(초기화), <-(삭제), **000**(천 단위 빠른 입력) |
| [ALL-IN] | Button (danger 색상) | **BET 버튼 위에 별도 배치**, stack 전액 |
...
> **제거된 항목**: Quick Preset(MIN/1/2 POT/POT), 슬라이더, `> player.stack` 자동 ALL-IN 전환.
...
> 위 12건은 team4-cc/specs/BS-05-command-center/ (L1 파생 문서)에서 자체 반영한 뒤, contracts/ (L0 계약)와의 차이를 `CCR-DRAFT-team4-YYYYMMDD-bs05-ui02-redesign.md`로 제출해야 한다.
