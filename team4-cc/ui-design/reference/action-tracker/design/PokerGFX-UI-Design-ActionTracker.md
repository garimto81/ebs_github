...
Action Tracker(이하 AT)는 PokerGFX 7개 앱 생태계에서 EBS Console(구 GfxServer UI) 다음으로 중요한 클라이언트 앱이다. 내부 키 `pgfx_action_tracker`. 본방송 중 운영자 주의력의 **85%가 AT에 집중**된다.
...
| 이유 | 설명 |
|------|------|
| 실수 방지 | Console 설정 오조작 차단 — AT는 액션 입력만 노출 |
...
- AT는 EBS Server에 액션을 **전송**하고, Server는 전체 GameState를 **브로드캐스트**

### Console Settings — AT 설정
...
sequenceDiagram
    participant AT
    participant SV as EBS Server
    AT->>SV: UDP Broadcast (:9000)
    SV-->>AT: 서버 발견 응답
    AT->>SV: TCP Connect (:9001)
    AT->>SV: AUTH (Password, Version)
    SV-->>AT: GAME_STATE
    SV-->>AT: GAME_INFO (75+ fields)
    SV-->>AT: PLAYER_INFO × N
...
| 우측 | Director Console | EBS Console 열기 |
...
<!-- PokerGFX 정밀 분석: Setup Mode — Vision 재분석 후 삽입 예정 (Step 4) -->
...
<!-- PokerGFX 정밀 분석: Pre-Flop / Post-Flop / Statistics — Vision 재분석 후 삽입 예정 (Step 4) -->
...
**UNDO**: 항상 표시, 최대 5단계 되돌리기. `UndoLastAction` → EBS Server GameState 복원 → 전체 클라이언트 브로드캐스트.
...
<!-- PokerGFX 정밀 분석: Card Selector — Vision 재분석 후 삽입 예정 (Step 4) -->
...
<!-- PokerGFX 정밀 분석: RFID Registration — Vision 재분석 후 삽입 예정 (Step 4) -->
...
## §9. 디자인 가이드라인 (업계 최선 사례)

웹 리서치(2026-03-13) 기반. 방송 운영 콘솔, 미션 크리티컬 HMI, UX 연구 문헌에서 AT에 직접 적용 가능한 가이드라인을 도출했다.

### §9.1 색상 체계 — Astro UXDS 6-State

**출처**: [Astro UXDS Status System](https://www.astrouxds.com/patterns/status-system/) — 미 우주군 지상 운영 콘솔용 검증 색상 체계

§3.4 좌석 상태별 시각 처리를 6-state 스케일로 정규화:

| AT 좌석 상태 | Astro 매핑 | 색상 (Hex) | 근거 |
|-------------|-----------|-----------|------|
| Empty | Off | `#A4ABB6` (gray) | 비활성 |
| Sitting Out | Standby | `#2DCCFF` (cyan) | 대기 |
| Active (in hand) | Normal | `#56F000` (green) | 정상 참여 |
| Short Stack (<10BB) | Caution | `#FCE83A` (yellow) | 주의 |
| All-in | Serious | `#FFB302` (orange) | 고위험 |
| Action-on (현재 턴) | Critical | `#FF3838` (red border) | 즉시 행동 필요 |
| Folded | — | 50% opacity + gray | 비참여 |

**배경색**: `#1E2124` (다크 그레이). High Performance HMI 표준 — 순수 블랙 아님 (눈부심 방지). 정상 상태는 모노크롬, 색상은 예외 상태 전용.

### §9.2 버튼 사이징

**출처**: [NN/G Touch Targets](https://www.nngroup.com/articles/touch-target-size/), [WCAG 2.5.5](https://www.w3.org/WAI/WCAG21/Understanding/target-size.html), [Fitts's Law](https://www.nngroup.com/articles/fitts-law/)

| 버튼 유형 | 최소 사이즈 | 권장 사이즈 | 간격 |
|----------|:---------:|:---------:|:---:|
| 핵심 액션 (F/C/B/A) | 56×56px | **72×72px** | 12px |
| 보조 액션 (UNDO, TAG) | 44×44px | 56×56px | 8px |
| 숫자 키패드 | 48×48px | 56×56px | 8px |

핵심 액션 버튼 클러스터: 화면 하단 중앙 밀집 배치. Fitts's Law — 다음 타겟까지 거리 최소화 우선.

### §9.3 키보드 우선 설계

**출처**: [Ross XPression](https://www.rossvideo.com/products/graphics-and-virtual/xpression/), [vMix Scoreboard Keyboard](https://www.vmix.com/knowledgebase/article.aspx/66/), [Knock Shortcut Design](https://knock.app/blog/how-to-design-great-keyboard-shortcuts)

| 원칙 | 구현 | 참조 |
|------|------|------|
| Embedded Badge | 버튼 우하단에 단축키 문자 배지 | Linear, Superhuman |
| 키 피드백 | 키 입력 시 95% 축소 애니메이션 (80-100ms) | Material Design 3 |
| Last Action Zone | "Fold — Seat 3" 확인 텍스트 2초 표시 | Viz Trio CUE/TAKE |
| F1 오버레이 | 첫 세션 시 3-4초 단축키 맵 자동 표시 | Apple "hold Command" |
| 숫자 키 좌석 선택 | 1-9 = 좌석 선택, 0 = Seat 10 | vMix 넘버 패드 |

§6 단축키 확장:

| 키 | 동작 | 카테고리 |
|----|------|---------|
| 1-9, 0 | 좌석 1-10 직접 선택 | 좌석 |
| Z | UNDO (어디서든 즉시 실행) | 보조 |
| H | 히스토리 패널 토글 | 보조 |
| Enter | 베팅 금액 확인 | 입력 |
| Esc | 취소 / 키패드 닫기 | 입력 |

### §9.4 상태 머신 UI — Progressive Disclosure

**출처**: [NN/G Progressive Disclosure](https://www.nngroup.com/articles/progressive-disclosure/), [Smashing Magazine FSM UI](https://www.smashingmagazine.com/2018/01/rise-state-machines/)

| 원칙 | 설명 |
|------|------|
| 무효 액션 숨김 | 비활성(gray) 아닌 **완전 숨김(hide)** — 스트레스 환경에서 인지 부하 감소 |
| Pipeline Indicator | 상단 고정 수평 단계 바로 현재 street 즉시 파악 |
| 전환 애니메이션 | 상태 배너 150-200ms ease-out cross-fade. 버튼 피드백 80-100ms |
| UNDO 예외 | 항상 표시 — 안전망 인식 필요. 스택 비었을 때만 gray |

### §9.5 NOW ACTING 위젯

**출처**: Chyron PRIME 방송 패턴, [PatternFly Dashboard](https://www.patternfly.org/patterns/dashboard/)

현재 액션 플레이어를 그리드 외 **별도 대형 영역**으로 복제 표시. 10인 그리드를 스캔하지 않고 즉시 확인:

- 플레이어 이름, 스택, 홀카드
- 현재 팟 사이즈
- 가능한 액션 버튼 (FOLD/CHECK/BET 또는 FOLD/CALL/RAISE)

### §9.6 Undo UX — Toast + 즉시 실행

**출처**: [ui-patterns.com Undo](https://ui-patterns.com/patterns/undo), [Fluent 2 Toast](https://fluent2.microsoft.design/)

| 규칙 | 설명 |
|------|------|
| 확인 다이얼로그 금지 | "Never use a warning when you mean undo" — 즉시 실행 + Toast |
| Toast 표시 | 고정 위치에 3초 표시 후 자동 소멸. Z키 Undo 힌트 포함 |
| Redo 미구현 | 라이브 환경에서 Redo는 혼란 유발 |
| 히스토리 패널 | H 키로 토글. 최근 5개 액션 표시. 기본 숨김 |

### §9.7 베팅 입력 강화

**출처**: [JuroJin Bet Sizes](https://jurojinpoker.com/en/solution/betsizes-overlay), [CardsChat Forum](https://www.cardschat.com/forum/online-poker-73/bet-slider-shortcut-buttons-463818/)

| 규칙 | 설명 |
|------|------|
| 슬라이더 금지 | 정확한 금액 기록에 부적합 |
| 금액 표시 프리셋 | "½ Pot ($1,250)" 형식 — 분수 + 계산된 금액 동시 표시 |
| Street별 프리셋 | Pre-flop: 2BB/2.5BB/3BB/4BB/All-in, Post-flop: ⅓/½/⅔/Pot/All-in |
| 3-step 입력 흐름 | B키 → 숫자 입력 → Enter |
| 실시간 미리보기 | 키패드 상단에 입력 중인 금액 대형 표시 |

### §9.8 참조 문헌

#### 방송 운영 콘솔
1. [Viz Trio 4 — Vizrt](https://www.vizrt.com/products/viz-trio/)
2. [XPression — Ross Video](https://www.rossvideo.com/products/graphics-and-virtual/xpression/)
3. [vMix Scoreboard Keyboard](https://www.vmix.com/knowledgebase/article.aspx/66/)

#### 미션 크리티컬 HMI
4. [Astro UXDS Status System](https://www.astrouxds.com/patterns/status-system/)
5. [NASA Open MCT](https://nasa.github.io/openmct/)
6. [High Performance HMI — Emerson](https://www.emersonautomationexperts.com/2023/industrial-internet-things/up-your-productivity-and-safety-with-high-performance-hmi-design-white-paper/)

#### UX 연구
7. [NN/G Touch Target Size](https://www.nngroup.com/articles/touch-target-size/)
8. [NN/G Progressive Disclosure](https://www.nngroup.com/articles/progressive-disclosure/)
9. [Smashing Magazine — Disabled Buttons UX](https://www.smashingmagazine.com/2021/08/frustrating-design-patterns-disabled-buttons/)
10. [ui-patterns.com — Undo](https://ui-patterns.com/patterns/undo)

#### 포커 방송 기술
11. [PokerGFX Features](https://www.pokergfx.io/features)
12. [PokerGO Acquires PokerGFX](https://www.pgt.com/press-releases/pokergo-acquires-leading-poker-graphics-and-stats-company)
13. [wTVision EPT RFID Case Study](https://www.wtvision.com/pokerstarsit-tournament-rfid-table-and-cards-system-used-for-the-first-time-on-a-broadcast)
...
| 2026-03-13 | v1.1.0 | §9 디자인 가이드라인 추가 — 업계 최선 사례 웹 리서치 반영 | PRODUCT | Astro UXDS, NN/G, Ross XPression, Viz Trio 등 20건 참조 |
| 2026-03-16 | v1.3.0 | §3.2 Setup Mode PokerGFX 정밀 annotation 삽입 — 16개 요소 Zone 분해 + 프로토콜 매핑 | PRODUCT | 구현 참조 자료 강화 |
| 2026-03-16 | v1.2.0 | "GfxServer" → 맥락별 분리: UI=Console, 백엔드=EBS Server | TECH | EBS-UI-Design-Console.md 용어 통일 |

---

**Version**: 1.3.0 | **Updated**: 2026-03-16
