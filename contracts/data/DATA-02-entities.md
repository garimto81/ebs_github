...
| 2026-04-13 | series.time_zone | IANA 포맷 명시 (venue 현지 시각 표시용). BS-06-02-clock §2 참조 |
...
| time_zone | TEXT | 대회 개최지 타임존 (IANA 포맷: `"America/Los_Angeles"`, `"Europe/Paris"` 등). **클라이언트가 이 값으로 현지 시각을 계산하여 표시** (WSOP LIVE `SeriesLocalClock` 패턴 준거). clock_tick payload와 무관. | NOT NULL | 'UTC' |
