...
| 2026-04-06 | UI 화면 설계 전면 교체 | ~~5화면 구조~~ → 4화면+Settings 별도 구조로 변경 (2026-04-09), WSOP LIVE 실제 운영 스크린샷 삽입, 화면별 EBS 적용/제거 요소 분류 |
...
| 2026-04-09 | 화면 구조 재배정 + Settings 글로벌 명시 | Event+Flight 화면 통합(accordion 펼침), 5화면→4화면+Settings 별도, 화면 번호 재배정(화면3→Table, 화면4→Player), Settings는 글로벌(모든 CC 동일 적용, 테이블별 Settings 무의미) |
...
WSOP LIVE Staff App은Series/Event/Flight/Table/Player 5계층 구조, 실시간 좌석 관리, Featured Table 운영 등 라이브 포커 대회 운영에 검증된 UI/UX를 갖추고 있다.

EBS Lobby는 이 중 **방송 테이블 운영에 필요한 기능만 선별하여 가져온다**:

| WSOP LIVE Staff App 기능 | EBS Lobby 적용 | 이유 |
|-------------------------|:--------------:|------|
| 5계층 네비게이션 (Series→Event→Flight→Table→Player) | ✅ 그대로 | 데이터 구조가 동일 |
...
![Lobby-Command Center 관계도](../../../images/lobby/ebs-lobby-cc-relationship.png)

Lobby와 Command Center는 **별도 앱**이다. WSOP LIVE가 Staff Page(웹) + WSOP LIVE(Flutter 앱) 분리 구조이므로 동일하게 따라간다.

| 항목 | Lobby | Command Center |
|------|-------|---------------|
| **기술** | 웹 프론트 페이지 (브라우저) | Flutter 앱 (별도 앱) |
| **역할** | 설정/관리 + 활성 Command Center 모니터링 | 실시간 방송 입력 |
| **데이터 방향** | REST API로 DB에 쓰기 + WebSocket 구독 | DB에서 로드 + 핸드 데이터 쓰기 + WebSocket 발행 |
| **동시 사용** | Command Center 활성화 후에도 브라우저에 열려있음 | Lobby와 동시 운영 |
| **직접 연동** | 없음 (Back Office DB 간접) | 없음 (Back Office DB 간접) |

### 1:N 관계 — 하나의 Lobby에서 여러 Command Center 관리

**핵심:**
- Lobby는 **1개** (웹 — 모든 테이블의 관제/설정 허브)
- Command Center는 **테이블당 1개** (Flutter 앱 인스턴스)
...
![앱 전환 흐름 — Launch, Open, 동시 사용](../../../images/lobby/ebs-flow-cc-launch.png)
...
![데이터 연동 흐름 — API→DB→Lobby→Command Center→Overlay](../../../images/lobby/ebs-flow-data-sync.png)
...
![Active Command Center 드롭다운 — 헤더에서 접근](../../../images/lobby/ebs-flow-active-cc-dropdown.png)
...
Command Center에 진입하면 해당 테이블의 전체 경로(Series/Event/Flight/Table)가 **마지막 세션 상태**로 저장된다. 이후 로그인 시 저장된 Command Center로 바로 진입한다.

![세션 복원 흐름 — Login→복원 옵션→Command Center 자동 진입](../../../images/lobby/ebs-flow-session-restore.png)

- Command Center가 호출된 적이 **있으면** → 다음 로그인 시 **바로 Command Center 호출** (마지막 테이블)
- Command Center가 호출된 적이 **없으면** → Lobby 5계층 탐색부터 시작 (최초 Setup)
...
> 참고: 비정상 종료(앱 크래시, 네트워크 단절) 시에도 `user_sessions` 테이블에 마지막 상태가 영구 보존되어 있으므로 동일하게 복원된다.
...
| 용어 | 정의 |
|------|------|
| **Lobby** | EBS의 테이블 관리 화면. Series→Event→Flight→Table→Player 5계층 탐색을 통해 방송 테이블을 찾고 CC로 진입하는 준비 화면 |
...
| **Session** | 사용자의 마지막 선택 상태(Series/Event/Flight/Table). 재접속 시 이전 상태를 복원하여 불필요한 탐색을 제거 |
...
![DB 테이블 관계 — series→events→flights→tables→seats→players](../../../images/lobby/ebs-flow-er-diagram.png)
...
| Player 목록 진입 | `GET /api/tables/:id/players` | 캐시 없음 (실시간) |
...
| `table:player_seated` | Player 목록 + 좌석 시각화 | 좌석 배정 변경 |
...
## UI 화면 설계 — 4화면 + Settings 별도

### 설계 근거

**EBS 로비의 핵심 목적**: Feature Table을 찾아서 Command Center로 진입하는 것. 대회/이벤트/Flight는 테이블을 찾기 위한 탐색 경로이다.

**왜 4화면 + Settings 별도인가**: WSOP LIVE의 데이터 구조는 `시리즈 > 이벤트 > Flight > 테이블 > 플레이어` 5계층이지만, Event와 Flight는 **하나의 화면에서 accordion 펼침**으로 제공한다. Settings는 테이블 종속이 아닌 **글로벌 단위**로 모든 CC에 동일 적용되므로 별도 설계한다.

| 화면 | 내용 | 비고 |
|------|------|------|
| 화면 0 | Login | 인증 + 세션 복원 |
| 화면 1 | Series Lobby | 카드 그리드 |
| 화면 2 | Event + Flight (통합) | Event 행 accordion 펼침 → Flight 인라인 |
| 화면 3 | Table Management | EBS 핵심 화면 |
| 화면 4 | Player List | 조회 전용 |
| Settings | 글로벌 Settings | 모든 CC 공통, 별도 화면 |
...
![EBS Login — 재설계](../../../images/lobby/ebs-lobby-00-login.png)
...
> 로그인 성공 → 역할(Admin/Operator/Viewer) 자동 할당 → 이전 세션 컨텍스트 복원 → 마지막 선택 화면으로 이동. 상세는 BS-01 Auth 참조.
...
![EBS Series Lobby — 재설계](../../../images/lobby/ebs-lobby-01-series.png)
...
![EBS Event List — 재설계 (Prize/Guarantee/Level/Late Reg 제거, Feature Table 뱃지 추가)](../../../images/lobby/ebs-lobby-02-events.png)
...
![Mix 게임 Command Center 화면 — Fixed Rotation + Dealer's Choice](../../../images/lobby/ebs-flow-mix-game.png)
...
![EBS Flight List — Event 행 accordion 펼침](../../../images/lobby/ebs-lobby-03-flights.png)
...
![EBS Table Management — 재설계 (Hold Search/Export 제거, RFID+Deck+Output+Command Center 추가)](../../../images/lobby/ebs-lobby-04-tables.png)
...
| **좌석 그리드** | | |
| 테이블 행 (Day2-#069, #070...) | `tables.name` | O |
...
### 화면 4: 플레이어 정보 (Player List)

> **근거**: 오버레이에 표시할 플레이어 데이터 확인 (칩, BB, 국적). 필요 시 조회.

**EBS 목업:** VPIP/PFR/AGR 컬럼 추가, Feature Table Player 금색 강조

![EBS Player List — 재설계 (Peg&Swap/Export 제거, VPIP+PFR+AGR+Feature 강조 추가)](../../../images/lobby/ebs-lobby-05-players.png)
...
![테이블 상태 전환 UI — Empty→Setup→Live→Completed](../../../images/lobby/ebs-flow-table-status.png)
...
> 정상/비정상 종료 후 재접속 시 이전 상태 복원.

| # | As a | When | Then | Edge Case |
|:-:|------|------|------|-----------|
| K-1 | 모든 역할 | 정상 로그아웃 후 재로그인하면 | 마지막 선택 (Series/Event/Flight/Table) 복원 다이얼로그 표시. Continue/Change Flight/Change Event/Change Series 옵션 | 마지막 선택 항목이 Completed 상태: 해당 항목 건너뛰고 상위 레벨로 이동 |
...
![Command Center Lock 상태 UI — LOCKED/CONFIRM/FREE 필드 구분](../../../images/lobby/ebs-flow-cc-lock.png)
...
| BS-06 Game Engine | 후행 | Lobby 게임 설정이 Engine 초기화 파라미터 |
| BS-07 Overlay | 후행 | Feature Table 오버레이 미리보기 썸네일 |
| WSOP LIVE API | 외부 | 선수 정보 수신 → Back Office DB `players` 테이블에 캐싱 |

![모듈 의존성 — BS-01~07 + DB + API](../../../images/lobby/ebs-flow-dependencies.png)
...
![API 호출 시퀀스 — Lobby↔Server↔DB↔Command Center](../../../images/lobby/ebs-flow-api-sequence.png)

### 수동 입력 → API 동기화 규칙

![수동→API 동기화 흐름 — Manual source→API 매칭→공존 운영](../../../images/lobby/ebs-flow-manual-to-api.png)

---

## Hand History

![Hand History 화면 — 목록 + 상세](../../../images/lobby/ebs-flow-hand-history.png)
...
![ER 다이어그램 — 11개 테이블 관계](../../../images/lobby/ebs-flow-er-diagram.png)
...
| Game Type | Select | O | NL Hold'em | 22종 게임 (BS-06-00-REF enum) |
