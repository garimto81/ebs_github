...
AT-05 RFID Register는 **54장의 RFID 카드 UID를 카드 얼굴(Rank + Suit)과 매핑**하는 운영자 화면이다. 덱 교체, 신규 덱 도입, 카드 손상 시 개별 재등록에 사용된다.
...
| 항목 | 내용 |
|------|------|
| **역할** | 덱의 52장 + Joker 2장을 순차적으로 RFID 리더에 탭하여 UID↔카드 매핑 |
| **페르소나** | Operator 이상 (Viewer 접근 불가) |
| **사용 시점** | 방송 준비 단계, 덱 교체 시, 카드 손상 시 개별 재등록 |
| **총 카드 수** | 54장 = 52장 + Joker 2 (Joker 제외 옵션은 BS-04-01 참조) |
...
3. 시스템이 ♠A → ♠K → ... → ♣2 → Joker1 → Joker2 순서로 순차 요청
   ...
   └─ [Restart] → 모든 등록 초기화
   │
4. 54장(또는 52장) 등록 완료 → "Registration Complete" 다이얼로그
   │
...
- `API-01-backend-endpoints` — Decks API
