...
CC는 Event Sourcing 아키텍처를 사용하여 모든 게임 이벤트를 순서대로 기록한다. Undo는 이 이벤트 로그에서 마지막 이벤트를 되돌려 이전 상태를 복원하는 기능이다. 최대 5단계까지 연속 Undo가 가능하며, 현재 핸드 내에서만 동작한다.
...
| **Undo Stack** | 현재 핸드에서 발생한 이벤트의 스택. 최대 5개 유지 |
...
| 4회 | Player D의 Bet 되돌리기 → biggest_bet_amt = 0 |
| 5회 | Player E의 Check 되돌리기 → action_on = E |
| 6회 이상 | "Undo 최대 5단계입니다" 경고, 추가 Undo 차단 |
...
| 여러 액션 연속 오류 | 연속 Undo (최대 5회) → 정상 지점에서 재입력 |
...
| 4 | 운영자 | 5번 Undo 후 추가 Undo 시도 | "Undo 최대 5단계입니다" 경고 |
