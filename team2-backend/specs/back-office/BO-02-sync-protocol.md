...
| Event 목록 | 5분 | 타이머 |
| Flight 상태 | 30초 | 타이머 |
| Player 벌크 | Event 시작 시 1회 | Event Running 전이 |
| Player 증분 | 30초 | 타이머 (Running Event만) |
| Seat 배치 | 30초 | 타이머 (Running Flight만) |

### 5.2 설정 키

| 설정 키 | 기본값 | 설명 |
|---------|:------:|------|
| `system.wsop_api_poll_sec` | 30 | 기본 폴링 주기 (초) |
| `system.wsop_api_base_url` | — | WSOP LIVE API 기본 URL |
| `system.wsop_api_enabled` | false | 자동 동기화 활성화 여부 |
| `system.wsop_api_timeout_ms` | 5000 | API 호출 타임아웃 |

---

## 6. WSOP LIVE UPSERT 규칙

### 6.1 source 필드 규칙

| source 값 | 의미 | CRUD 권한 | API 동기화 |
|----------|------|----------|:---------:|
| `manual` | 수동 생성 | Admin 자유 수정/삭제 | 대상 아님 |
| `api` | API에서 수신 | API 필드 읽기 전용, EBS 전용 필드만 수정 | 대상 |

**UPSERT 조건:**

| 조건 | 동작 |
|------|------|
| 매칭 키 미존재 (신규) | INSERT, `source='api'` |
| 매칭 키 존재 + API 데이터 변경 | UPDATE (API 필드만) |
| 매칭 키 존재 + 수동 수정 필드 | 수동 수정 필드 보호 (덮어쓰지 않음) |
| 수동 레코드 (`source='manual'`) | API 동기화 대상 아님 |

### 6.2 수동→API 전환

| 시나리오 | 처리 |
|---------|------|
| 수동 Series가 API에서 매칭됨 | `series_name + begin_at` 기준 매칭 → Admin 확인 후 `source='api'` 전환 |
| 수동 Player가 API에서 매칭됨 | `first_name + last_name` 유사도 매칭 → Admin 확인 후 `wsop_id` 연결 |
| 매칭 실패 | 수동 레코드 유지, 사용자 확인 후 처리 |
